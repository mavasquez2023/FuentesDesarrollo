				...
				for (Iterator iterator = rechazos.iterator(); iterator.hasNext();) {
					...
					if(rechazo.getIdTupla()==null){
						...
						if(!rechazo.getCodigoEntidad().trim().equals(rechazo.getCodigoEntidadRechazo().trim())){
							...
							if(opciones.indexOf("C")>-1 && rechazo.getCodigoCausante()!=null && !rechazo.getCodigoCausante().trim().equals(rechazo.getCodigoCausanteRechazo().trim())){
								rechazo.setCausante(true);
							}
							if(opciones.indexOf("B")>-1 && rechazo.getRutBeneficiario()!=null && !rechazo.getRutBeneficiario().trim().equals(rechazo.getRutBeneficiarioRechazo().trim())){
								rechazo.setBeneficiario(true);
							}
							if(opciones.indexOf("E")>-1 && rechazo.getRutEmpleador()!=null && !rechazo.getRutEmpleador().trim().equals(rechazo.getRutEmpleadorRechazo().trim())){
