...
public class ConstruyeXml {
	 ...
	 */
	public String creaXmlActualizacion(CamposXmlTO oCampo, int fechavig) {
		...
		//Tramos
		if(Integer.parseInt(oCampo.getFecIniBeneficio())>=fechavig){
			content += "<IngPromedio>" + oCampo.getIngPromedio().trim() + "</IngPromedio>" + 
			"<MontoUnitarioBeneficio>" + oCampo.getValorTramo().trim() + "</MontoUnitarioBeneficio>"+
			"<TramoAsigFam>" + oCampo.getTramoAsigFam() + "</TramoAsigFam>"+
			"<FecPagoBeneficio>" + oCampo.getFecRecCausante().substring(0, 4) + "-" + oCampo.getFecRecCausante().substring(4, 6) + "-" + oCampo.getFecRecCausante().substring(6, 8) + "</FecPagoBeneficio>";
		}else{
			content += "<Tramos>";
...
*/
	...
	public String creaXml(CamposXmlTO oCampo, boolean vigente ) throws Exception {
	...
	public String creaXmlExtinsion(DatosExtinsionTO fromDB2, TuplaTO fromWS) {
		...
		content = "<?xml version='1.0' encoding='ISO-8859-1' ?>" + " <ExtincionReconocimiento>" + " <Documento version='1.0'>" + " <FechaEmision>" + fromDB2.getFechaEmision() + "</FechaEmision>" + " <FechaExtCausante>" + fromDB2.getFechaExtincion() + "</FechaExtCausante>" + " <CausaExtCausante>"
				+ fromDB2.getCodigoExtincion() + "</CausaExtCausante> " + "<Tupla>" + " <RutCausante>" + fromWS.getRutCausante() +"-"+ fromWS.getDVCausante() + "</RutCausante>" + " <TipoCausante>" + fromWS.getCodTipoCausante() + "</TipoCausante>"
				+
				//" <TipoCausante>22</TipoCausante>" + 
				" <CodEntidadAdm>" + fromWS.getCodEntidadAdm() + "</CodEntidadAdm>" + " <FecRecCausante>" + fromWS.getFecRecCausante() + "</FecRecCausante> " + " <RutBeneficiario>" + fromWS.getRutBeneficiario() + "-" + fromWS.getDVBeneficiario() + "</RutBeneficiario> " + " <IdTipoBeneficio>" + fromWS.getCodTipoBeneficio()
				+ "</IdTipoBeneficio> " +
	...
	private String getPeriodoActual(){
		...
		int month= today.get(Calendar.MONTH); //meses de 0 a 11
		if(year== 2010){
			if(month<7){ //menor a agosto
				periodo="01";
			}else {
				periodo="02";
			}
		}else if(year== 2018){
			if(month<6){ //menor a julio
				periodo="01";
			}else if(month==6){ //igual a julio
				periodo="02";
			}else{
				periodo="03";
			}
		}else if(year== 2019){
			if(month<2){ //menor a marzo
				year= 2018;
				periodo="03";
			}else{
				periodo= month<6?"01":"02"; //menor a julio
			}
		}else if(year== 2020){
			if(month<2){ //menor a marzo
				year= 2019;
				...
				periodo="01";
			}else if(month<8){ //igual a julio o agosto
				periodo="02"; 
			}else{
				periodo="03"; 
			}
		}else if(year== 2021){
			if(month<4){ //menor a mayo
				year= 2020;
				periodo="03";
			}else{
				periodo= month<6?"01":"02"; //menor a julio
			...
			if(month<4){ //menor a mayo
				periodo="01";
			}else if(month<6){ //igual a mayo o junio
				periodo="02";
			}else if(month==6){ //igual a julio
					periodo="03";
			}else{
			...
			if(month<4){ //menor a mayo
				periodo="01";
			}else if(month<6){ // mayo o junio
				periodo="02";
			}else if(month<9){ //entre julio y septiembre
					periodo="03";
			}else{
				periodo="04";
			}
