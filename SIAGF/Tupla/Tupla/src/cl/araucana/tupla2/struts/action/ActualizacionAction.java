		...
		String codtcau_Invalidos= ",5,8,11,20,27,";
			try {
				...
				if(rechazos.size()>0){
					...
					//Se busca máxima fecha vigente
					int fechavig= dao.getFechaVigente();
						...
						try {
							...
							if(oCampo.isActualizaSIAGF() && acciones.indexOf("SF")>-1){
								if(!listasiagf.contains(oCampo.getRutCausante() + oCampo.getPeriodo())){
									...
									xml = construye.creaXmlActualizacion(oCampo, fechavig);
									...
									oRetorno.setId(oCampo.getId());
									oRetorno.setCodigo("0");
									oRetorno.setMensaje("Periodo Causante Actualizado en SIAGF");
									oRetorno.setIdtupla(oCampo.getTupla());
