...
<c:forEach var="item" items="${permisos}">
	...
	<c:if test="${item eq 'actualizaDatos'}">
		<input class="botonmenu" type="button" value="Actualiza tus Datos"
			onclick="window.location.href='<c:out value="${pageContext.request.contextPath}"/>/web/actualizaDatos.do?op=cargaInfo'">
	</c:if>
