...
import cl.laaraucana.botonpago.web.database.dao.MigradoDAO;
...
public class ManagerDeudor {
	...
	public SalidaDeudorVO esDeudorNoAfiliadoSapYAs400(String rut) throws Exception {
		...
		/*if(cliente.isDeudor()){
			ManagerBPAGC002 mgr = new ManagerBPAGC002();
			salida = mgr.consultaEsDeudor(rutDeudor);
			salida.setDeudor(true);
			//esNoAfiliado = mgr.esCodigoNoAfiliadoMigrado(salida.getCodSalida());
	...
	/*public SalidaDeudorVO esDeudorNoAfiliadoSapYAs400(String rut) throws Exception {
		String rutDeudor = rut;
		boolean esNoAfiliado = false;
		boolean esDeudor = false;
		SalidaDeudorVO salida = new SalidaDeudorVO();
		String[] rutArray = rut.replace(".", "").split("-");
		MigradoDAO migradoDao = new MigradoDAO();
		boolean esMigrado = migradoDao.isMigradoByRut(rutArray[0], rutArray[1]);
		logger.debug("El rut " + rutDeudor + " migrado = " + esMigrado);

		// migrados: 005 006 007 008 011
		if (esMigrado) {
			ManagerBPAGC002 mgr = new ManagerBPAGC002();
			salida = mgr.consultaEsDeudor(rutDeudor);
			esNoAfiliado = mgr.esCodigoNoAfiliadoMigrado(salida.getCodSalida());
			...
			if (esNoAfiliado) {
				ManagerSAP managerSap = new ManagerSAP();
				SalidaDeudor salidaDeudor = managerSap.tieneDeuda(rutDeudor);
				logger.debug("es deudor =" + salidaDeudor.getCodError());
				if (Constantes.getInstancia().APP_COD_SUCCESS.equals(salidaDeudor.getCodError())) {
					esDeudor = true;
				} else {
					esDeudor = false;
				}
			...
			// no migrados: 005
			// cuando es no migrado se valida el codigo 005 indica que
			// es deudor directo
			ManagerBPAGC002 mgr = new ManagerBPAGC002();
			salida = mgr.consultaEsDeudor(rutDeudor);
			esNoAfiliado = mgr.esCodigoNoAfiliadoNoMigrado(salida.getCodSalida());
		...
		logger.debug("noafi:" + esNoAfiliado + " && deudor:" + esDeudor);
		if (esNoAfiliado && esDeudor) {
			salida.setDudorNoAfiliado(true);
		} else {
			salida.setDudorNoAfiliado(false);
		}
		return salida;
	}*/
