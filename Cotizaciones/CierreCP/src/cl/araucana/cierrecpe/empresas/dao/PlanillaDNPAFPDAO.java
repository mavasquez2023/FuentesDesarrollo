 ...
 * @version 1.0
 */
public class PlanillaDNPAFPDAO implements DAO_Interface {
	 ...
	 * 
	 */
	public PlanillaDNPAFPDAO(ConectaDB2 db2, String esquema) {
		this.db2= db2;
	...
	public int insert(Object obj) throws SQLException {
		...
		query.append("INSERT INTO " + getEsquema() + ".pwf2001 values( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,? ) ");
		...
		db2.setStatement(48, planilla.getCotizacionSalud()); //Fonasa Independiente
	...
	private int insertDetalle(Object obj) throws SQLException {
		...
		StringBuffer query= new StringBuffer();
		query.append("INSERT INTO " + getEsquema() + ".pwf2101 values( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,? ) ");
		//logger.finest("Query=" + query.toString());
		db2.prepareQuery(query.toString());
		for (Iterator iterator = pagina.getCotizantes().iterator(); iterator.hasNext();) {
			...
			db2.setStatement(27, cotizante.getSeguroInvalidezFdoPensionCotizante());
			db2.setStatement(28, cotizante.getCotizacionFonasaIndependiente());
			db2.executeUpdate();
		}
