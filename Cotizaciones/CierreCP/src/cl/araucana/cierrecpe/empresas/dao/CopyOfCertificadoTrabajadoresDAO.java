 ...
 * @version 1.0
 */
public class CopyOfCertificadoTrabajadoresDAO implements DAO_Interface {
	 ...
	 * 
	 */
	public CopyOfCertificadoTrabajadoresDAO(ConectaDB2 db2) {
		this.db2= db2;
	}
...
//				Nacionalidad {0:Chileno, 1:Extranjero}
	...
	private int insertDetalle(PlanillaBasePaginaDetalle pagina, PlanillaBaseCotizante cotizante, String tipoEntidad, String descConcepto, int imponible, int monto) throws SQLException {
		...
		//for (Iterator iterator = pagina.getCotizantes().iterator(); iterator.hasNext();) {
			...
			if(getTipoNomina().equals("R")){
				tabla= "PWF6100";
				if((pagina instanceof PlanillaApvPaginaDetalle)) {
					if (tipoEntidad.equals(Constants.TIPO_ENTIDAD_DEP_CONV) || tipoEntidad.equals(Constants.TIPO_ENTIDAD_DEP_CONV_APO_IND)){
						tabla= "PWF6102";
					}else{
						tabla= "PWF6103";
					}
				...
				tabla= "PWF6101";
			}else if(getTipoNomina().equals("D")){
				tabla= "PWF6102";
			}else if(getTipoNomina().equals("A")){
				tabla= "PWF6105";
			}
			query.append("INSERT INTO ");
			query.append( tabla);
			query.append(" (PWDCRUTEM, PWDCRUTAF, PWDCTIPEN, PWDCENTID, PWDCMESRE, PWDCANORE, PWDCDIGEM, PWDCDIGAF, PWDCREMIM, PWDCMONCO, PWDCFECPA, PWDCFOLIO, PWDCSECFO, PWDCCONVE, PWDCCDHOL)");
			query.append(" values( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,? ) ");
			...
			db2.setStatement(3, tipoEntidad);
			if(tipoEntidad.equals(Constants.TIPO_ENTIDAD_DIAS)){
				...
				db2.setStatement(4, nombreINP + descConcepto);
				db2.setStatement(12, pagina.getCabeceraPlanilla().getFolio());
				db2.setStatement(13, pagina.getCabeceraPlanilla().getSecuenciaFolio());
			}
			db2.setStatement(5, periodo.substring(4));
			db2.setStatement(6, Integer.parseInt(periodo.substring(0, 4)));
			...
			db2.setStatement(9, imponible);
			db2.setStatement(10, monto);
			db2.setStatement(11, pagina.getCabeceraPlanilla().getFechaPago());
			db2.setStatement(14, pagina.getCabeceraPlanilla().getConvenio());
			db2.setStatement(15, pagina.getCabeceraPlanilla().getGrupoConvenio());
			try {
				db2.executeUpdate();
	...
	public int insertDetalleMovPer(PlanillaBasePaginaDetalle pagina, PlanillaBaseCotizante cotizante, int codMovimiento, int fechaInicio, int fechaTermino, String entidadRecaudadora, int correlativo) throws SQLException {
		...
		db2.setStatement(1, rutempresa);
		db2.setStatement(2, rutcotizante);
		db2.setStatement(3, periodo.substring(4));
		db2.setStatement(4, Integer.parseInt(periodo.substring(0, 4)));
		db2.setStatement(5, correlativo); //Correlativo
		db2.setStatement(6, codMovimiento);
		db2.setStatement(7, fechaInicio);
		db2.setStatement(8, fechaTermino);
		...
		db2.setStatement(11, pagina.getCabeceraPlanilla().getFolio());
		db2.setStatement(12, pagina.getCabeceraPlanilla().getSecuenciaFolio());
		db2.setStatement(13, entidadRecaudadora);
		//db2.setStatement(14, ""); //AFP // se comentó campo ya que por omisión queda null (como debe ser).
		db2.setStatement(14, convenio);
		db2.setStatement(15, grupoconvenio);
