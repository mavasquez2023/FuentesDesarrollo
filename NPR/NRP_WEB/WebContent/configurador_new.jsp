			...
			<div id="main_content" > 
				<div id="configurador">
					...
					<div id="configurable">
						...
						<a href="javascript:cargarDataColumnas()" style="display:none">recargar</a>
						<div id="div_seleccion_seccion">
							<h4>Secciones</h4>
							<div style="margin-left:10px;">
								<font onclick="javascript:verMas('header')" id="link_header_ver_mas" class="clickeable" style="display:none"> 
									<span class="label label-default"><span class="glyphicon glyphicon-menu-down"></span> Header </span>
								</font>
								<font onclick="javascript:verMenos('header')" id="link_header_ver_menos" class="clickeable" style="display:none"> 
									<span class="label label-success"><span class="glyphicon glyphicon-menu-up"></span> Header </span>
								</font>
								&nbsp;

								<font onclick="javascript:verMas('detalle')" id="link_detalle_ver_mas" class="clickeable" style="display:none"> 
									<span class="label label-default"><span class="glyphicon glyphicon-menu-down"></span> Detalle </span>
								</font>
								<font onclick="javascript:verMenos('detalle')" id="link_detalle_ver_menos" class="clickeable" style="display:none"> 
									<span class="label label-success"><span class="glyphicon glyphicon-menu-up"></span> Detalle </span>
								</font>
								&nbsp;

								<font onclick="javascript:verMas('footer')" id="link_footer_ver_mas" class="clickeable" style="display:none"> 
									<span class="label label-default"><span class="glyphicon glyphicon-menu-down"></span> Footer </span>
								</font>
								<font onclick="javascript:verMenos('footer')" id="link_footer_ver_menos" class="clickeable" style="display:none"> 
						...
						<div id="div_conf_header" class="container_conf" style="display:none">
							...
							<div style="margin-left:10px">
								<font class="conf_header_habilitado opcion_habilitar_deshabilitar clickeable" > 
									<font id="estado_conf_header_habilitado"  class="conf_header_habilitado"> 
										<span class="label label-success">Habilitado</span>
									</font>
									<span onclick="javascript:deshabilitar('header')"  class="label label-warning"><span class="glyphicon glyphicon-remove"></span> Deshabilitar</span>
								</font>

								<font class="conf_header_deshabilitado opcion_habilitar_deshabilitar clickeable" > 
									...
									<span  onclick="javascript:habilitar('header')" class="label label-success"><span class="glyphicon glyphicon-ok"></span> Habilitar</span>
								</font>
						...
						<div id="div_conf_detalle" class="container_conf" style="display:none">
							...
							<div style="margin-left:10px">
								<font  class="conf_detalle_habilitado opcion_habilitar_deshabilitar clickeable" > 
									<font id="estado_conf_detalle_habilitado"  class="conf_detalle_habilitado"> 
										<span class="label label-success">Habilitado</span>
									</font>
									<span onclick="javascript:deshabilitar('detalle')" class="label label-warning"><span class="glyphicon glyphicon-remove"></span> Deshabilitar</span>
								</font>

								<font class="conf_detalle_deshabilitado opcion_habilitar_deshabilitar clickeable" > 
									<font id="estado_conf_detalle_deshabilitado"  class="conf_detalle_deshabilitado"> 
										<span class="label label-warning">Deshabilitado</span>
									</font>
									<span onclick="javascript:habilitar('detalle')"  class="label label-success"><span class="glyphicon glyphicon-ok"></span> Habilitar</span>
								</font>
										...
										<tr>
											...
											<td valign="top">
												<div id="detalle_cols_disp" style='float:right'  class="content_columnas_configurador">
													<div class='cols_disp_titulo' > <span class='glyphicon glyphicon-list-alt'></span>&nbsp;&nbsp; Columnas Disponibles</div>
													<div class='cols_disp_titulo' style="height:45px;" id="tags_detalle"> <span class='glyphicon glyphicon-filter' style="margin-bottom:10px"></span>&nbsp;&nbsp; Agrupaciones <br/>
														<!-- 
														<span class="label label-success" onclick="show('fechas')">Fechas</span> &nbsp; <span class="label label-success"  onclick="show('nombres')">Nombres</span>
														 -->
													</div>
						...
						<div id="div_conf_footer" class="container_conf" style="display:none">
							...
							<div style="margin-left:10px">
								<font class="conf_footer_habilitado opcion_habilitar_deshabilitar clickeable" > 
									<font id="estado_conf_footer_habilitado"  class="conf_footer_habilitado"> 
										<span class="label label-success">Habilitado</span>
									</font>
									<span onclick="javascript:deshabilitar('footer')"  class="label label-warning"><span class="glyphicon glyphicon-remove"></span> Deshabilitar</span>
								</font>

								<font class="conf_footer_deshabilitado opcion_habilitar_deshabilitar clickeable" > 
									<font id="estado_conf_footer_deshabilitado"  class="conf_footer_deshabilitado"> 
										<span class="label label-warning">Deshabilitado</span>
									</font>
									<span   onclick="javascript:habilitar('footer')" class="label label-success"><span class="glyphicon glyphicon-ok"></span> Habilitar</span>
								</font>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
			...
			<div class="form-group">
				...
				<input type="text" class="form-control" id="col_const_texto" placeholder="Ingrese texto constante" value="" onkeyPress="mostrarValorConstante(event)" onchange="mostrarValorConstante(event)">
				
				<font id="val_col_const_texto"></font>
			</div>
...
function init(){
	...
	for(var i=0; i < data.item.columnas.length ; i++){
		...
		//Tags <span class="label label-success" onclick="show('fechas')">Fechas</span>
		
		if(data.item.item[configuracion.id] != null && data.item.item[configuracion.id].length > 0){
			try{
				var tags = data.item.item[configuracion.id].split(";");
				for(var j=0; j<tags.length; j++){
					$("#tags_"+configuracion.id).append("<span class=\"label label-success\" style=\"cursor:hand;cursor:pointer\" title=\"Ver columnas agrupadas para "+tags[j].split(":")[0]+"\" onclick=\"showTags('"+configuracion.id+"','"+tags[j].split(":")[0]+"')\">"+tags[j].split(":")[0]+"</span>&nbsp;")
				}
			}
			catch(e){
				alert(e)
		...
		procesarColumnas(configuracion.id,true,false, true);
...
function obtenerColumnasConstantes(div){
	
	var constantesAgregadas = new Array();
	for(var i=0; i< seleccionados[div].length; i++){
		if( seleccionados[div][i].nombre.indexOf("data_constante_")==0){
			constantesAgregadas.push(seleccionados[div][i]);
		}
		else if( seleccionados[div][i].nombre.indexOf("sys.")==0){
			constantesAgregadas.push(seleccionados[div][i]);
		}
	}

	return constantesAgregadas;

}



function existeColumnaEnSeleccionado(div, nombre){

	console.log("buscando si columna ["+nombre+"] existe en div ["+div+"] ");
	console.log("cantidad seleccionados: "+ seleccionados[div].length);
	var retorno = false;
	for(var i =0; i< seleccionados[div].length ; i++ ){
	
		if( seleccionados[div][i].nombre == nombre ){
			console.log("columna ["+nombre+"] existe en div ["+div+"] ");
			return true;
		}
	
	}

	console.log("columna ["+nombre+"] no existe en div ["+div+"] ");
	return false;
	
}

function existeColumnaEnConfiguracionInicial(div, nombre){
...
function procesarColumnas(div, agregar, agregarConstantes, inicial){
	console.log(" columnas seleccionadas -> "+ seleccionados[div].length)
	
	var constantesAgregadas = obtenerColumnasConstantes(div);
	
	console.log(" constantes seleccionadas -> "+ constantesAgregadas.length)
	console.log("agregar ? "+ agregar );
	console.log("agregarConstantes ? "+ agregarConstantes );
	...
	if(agregar){
		...
		if(agregarConstantes ){
			for(var i=0; i< constantesAgregadas.length; i++){
				console.log("validando columna -> "+ constantesAgregadas[i].nombre)
				if(!existeColumnaEnConfiguracionInicial(div,constantesAgregadas[i].nombre) ){
					console.log("la agrego a seleccionados ["+constantesAgregadas[i].nombre+"]");
					seleccionados[div].push(constantesAgregadas[i]);
				}				
			}	
		}
	...
	console.log("seleccionados antes -> "+ seleccionados[div].length);
...
var btn = "<div style='width:100%;text-align:right;padding-right:10px;padding-top:5px' class='btn_add_constante'><button type='button' class='btn btn-success btn-sm ' title='Agregar Columna' style='padding-right:10px;'  onclick='cargarFomularioConstante(\""+div+"\");'><span class='glyphicon glyphicon-plus'></span>&nbsp;Agregar Constante</button></div><div style='width:100%;text-align:right;padding-right:10px;padding-top:5px' class='btn_add_constante'><button type='button' class='btn btn-info btn-sm ' title='Agregar Columna' style='padding-right:10px;'  onclick='cargarFomularioVariableSistema(\""+div+"\");'><span class='glyphicon glyphicon-plus'></span>&nbsp;Agregar Variable Sistema</button></div>";
	...
	console.log("seleccionados despues -> "+ seleccionados[div].length);
	for(var i=0; i< seleccionados[div].length; i++){
		console.log("col dsp ["+i+"]-> " + seleccionados[div][i].nombre, seleccionados[div][i])
...
function showTags(div,tag){
	$(".col_disp").hide();
	//$(".col_"+div+"_todos").hide();
	$(".col_"+div+"_"+tag).show();
	$(".no_incluir").hide();
}

function obtenerTags(div, nombre){
	var estilos = " col_"+div+"_todos ";
	var dataTag = misTags[div];
	var tags = dataTag.split(";");
	for(var i=0; i< tags.length; i++){
		var tag = tags[i].split(":")[0];
		var valores = ","+tags[i].split(":")[1]+",";
		
		if(valores.indexOf(nombre)>-1){
			estilos = estilos + " col_"+div+"_"+tag +" ";
		}
	}
	return estilos;
}
...
function putDivColumna(div, data, id_container){
	...
	var keyAux = "col_"+div+"_"+data.nombre;
	
	if(data.nombre.indexOf("data_constante") >=0 && $("#"+keyAux).html()!= null && $("#"+keyAux).html().length > 0 ){
		return;
	}
	
	var _class_no_incluir = "";
	var _desc_no_incluir = "";
	var _style_no_incluir = "";
	var _no_incluir = false;
	if(data.no_incluir == "true"){
		_no_incluir = true;
		_class_no_incluir = "no_incluir";
		_desc_no_incluir="<font color='YELLOW' title='Este dato no se incluye en el la información de salida del reporte.' style='cursor:hand;cursor:pointer'>&nbsp;(*) EXCLUIDA &nbsp;</font>";
	}
	var tags = obtenerTags(div, data.nombre);
	var _div = "<div id='col_"+div+"_"+data.nombre+"' class='col_disp div_columna "+_class_no_incluir+" "+tags+"' onmouseover='seleccionarColumna(\""+div+"\",\""+data.nombre+"\")' onmouseout='deseleccionarColumna(\""+div+"\",\""+data.nombre+"\")'> "+
							...
							"<div class='titulo_columna '>"+
									...
									"</button>&nbsp; "+
								"<font ></font>&nbsp<font>"+  data.header  +"</font>"+_desc_no_incluir+
							...
							"<div class='data_columna data_"+unique_key+"' style='display:none' >"+
							...
							"<div class='data_columna data_no_incluir_"+unique_key+"' style='display:none' >"+
							"<font class='titulo_data_columna'>Descripci&oacute;n: </font>&nbsp<font>"+data.data_ej+"</font>"+
	...
	if(!_no_incluir){
		$(".data_"+unique_key).show();
	}
	else{
		$(".data_no_incluir_"+unique_key).show();
		$("."+_class_no_incluir).hide();
	}
...
function removeColumna(div, nombre){
			if(nombre.indexOf("data_constante") == 0){
				console.log("eliminando: es constante ");
				var seleccionadosTMP = new Array();
				for(var j=0; j<seleccionados[div].length; j++){
					console.log("nombre->"+seleccionados[div][j].nombre);
					if(seleccionados[div][j].nombre == nombre){
						//
						
						console.log("se eliminara "+ nombre);
					}
					else{
						seleccionadosTMP.push(seleccionados[div][j]);
						console.log("se mantiene "+ nombre);
					}
				}
				seleccionados[div] = seleccionadosTMP;
	...
	procesarColumnas(div,true,true);
}
...
function addColumna(div, nombre){
	...
	$("#data_ejemplo_"+div).html("");
	var columnas = obtenerColumnasPorDiv(div);
	var cantidadConstantes = obtenerColumnasConstantes(div).length;
	for(var i=0; i< columnas.length; i++){
		if(columnas[i].nombre == nombre ){
			columnas[i].configurada = true;
			columnas[i].posicion = seleccionados[div].length  + cantidadConstantes;
			i=columnas.length;
		}
	}

	var agregarConstante = true;

	


	console.log("la agrego ? ["+nombre+"]" + existeColumnaEnSeleccionado(div, nombre));
	procesarColumnas(div,true, agregarConstante);
...
function upColumna(posicion, div, nombre){
	...
	if(posicion>1){
		...
		procesarColumnas(div,false,false);
	}
}
...
function cambiarLargo(div,nombre){
	...
	procesarColumnas(div,true,false);
}
...
function getJSONSeleccionados(div){
	...
	for(var i = 0; i < dataColumnasConfiguradas.length; i++){	
		var item = '{'+
				...
				'"no_incluir":"'+ dataColumnasConfiguradas[i].no_incluir +'",' +
				'"metodo":"'+ dataColumnasConfiguradas[i].metodo +'",' +
				'"data":"'+ dataColumnasConfiguradas[i].data +'"' +
...
function agregarColumnaConstante(){
	if($("#col_const_texto").val().length == 0 || $("#col_const_header").val().trim().length == 0){
		alert("ingrese los campos requeridos");
		return;
	}

	$("#col_const_texto").val( limpiarValor(  $("#col_const_texto").val() )  )
	...
	procesarColumnas(div_llamador_constante,false,false);
...
function limpiarValor(valor){
	...
	if(valor.length > 0){
		...
		for(var i=0; i< valor.length; i++){
			if( "áéíóúÁÉÍÓÚñÑ".indexOf(valor.charAt(i)) < 0 ){
				texto = texto + valor.charAt(i);
			}
		}
...
function mostrarValorConstante(e){ 
	var valor = $("#col_const_texto").val();
	patron =/[áéíóúÁÉÍÓÚñÑ]/; 
	if (patron.test(valor)) {
	   alert('Error caracteres no permitidos'); 
	   $("#col_const_texto").val( limpiarValor($("#col_const_texto").val())  ) 
	}	
	$("#val_col_const_texto").html("valor: ["+$("#col_const_texto").val()+"] - "+ $("#col_const_texto").val().length +" caracteres")
}
...
function cargarDataColumnas(){
	console.log("cantidad-> " + seleccionados["detalle"].length)

	for(var i=0; i< seleccionados["detalle"].length ; i++){
		console.log("columna -> "+ seleccionados["detalle"][i].nombre);
