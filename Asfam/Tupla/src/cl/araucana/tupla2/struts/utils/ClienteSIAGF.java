...
import cl.araucana.util.XMLSiagf;
...
public class ClienteSIAGF {
	...
	public ArrayList consultaCausante(String rut, boolean recXml) throws Tupla2Exception, FatalException {
		...
		for (int i = 0; i < MAX_INTENTOS && !exito; i++) {
			...
			try {
				...
				if(recXml){
					XMLSiagf.saveXML(rut, respuestaConsultaXml);
				}
