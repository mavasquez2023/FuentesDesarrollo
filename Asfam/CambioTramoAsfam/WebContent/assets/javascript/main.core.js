...
function enviarPropuesta(rutEmpresa){
	ancho= "1000";
	alto= "600";
	document.forms[0].rutt.value=rutEmpresa;
	document.forms[0].ruts.value=rutEmpresa;
	var opciones="toolbar=no,location=no, directories=no, status=yes, menubar=no, scrollbars=yes, resizable=yes, "
		...
		window.open("", "ENVIAR", opciones);
}

function enviarPropuestaEjecutivo(){
	ancho= "1000";
	alto= "600";
	var opciones="toolbar=no,location=no, directories=no, status=yes, menubar=no, scrollbars=yes, resizable=yes, "
		+ "width=" + ancho + ", height=" + alto + ", "
...
if(c==0){
	alert('Por favor, debe seleccionar la empresa.');
	return false;
}
...
function descargaPropuesta(){
	rutempone=document.forms[0].rutempdown.value;
	if(rutempone==""){
		alert("Ingrese un Rut Empresa");
		return false;
	}
	document.forms[0].rutt.value=rutempone;
}
...
function CopiarTabla(){
	...
	var f = document.forms[0];
	f['action'] = f['action'] + "?action=cpyFile";
	//alert("action=" + f['action']);
	if(f.copiaAFP64.value!=""){
		f.submit();
		return true;
	}
	return false;
...
var numero=rut[0];
var ext=rut[rut.length-1];
var uext=ext.toUpperCase();
var txt='txt'.toUpperCase();
var csv='csv'.toUpperCase();
var zip='zip'.toUpperCase();	

if(uext!=txt&&uext!=csv)
{
	alert('Extensión de archivo no válida. Sólo se permite extensiones txt o csv.');
	return false;
}
if(rol=="Ejecutivo"){
	if(uext!=zip){
		...
		if(rut.length>2){
			alert('Nombre de archivo no válido. Debe corresponder al <Rut de la Empresa sin dígito verificador>.csv');
			return false;
		}
