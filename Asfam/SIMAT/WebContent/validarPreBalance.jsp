	...
	<div id="workSpace" name="workSpace">
		...
		<fieldset class="form-fieldset">
			<div class="field full-width">
				...
				<input class="boton" type="button" value='Consultar Estado Procesos' onclick="recargarPagina();"/>									
			</div>
			<br>
		</fieldset><br>
				<fieldset class="form-fieldset">
					<div class="contenido">
					<c:if test="${not empty cuentasPreBalance}">
						<table align="center">
							<thead>
								<tr>
									<th>Cuenta</th>
									<th>Resultado Balance General</th>
									<th>Información de planos</th>
									<th>Diferencia</th>
								</tr>
							</thead>
							<tbody>
								<c:forEach items="${cuentasPreBalance}" var="cuenta" >
									<tr class="margin <c:if test="${cuenta.getDiferencia()!=0}">alert-color</c:if>">
										<td>${cuenta.descripcion}</td>
										<td><fmt:formatNumber currencySymbol="$" value="${cuenta.resultadoBalance}" type="currency" /></td>
										<td><fmt:formatNumber currencySymbol="$" value="${cuenta.resultadoPlanos}" type="currency" /></td>
										<td><fmt:formatNumber currencySymbol="$" value="${cuenta.getDiferencia()}" type="currency" /></td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
						...
						<div class="right">
								...
								<input type="button" class="boton" value="Siguiente"onclick="obtenerPaseContable();">
						</div>
							</c:if>
					</c:if>
					</div>
...
var opts = {
	...
	function recargarPagina(){
		if(procesoActivo==0){
		 window.location = "<%=request.getContextPath()%>/validarPreBalance.do";
		}else{
	...
	function obtenerPaseContable(){
		window.location ='<%=request.getContextPath()%>/obtenerPaseContable.do';
<%-- 		procesoActivo=1;
		//$("#loadMenu").show();
		spinner.spin(target);
		$.ajax({
 			url : '<%=request.getContextPath()%>/obtenerPaseContable.do',
 			type : 'POST',
