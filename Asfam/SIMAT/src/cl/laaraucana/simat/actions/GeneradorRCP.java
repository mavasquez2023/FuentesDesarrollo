...
import cl.laaraucana.simat.documentos.RCP.EscritorRCP;
...
public class GeneradorRCP extends AbstractAction {

	public ActionForward generarRCP(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (estadoValidacion.equals("validado")) {
			...
			try {
				...
				if (reVO.isEstado()) {
					...
					msgEP = "Se generó correctamente el" + " RESUMEN DE COTIZACIONES PREVISIONALES (" + reVO.getNombreArchivo() + ")en la siguiente direccion: <br> <br>" + rutaUbicacion + "";
					forward = mapping.findForward("escrituraOK");
				} else {
					msgEP = "Problemas en la escritura del cuadro 'RESUMEN DE COTIZACIONES PREVISIONALES',\n puede que el archivo este siendo usado.";
					forward = mapping.findForward("escrituraError");
				}
			} catch (Exception e) {
				// Report the error using the appropriate name and ID.
				errors.add("name", new ActionError("id"));
				System.out.println("error en la escritura de planos,**");
				msgEP = "Problemas en la escritura del 'RESUMEN DE COTIZACIONES PREVISIONALES' <br>" + e;
				forward = mapping.findForward("escrituraError");
			}
		} else {
		...
		//finish whit 
		return (forward);
	}//end  Resumen cotizaciones Previsionales
