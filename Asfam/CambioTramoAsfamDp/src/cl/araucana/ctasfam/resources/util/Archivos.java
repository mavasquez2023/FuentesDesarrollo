...
public Archivos(String server, String usuario, String password){
	if (server!=null && !server.equals("")){
		this.system = new AS400(server, usuario, password);
	}
}

public Archivos(AS400 system){
   	this.system = system;
}
...
public static void main(String[] args) {
	AS400 system = new AS400("146.83.1.32", "qondadm", "qondadm");
	Archivo files= new Archivo(system);
	//Archivo filein= new Archivo();
	System.out.println("entrando");
	//Archivo files= new Archivo();
	try {
		Vector vec= new Vector();
		files.leerArchivo("/reports/Temporal/prueba.txt", vec);
		files.crearArchivo("/reports/Temporal/Copy of prueba.txt", vec);
	} catch (Exception e) {
