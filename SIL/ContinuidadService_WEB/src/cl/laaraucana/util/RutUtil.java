...
public class RutUtil {
	...
	public static boolean IsRutValido(String rutEntrada) {
		...
		if (PatronRut(rutEntrada) && rutEntrada.length() <= 10) {
			...
			int rut = Integer.parseInt(rutEntrada.substring(0,rutEntrada.length()-1));
			char dv = rutEntrada.charAt(rutEntrada.length()-1);
...
/*		Matcher encaja = patron.matcher(rutEntrada);
		if (encaja.find())
			...
			return false;*/
		return rutEntrada.matches("[0-9]{1,9}[K|0-9]{1}");
	}
