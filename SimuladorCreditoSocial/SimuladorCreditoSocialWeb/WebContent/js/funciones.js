...
var validarMonto = function validarMonto() {
	...
	_monto.addEventListener('blur', function(event) {
		...
		_validar = setTimeout(function() {
			if (fln.obtener_numeros(_el.value) >= 20000
					&& fln.obtener_numeros(_el.value) <= 25000000) {
...
$('#tipo_afiliado').change(
		function(event) {
			if (event.target.value === 'ZFSO') {
				$('#seguro-cesantia').attr('disabled', true).attr('checked',
						false).parent('.form__checkbox--checked').removeClass(
						'form__checkbox--checked').addClass(
						'form__checkbox--disabled');
			} else {
				$('#seguro-cesantia').attr('disabled', false).parent(
						'.form__checkbox').removeClass(
						'form__checkbox--disabled');
			}
		});
...
function limpiarString(cadena) {
	...
	cadena = cadena.toLowerCase(); // Quitamos espacios y los sustituimos por _
	// porque nos gusta mas asi

	cadena = cadena.replace(/ /g, "_"); // Quitamos acentos y "ñ". Fijate en que
	// va sin comillas el primer parametro
				...
				var _f = function _f(el) {
					if (el.region_number === region) {
						...
						var _f2 = function _f2(el) {
							comunas.append("<option value=\"".concat(
									limpiarString(el), "\">").concat(el,
									"</option>"));
						};
	...
	$('.tabla--paginada')
			.each(
					function(i) {
						...
						// return $(this).text() === "-";
						// }).parent().remove();
						var id = $(this).attr('id'), buscador = $(this).find(
								'.filtrar'), tabla = $(this).find('table'), tablaTr = $(
								this).find('tbody tr'), tablaTrTotal = tabla
								.data('filas'), alerta = $(this)
								.find('.alerta'), paginador = $(this).find(
								'.paginador');
						paginador.attr('id', 'paginador' + i);
						buscador.attr('id', 'buscador' + i);
						i++; // var pagina = 0;
						// $('.carousel-tablas tbody
						// tr.actual').first().each(function () {
...
var validarDatos = function validarDatos() {
	...
	if (fln.obtener_numeros(_monto.value) >= 20000
			&& fln.obtener_numeros(_monto.value) <= 25000000) {
		_montoValido = true;
	} else
		_montoValido = false;
	...
	validarFormulario('simulador-datos', {
		...
		otrosCampos : {
			'Monto superior a $20.000 e inferior a $25.000.000' : [
					_montoValido, _monto ]
		},
		exito : function exito() {
			document.forms["simulador-datos"].submit();
			/**
			 * INSTRUCCIONES exito() es para la validación de los campos, aquí
			 * se debe ejecutar lo que pasará si la validación es éxitosa.
			 * fln.preloader(1) activa el preloader fln.preloader(0) desactiva
			 * el preloader
			 */
			// fln.preloader(1); // ejecutar lo sgte si la respuesta para
			// avanzar al paso 2 es éxitosa
			// simulador.sgte(2);
...
var validarDatosCredito = function validarDatosCredito() {

	validarFormulario('simulador-resultado', {
		texto : "Debes ingresar ",
		otrosCampos : {
		...
		exito : function exito() {
			...
			document.forms["simulador-resultado"].submit();
...
var validarSolicitud = function validarSolicitud() {
	...
	validarFormulario('simulador-solicitar', {
		...
		otrosCampos : {
			...
			'Email' : [ _email, $('#email') ],
			'Captcha': [captchaValido, $('#simulador-credito').find('.captcha')]
		},
		exito : function exito() {
			/**
			 * INSTRUCCIONES exito() es para la validación de los campos, aquí
			 ...
			 * el preloader
			 */
			document.forms["simulador-credito"].submit();
...
var validarTelefono = function validarTelefono() {
	...
	if (_inputs.length) {
		...
		var _f4 = function _f4(el, i) {
			...
			var _dataPrefijoTipo = el.getAttribute('data-prefijo-tipo'); // let
			// _blur
			// =
			// new
			// Event('blur');
