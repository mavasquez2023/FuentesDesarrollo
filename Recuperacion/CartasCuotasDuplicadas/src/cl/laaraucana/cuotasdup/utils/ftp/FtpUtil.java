...
public class FtpUtil {
	...
	public boolean bajarArchivoViaFtp(String nombreArchivo, String pathSalida) {
		boolean res = false;
		try {
			ftpClient = conectar();
			// enter passive mode
			...
			if(ftpClient!=null){
				...
				// bajar archivo
				File downloadFile = new File(pathSalida + nombreArchivo);
				OutputStream outputStream = new BufferedOutputStream(new FileOutputStream(downloadFile));

				ftpClient.setFileType(FTP.BINARY_FILE_TYPE);
				ftpClient.setBufferSize(config.getBufferSize());//1024 KB
				res= ftpClient.retrieveFile(nombreArchivo, outputStream);
...
/*			ByteArrayInputStream bais = new ByteArrayInputStream(archivo);
	...
	public List<ArchivoVO> listarArchivos(String filtroExt, String directorioPrinc){
		List<ArchivoVO> lista = new ArrayList<ArchivoVO>();
		try {
			ftpClient = conectar();
	...
	public static void main(String[] args) throws IOException {
		...
		List<ArchivoVO> archivos = test.listarArchivos("PDF","");
		for (ArchivoVO archivo : archivos) {
