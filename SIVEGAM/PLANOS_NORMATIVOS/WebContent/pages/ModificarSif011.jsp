...
<script type="text/javascript">
	...
	function validadorFuenteOrigenAgregar(){
		...
		if ((fuenteorigen == 1) && ((rutempresa == "") || (dvempresa == "") || (nombreempresa == "") || (rutafiliado == "") || (dvafiliado == "") || (nombreafiliado == "") 
			...
			|| (montobeneficio == 0) || (montodocumento != 0) || (nmeroserie != "") || (nmerodocumento != "") || (fechemisiondoc != 0) || (codigobanco != 0) 
			|| (diasasfam == 0))){
	...
	function validacionUno(){
		...
		if (id_tipo_declaracion == 0){
			alert("Seleccione Tipo de Declaración previo a seleccionar las fechas.");
			...
			if((mesInformado == mesIni) && (mesInformado == mesTerm) && (id_tipo_declaracion == 2)){
				...
				if((mesProceso == mesIni) && (mesProceso == mesTerm) && (id_tipo_declaracion == 1)){
					...
					if((diaIni < diaTerm) && (mesIni == mesTerm) && (anioIni == anioTerm)){
						...
						alert("La fecha ingresada no corresponde al periodo procesado, o al Tipo de Declaración seleccionada.");
						document.getElementById("txt_FechaInicioBeneficio").value = " ";
						document.getElementById("txt_FechaTerminoBeneficio").value = " ";
						return false;
					}
	...
	function validacionDos(){
		...
		if( tipo_Declaracion == 2 ){
			if (document.AgregarSif011Form.dbx_ModPago.value == 7){
				...
				document.getElementById("dbx_CodBanco").disabled=false;
				document.AgregarSif011Form.dbx_ModPago.value = 0;	
				alert("La modalidad de Pago no correspondia con el Tipo de Declaración seleccionado, cambie la modalidad de Pago una vez mas.");
			}
		}
	...
	function verificaValidacionDos(tipo_declaracion, modo_pago){
		...
		if(tipo_declaracion == 1 && modo_pago == 7){
			...
			alert("La modalidad de Pago no corresponde con el Tipo de Declaración seleccionado.");
			return false;
		}		
	}
	...
	function validacionSeis(tipo_emision, mes_recaudacion, mes_remuneracion, numero_declaracion){
		...
		if(tipo_emision == 101){
			if((Trim(mes_recaudacion) != "") && (Trim(mes_remuneracion) != "") && (Trim(numero_declaracion) != "")){
				...
				alert("Los campos Mes Recaudación, Mes Remuneración y Número Declaración son obligatorios para el Tipo Emisión seleccionado");
				return false;
			}		
		}
	}
	...
	function validacionOcho(tipo_declaracion, monto_documento, numero_serie, numero_documento){
		...
		if(tipo_declaracion == 2){
			if((Trim(monto_documento) != "") && (Trim(numero_serie) != "") && (Trim(numero_documento) != "")){
				...
				alert("Los campos Monto Documento, Número Serie y Número Documento son obligatorios para el Tipo Declaración seleccionado.");
				return false;
			}	
		}else{
			return true;
	...
	function validacionNueve(fuenteOrigen, montoDocumento, numeroSerie, numeroDocumento, fechaEmision, codigoBanco){
		...
		if(fuenteOrigen == 2){
			if((Trim(montoDocumento) != "") && (Trim(numeroSerie) != "") && (Trim(numeroDocumento) != "")&& (Trim(fechaEmision) != "")&& (Trim(codigoBanco) != 0)){
				...
				alert("Los campos Monto Documento, Número Serie, Número Documento, Fecha Emisión y Código de Banco son obligatorios para la Fuente de Origen seleccionada.");
				return false;
			}
		if (fuenteOrigen == 0){
	...
	function agregarRegistroSif011(){
		...
		if(validaIngreso == true){
			...
			document.AgregarSif011Form.btn_Modificar.disabled=true;
			...
			AgregarRegistroDivisionPrevisionalDWR.insertSif011( mesInformado,
				...
				if(resp == 0){
					...
					document.AgregarSif011Form.btn_Modificar.disabled = false;
			...
			alert("Error, los datos no cumplen la validación.");
		}
	}
	...
	function cerrarVentana(){
		...
		document.AgregarSif011Form.idSelectedItem.value = '<%=session.getAttribute("idSelectedItem")%>';
		var idSelectedItem = document.AgregarSif011Form.idSelectedItem.value;
		if((idSelectedItem != "") && (idSelectedItem != null)){
			enviaFormulario(2);
		}else{	
			window.close();
		}
	...
	function modificarRegistroSif011(){
		
		var id = document.AgregarSif011Form.idSif011_glob.value;
		
		var mesInformado = document.AgregarSif011Form.fecpertras.value;
		
		var fteOrigen = document.AgregarSif011Form.dbx_FuenteOrigen.value;
		...
		if(validaIngreso == true){
			...
			document.AgregarSif011Form.btn_Modificar.disabled=true;
			document.AgregarSif011Form.btn_Cancelar.disabled=true;
			EditarReporteDivisionPrevisionalDWR.updateSif011(id,
																fteOrigen,
																mesInformado,
																rutEmpresa,
																dvEmpresa,
	...
	function cargarModificar(){
		document.AgregarSif011Form.idSelectedItem.value = '<%=session.getAttribute("idSelectedItem")%>';
		document.AgregarSif011Form.idSif011_glob.value = '<%=session.getAttribute("idSif011_glob")%>';
		document.AgregarSif011Form.rutSearch.value = '<%=session.getAttribute("rutSearch")%>';
		var idSelectedItem = '3' //document.AgregarSif011Form.idSelectedItem.value;
		if((idSelectedItem != "") && (idSelectedItem != 'null')){
			//document.getElementById("btn_Modificar").style.visibility = "visible";
			
			//document.AgregarSif011Form.btn_Modificar.disabled = false;
			
			document.AgregarSif011Form.dbx_filtroBusqueda.value = '<%=session.getAttribute("dbx_filtroBusqueda")%>';
			document.AgregarSif011Form.txt_rut.value = '<%=session.getAttribute("txt_rut")%>';
			document.AgregarSif011Form.txt_digitoVerificador.value = '<%=session.getAttribute("txt_digitoVerificador")%>';
			document.AgregarSif011Form.txt_primerRango.value = '<%=session.getAttribute("txt_primerRango")%>';
			
			var idSif011_glob = document.AgregarSif011Form.idSif011_glob.value;
			var rutSearch = document.AgregarSif011Form.rutSearch.value;
			DWREngine.setAsync(false);
			EditarReporteDivisionPrevisionalDWR.obtenerDatosPorRutId(idSelectedItem, idSif011_glob, rutSearch, function(data){
						var datosEdit = data.listSif011[0];
						if(datosEdit != null){
							document.AgregarSif011Form.txt_Rut.value = datosEdit.rut_empresa;
							document.AgregarSif011Form.txt_NumVerif.value = Trim(datosEdit.dv_empresa);
							document.AgregarSif011Form.txt_NombreEmpresa.value = Trim(datosEdit.nombre_empresa);
							document.AgregarSif011Form.txt_NumeroDeclaracion.value = Trim(datosEdit.codigo_barra);
							document.AgregarSif011Form.txt_RutAfiliado.value = datosEdit.rut_afiliado;
							document.AgregarSif011Form.txt_NumVerifAfiliado.value = Trim(datosEdit.dv_afiliado);
							document.AgregarSif011Form.txt_NombreAfiliado.value = Trim(datosEdit.nombre_afiliado);
							document.AgregarSif011Form.dbx_TipoBeneficio.value = datosEdit.cod_tipo_beneficio;
							document.AgregarSif011Form.dbx_TipoBeneficiario.value = datosEdit.tipo_beneficiario;
							document.AgregarSif011Form.txt_RutCausante.value = datosEdit.rut_causante;
							document.AgregarSif011Form.txt_NumVerifCausante.value = Trim(datosEdit.dv_causante);
							document.AgregarSif011Form.txt_NombreCausante.value = Trim(datosEdit.nombre_causante);
							document.AgregarSif011Form.dbx_TipoCausante.value = datosEdit.cod_tipo_causante;
							document.AgregarSif011Form.txt_FechaInicioBeneficio.value = Trim(datosEdit.fechaInicioBeneficioDate);
							document.AgregarSif011Form.txt_FechaTerminoBeneficio.value = Trim(datosEdit.fechaTerminoBeneficioDate);
							document.AgregarSif011Form.txt_DiasAsfam.value = datosEdit.dias_asfam;
							document.AgregarSif011Form.dbx_CodigoTramo.value = datosEdit.codigo_tramo;
							document.AgregarSif011Form.txt_MontoBeneficio.value = datosEdit.monto_beneficio;
							document.AgregarSif011Form.dbx_TipoEmision.value = datosEdit.tipo_emision;
							document.AgregarSif011Form.dbx_TipoEgreso.value = datosEdit.cod_tipo_egreso;
							document.AgregarSif011Form.dbx_ModPago.value = datosEdit.modalidad_pago;
							document.AgregarSif011Form.txt_MontoDocumento.value = datosEdit.monto_documento;
							document.AgregarSif011Form.txt_NumeroSerie.value = Trim(datosEdit.numero_serie);
							document.AgregarSif011Form.txt_NumeroDocumento.value = Trim(datosEdit.numero_documento);
							document.AgregarSif011Form.txt_FechaEmision.value = Trim(datosEdit.fechaEmisionDocumentoDate);
							document.AgregarSif011Form.dbx_CodBanco.value = datosEdit.codigo_banco;
							document.AgregarSif011Form.dbx_FuenteOrigen.value = datosEdit.fuente_origen;
							document.AgregarSif011Form.dbx_TipoDeclaracion.value = datosEdit.tipo_declaracion;
			...
			document.AgregarSif011Form.txt_NumVerif.disabled = false;
			document.AgregarSif011Form.txt_NombreEmpresa.disabled = false;
			document.AgregarSif011Form.txt_NumeroDeclaracion.disabled = false;
			document.AgregarSif011Form.txt_RutAfiliado.disabled = false;
			document.AgregarSif011Form.txt_NumVerifAfiliado.disabled = false;
			document.AgregarSif011Form.txt_NombreAfiliado.disabled = false;
			document.AgregarSif011Form.dbx_TipoBeneficio.disabled = false;
			document.AgregarSif011Form.dbx_TipoBeneficiario.disabled = false;
			document.AgregarSif011Form.txt_RutCausante.disabled = false;
			document.AgregarSif011Form.txt_NumVerifCausante.disabled = false;
			document.AgregarSif011Form.txt_NombreCausante.disabled = false;
			document.AgregarSif011Form.dbx_TipoCausante.disabled = false;
			document.AgregarSif011Form.txt_FechaInicioBeneficio.disabled = false;
			document.AgregarSif011Form.txt_FechaTerminoBeneficio.disabled = false;
			document.AgregarSif011Form.txt_DiasAsfam.disabled = false;
			document.AgregarSif011Form.dbx_CodigoTramo.disabled = false;
			document.AgregarSif011Form.txt_MontoBeneficio.disabled = false;
			document.AgregarSif011Form.dbx_TipoEmision.disabled = false;
			document.AgregarSif011Form.dbx_TipoEgreso.disabled = false;
			document.AgregarSif011Form.dbx_ModPago.disabled = false;
			document.AgregarSif011Form.txt_MontoDocumento.disabled = false;
			document.AgregarSif011Form.txt_NumeroSerie.disabled = false;
			document.AgregarSif011Form.txt_NumeroDocumento.disabled = false;
			document.AgregarSif011Form.txt_FechaEmision.disabled = false;
			document.AgregarSif011Form.dbx_CodBanco.disabled = false;
			document.AgregarSif011Form.dbx_FuenteOrigen.disabled = false;
			document.AgregarSif011Form.dbx_TipoDeclaracion.disabled = false;
		}
	}
...
<div id="caja_registro">
	...
	<input type="hidden" name="dbx_filtroBusqueda">
	<input type="hidden" name="txt_rut">
	<input type="hidden" name="txt_digitoVerificador">
	<input type="hidden" name="txt_primerRango">
	<input type="hidden" name="txt_segundoRango">
	...
	<table width="1100" border="0">
		...
		<tr>
			<td height="25"><strong><p1>MODIFICAR REGISTROS REPORTE DIVISION REGIMENES LEGALES SIF011 </p1></strong></td>
		</tr>
		<tr></tr>
		...
		<tr>
			...
			<table width="100%" border="1" rules="groups">
				...
				<tr>
				  ...
				  <td colspan="4">						
				    <html:select property="dbx_FuenteOrigen" styleClass="dbx_modPago" value="0">
				...
				<tr>
				  <td  height="40"><strong>Tipo Declaraci&oacute;n</strong></td>
				  <td>
				  ...
				  <td>&nbsp;</td>
				  <td width="200"><strong>Numero Declaraci&oacute;n</strong></td>
				  <td>
				  <input type="text" name="txt_NumeroDeclaracion" id="txt_NumeroDeclaracion" size="20" maxlength="20" onkeyup="Upper(this,'T')"
				   onkeypress="Upper(this,'T')"/>
				  </td>
			  </tr>
 				<tr>
				  <td  height="40">
				  ...
				  <td>
				  <input type="text" name="txt_Rut" id="txt_Rut" maxlength="9" onkeyup="Upper(this,'N')" onkeypress="Upper(this,'N')" 
				  size="11" onblur="autoCompletarDigitoVerificador();"/><strong> - </strong><input type="text" name="txt_NumVerif" 
				  id="txt_NumVerif" size="1" maxlength="1" onkeyup="Upper(this,'A')" onkeypress="Upper(this,'A')">
				  </td>
				  <td>&nbsp;</td>
				  <td><strong>Nombre Empresa</strong></td>
				  <td><input type="text" name="txt_NombreEmpresa" id="txt_NombreEmpresa" size="50" maxlength="80" onkeyup="Upper(this,'A')" onkeypress="Upper(this,'A')"/>
				   </td>
			  </tr>
				<tr>
				  <td height="40"><strong>RUT Afiliado</strong></td>
				  <td><input type="text" name="txt_RutAfiliado" id="txt_RutAfiliado" maxlength="9" onkeyup="Upper(this,'N')" 
				  onkeypress="Upper(this,'N')" size="11" onblur="autocompletarDigitoAfiliado();"/><strong> - </strong><input type="text" 
				  name="txt_NumVerifAfiliado" id="txt_NumVerifAfiliado" size="1" maxlength="1" onkeyup="Upper(this,'A')" 
				  onkeypress="Upper(this,'A')">
				  </td>
				  <td>&nbsp;</td>
				  <td><strong>Nombre Afiliado</strong></td>
				  <td><input type="text" name="txt_NombreAfiliado" id="txt_NombreAfiliado" size="50" maxlength="80" onkeyup="Upper(this,'L')" 
				  onkeypress="Upper(this,'L')"/>
				  </td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td >
						...
						<IMG style="cursor:hand" border="0" src="./images/Calendar.jpg" width="16" height="16" onClick="ShowCalendarFor(txt_FechaInicioBeneficio);"/>					</td>
				  <td>&nbsp;</td>
					<td>
					...
					<td >
						...
						<IMG style="cursor:hand" border="0" src="./images/Calendar.jpg" width="20" height="18" onClick="ShowCalendarFor(txt_FechaTerminoBeneficio);"/>					</td>
			  </tr>
				<tr>
				...
				<tr>
					...
					<td>
						<input type="text" name="txt_MontoBeneficio" id="txt_MontoBeneficio" size="20" maxlength="15" onkeyup="Upper(this,'N')" onkeypress="Upper(this,'N')"/></td>
				  <td>&nbsp;</td>
					<td >
				...
				<tr>
					...
					<td colspan="4">
						<input type="text" name="txt_MontoDocumento" id="txt_MontoDocumento" size="20" maxlength="15" onkeyup="Upper(this,'N')" onkeypress="Upper(this,'N')"/>					</td>
				</tr>
				<tr>
		...
		<tr>
			<td height="37" align="right" width="80%">
				<input type="button" name="btn_Modificar" id="btn_Modificar" class="btn_limp" value="Modificar" onclick="modificarRegistroSif011();"/>
				&nbsp;&nbsp;&nbsp;
