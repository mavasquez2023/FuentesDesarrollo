<%@ taglib uri="/WEB-INF/struts-html.tld" prefix="html" %>
<%@ taglib uri="/WEB-INF/struts-bean.tld" prefix="bean" %>
<%@ taglib uri="/WEB-INF/struts-logic.tld" prefix="logic" %>

<style>
.nota {color: #333366; text-align: justify; vertical-align: top; font:8.5pt tahoma;}
</style>

<logic:notEmpty name="lista" scope="request">										
<table style="width : 100%; border : 0px;" cellpadding=0 cellspacing=0 >
	<tbody>
	
		<tr style="height : 18px;">
			<td class=smallfontreverse >
				<b>
					<bean:message key="liquidacion.671.nivel1.table.header" />&nbsp;
					<bean:message key="global.text.unidadMonetaria" />
				</b>
			</td>
		</tr>
		
		<tr>
			<td>
				<table style="width : 100%; border : 0px; border-color : #cccccc; padding-left: 2px; padding-right: 2px; border-spacing: 1px">
					<tbody>
					
						<tr style="background-color : #cccccc;">
							<td class="smalltableheader" style="width : 57%;" colspan="2"><bean:message key="liquidacion.671.nivel1.table.header.afiliado" /></td>
							<td class="smalltableheader" style="width : 18%;"><bean:message key="liquidacion.671.nivel1.table.header.folio" /></td>
							<td class="smalltableheader" style="width : 7%;"><bean:message key="liquidacion.671.nivel1.table.header.cuota" /></td>
							<td class="smalltableheader" style="width : 12;"><bean:message key="liquidacion.671.nivel1.table.header.monto" /></td>
						</tr>
						
						<logic:iterate id="detalle" name="lista" scope="request">							
						<tr>
							<td class="smallprompt" style="width : 18%;">
								<bean:write name="detalle" property="deudorCredito.rut.formattedRut" scope="page" />-
								<bean:write name="detalle" property="deudorCredito.rut.dv" scope="page" />					
							</td>
							<td class="smallprompt" style="width : 45%;">
								<bean:write name="detalle" property="deudorCredito.apellidoPaterno" scope="page" />&nbsp;
								<bean:write name="detalle" property="deudorCredito.apellidoMaterno" scope="page" />&nbsp;
								<bean:write name="detalle" property="deudorCredito.nombre" scope="page" />
							</td>
							<td class="smallprompt">
								<!--
									<bean:write name="detalle" property="codigoOficinaProceso" filter="true" ignore="true" />-
								-->
								<bean:write name="detalle" property="folio" filter="true" ignore="true" />
							</td>
							<td class="smallprompt" style="text-align: right">
								<bean:write name="detalle" property="numeroCuota" filter="true" formatKey="global.monto" ignore="true" />
							</td>
							<td class="smallcurrency">
								<bean:write name="detalle" property="monto" filter="true" formatKey="global.monto" ignore="true" />
							</td>
						</tr>
						</logic:iterate>

						<tr>
							<td colspan="5" style="height : 1px; background-color : #006666;"></td>
						</tr>							

						<tr>
							<td class="smallprompt" colspan="4">
								<bean:message key="liquidacion.671.nivel1.table.label.total" />
							</td>
							<td class="smallcurrency">
								<bean:write name="total" filter="true" formatKey="global.monto" ignore="true" />									
							</td>
						</tr>
					</tbody>
				</table>
			</td>
		</tr>

		<tr>
			<td>&nbsp;</td>
		</tr>

	</tbody>
</table>

<p />
</logic:notEmpty>


<logic:equal name="concepto" property="tieneSubsistema" value="false" scope="request">
	<table border="0" cellspacing="1" cellpadding="0">
		<tbody>
			<tr style="height : 20px;">
				<td>
					<html:img alt="" height="1" width="4" page="/img/c.gif"/>
				</td>
			</tr>
			
			<tr>
				<td class="nota" width="430px">
					<bean:write name="concepto" property="mensaje" scope="request" />
				</td>
			</tr>
			<tr><td style="height : 14px;"><html:img alt="" height="1" width="4" page="/img/c.gif"/></td></tr>
			<tr>
				<td>
				</td>
			</tr>
		</tbody>
	</table>
</logic:equal>	