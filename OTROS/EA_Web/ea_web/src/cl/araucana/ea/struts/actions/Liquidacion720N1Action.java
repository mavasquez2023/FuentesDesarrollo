 ...
 * @author
 */
public class Liquidacion720N1Action extends Action {
	...
	public ActionForward execute(
		...
		try {
			...
			Collection listaLlaveCarga = delegate.getListaLlaveCarga( 
			...
			if((listaLlaveCarga != null) && (listaLlaveCarga.size() > 1)) {
				...
				request.setAttribute("listaLlaveCarga", listaLlaveCarga);
				...
				toPage = "aPaginaCompensacionSaldoAFavorN1";
			} else if((listaLlaveCarga != null) && (listaLlaveCarga.size()==1)) {
				Long llave = null;
				Iterator it = listaLlaveCarga.iterator();
				while(it.hasNext()) {
					llave = (Long) it.next();
				}				
				PropertyUtils.setSimpleProperty(form, "llave", llave);			
				toPage = "aPaginaCompensacionSaldoAFavorN2";			
			} else {
				toPage = "error";
			}
		} catch (Exception e) {
