 ...
 * @author
 */
public class CertificadoSaldoDeudaCapitalAction extends Action {
	...
	public ActionForward execute(
		...
		try {
			...
			Collection creditos = delegate.getSaldoDeudaCapitalParaFiniquito(
																dtoEmpresa.getRut(), 
																dtoAfiliado.getRut(),
																dtoFechaFiniquito);
			...
			if((creditos != null) || (creditos.size() != 0)) {
				...
				while(it.hasNext()) {
					SaldoDeudaCapitalParaFiniquitoTO aCredito = (SaldoDeudaCapitalParaFiniquitoTO) it.next();
					total += aCredito.getSaldoCapital() + aCredito.getGravamenes();
				}
				folioCertificado = 10;
			}						
			...
			request.setAttribute("fechaFiniquito", dtoFechaFiniquito);
