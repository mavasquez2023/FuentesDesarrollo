...
import cl.araucana.ea.ctacte.dto.ChequePagoTO;
import cl.araucana.ea.ctacte.dto.ChequeTO;
 ...
 * @author
 */
public class Liquidacion751N2Action extends Action {
	...
	public ActionForward execute(
		...
		try {
			...
			idChanged = 
					(String) PropertyUtils.getSimpleProperty(form, "idChanged");
			folioMovimiento = 
					((Long) PropertyUtils.getSimpleProperty(form, "folioMovimiento")).longValue();
		} catch (Exception e) {
			StringBuffer msg = new StringBuffer();
			msg.append("\r\n\tAtrapado en: ").append(this.getClass().getName()).append("#execute");
		...
		try {
			...
			Collection lista = delegate.getChequesReemplazados(
										dtoEmpresa.getRut(),
										codigoOficina,
										codigoSucursal,
										dtoPeriodoRemuneracion);

			ChequeTO cheque = null;										
			if (lista.size() == 1) {
				// hay un solo cuque reemplazado por lo tanto lo muestra directamente o
				Iterator it = lista.iterator();
				while(it.hasNext()) {
					cheque = (ChequeTO) it.next();
				}
				request.setAttribute("cheque", cheque);
			} else if ("S".equalsIgnoreCase(idChanged)) {
				// un cheque fue seleccionado.
				Iterator it = lista.iterator();
				while(it.hasNext()) {
					cheque = (ChequeTO) it.next();
					if(folioMovimiento == cheque.getFolioComprobante()) {
						request.setAttribute("cheque", cheque);
					}
				}
			} else if (lista.size() > 1) {			
				// hay mas de un cheque reemplazado
				Iterator it = lista.iterator();
				while(it.hasNext()) {
					cheque = (ChequeTO) it.next();
				}				
				request.setAttribute("listaCheques", lista);
			} else {
				request.setAttribute("hayDetalle", new Boolean(false));		
			}
