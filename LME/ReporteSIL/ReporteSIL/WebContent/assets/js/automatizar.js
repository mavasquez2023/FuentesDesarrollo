/*
Libreria de automatizacion para jQuery
Autor: J-Factory Solutions
...
function automatizar_init () {
  ...
  $(':text, :password, textarea, select').each(function (i, el) {
    ...
    // forzar a que se validen los campos que no sean requeridos en caso de tener algùn caracter
  ...
  $(':text, textarea').each(function (i, el) {
    ...
    // bloqueo de teclas segùn patrones
    for (key in patrones) {
      if ($(el).hasClass(key)) {
        // patrones
        $(el).data('tipo', key);
  ...
  // crÃ©ditos
  console.log(
    'Se cargó la librería de automatización de J-Factory Solutions v' +
      version +
      '.'
  );
}
  ...
  if (tipo == 'tipo_fecha') {
	    ...
	    $(el).blur(function () {
	      if (
	        ...
	        aplicarClases(this, validarFecha(this.value));
	      } else {
	        aplicarClases(this, '');
	      }
	    });
  ...
  if (tipo == 'tipo_monto') {
	    ...
	    $(el).blur(function () {
	      if (
	        ...
	        aplicarClases(this, validarMonto(this.value));
	        formatearMonto(this);
	      } else {
	        aplicarClases(this, '');
	      }
...
function validarFecha(fecha){
	...
	if(fecha.length != 10){
		...
		var primer_slash = fecha.substr(2, 1);
		var segundo_slash = fecha.substr(5, 1);
		...
		}else if(isNaN(dd) || dd < 1 || dd>diasmes[mm-1]){
			estado = 'invalido';
		}else if(primer_slash != "\/" || segundo_slash != "\/"){
			estado = 'invalido';
		}else{
			estado = 'valido';
		}
