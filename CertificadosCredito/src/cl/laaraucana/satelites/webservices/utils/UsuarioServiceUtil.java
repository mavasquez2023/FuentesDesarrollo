		...
		try {
			...
			if (salidaSAP.getCodigoError().equals(ConstantesRespuestasAPP.COD_RESPUESTA_ERROR)) {
				...
				for (SalidaAfiliadoVO detSap : listaSap) {
					...
					String nombre="";
					if(detSap.getNombreCompleto()!=null){
						nombre= detSap.getNombreCompleto();
					}
					afiliado.setNombreAfiliado(nombre);
					String rutEmpresaSap="";
					if(detSap.getRutEmpresa()!=null){
						rutEmpresaSap = CompletaUtil.llenaConCeros(Utils.obtenerValorAnteriorA(detSap.getRutEmpresa(), "-"), 9, true);
