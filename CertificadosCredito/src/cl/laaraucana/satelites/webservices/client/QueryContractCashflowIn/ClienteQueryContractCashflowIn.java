...
//<=====	Codigo antiguo, sin validacion
	...
	*/
	...
	private SalidaListaQueryContractCashflowInVO mapear(QueryContractCashFlowResponse entrada){
		...
		salidaVO.setCuotasPagadas(String.valueOf(entrada.getCANT_CUOTAS_PAG()));
		salidaVO.setCuotasMorosas(String.valueOf(entrada.getCANT_CUOTAS_MOR()));
		salidaVO.setCuotasFuturas(String.valueOf(entrada.getCANT_CUOTAS_FUT()));
		...
		for (DetalleCuotasCF detalle : entrada.getDetalleCuotas()) {
			...
			if(detalle.getULT_FECHA_CONT_PAGO()!=null){
				cuotas.setUltFechaContablePago(Utils.dateToStringSAP(detalle.getULT_FECHA_CONT_PAGO()));
			}
		...
		CondonacionCashFlowVO condonacion= mapeoCondonacion(entrada);
		salidaVO.setCondonacion(condonacion);
