...
public class _OrqOutputRemoteHome_Stub extends Stub implements OrqOutputRemoteHome {
    
    private static final String[] _type_ids = {
        "RMI:cl.araucana.aporte.orqOutput.ejb.OrqOutputRemoteHome:0000000000000000", 
    ...
    public OrqOutputRemote create() throws CreateException, RemoteException {
        while(true) {
            if (!Util.isLocal(this)) {
                InputStream in = null;
                try {
                    try {
                        ...
                        in = (InputStream)_invoke(out);
                        return (OrqOutputRemote) in.read_Object(OrqOutputRemote.class);
                    } catch (ApplicationException ex) {
                        in = (InputStream) ex.getInputStream();
                        String id = in.read_string();
                ...
                ServantObject so = _servant_preinvoke("create",cl.araucana.aporte.orqOutput.ejb.OrqOutputRemoteHome.class);
                if (so == null) {
                    continue;
                }
                try {
                    OrqOutputRemote result = ((cl.araucana.aporte.orqOutput.ejb.OrqOutputRemoteHome)so.servant).create();
                    return (OrqOutputRemote)Util.copyObject(result,_orb());
                } catch (Throwable ex) {
                    Throwable exCopy = (Throwable)Util.copyObject(ex,_orb());
