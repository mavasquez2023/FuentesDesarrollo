...
public class _OrqInputRemoteHome_Stub extends Stub implements OrqInputRemoteHome {
    
    private static final String[] _type_ids = {
        "RMI:cl.araucana.aporte.orqInput.ejb.OrqInputRemoteHome:0000000000000000", 
    ...
    public OrqInputRemote create() throws CreateException, RemoteException {
        while(true) {
            if (!Util.isLocal(this)) {
                InputStream in = null;
                try {
                    try {
                        ...
                        in = (InputStream)_invoke(out);
                        return (OrqInputRemote) in.read_Object(OrqInputRemote.class);
                    } catch (ApplicationException ex) {
                        in = (InputStream) ex.getInputStream();
                        String id = in.read_string();
                ...
                ServantObject so = _servant_preinvoke("create",cl.araucana.aporte.orqInput.ejb.OrqInputRemoteHome.class);
                if (so == null) {
                    continue;
                }
                try {
                    OrqInputRemote result = ((cl.araucana.aporte.orqInput.ejb.OrqInputRemoteHome)so.servant).create();
                    return (OrqInputRemote)Util.copyObject(result,_orb());
                } catch (Throwable ex) {
                    Throwable exCopy = (Throwable)Util.copyObject(ex,_orb());
