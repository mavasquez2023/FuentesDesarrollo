...
import com.microsystem.lme.job.LmeCiclo;

import cl.laaraucana.util.email.Email;

public class SQLExceptionHandler{

	private static String connErrorSQLStates = Configuraciones.getConfig("conn.error.sql.states");
	private static String[] sqlStates = connErrorSQLStates.split(",");
	
	private static int cantErroresAntesNotif = Integer.parseInt(Configuraciones.getConfig("conn.error.qty"));//Cantidad de errores de conexion que se deben dar antes de notificar
	...
	private static boolean enviarCorreoNotificacion(String mensajeError){
		boolean res = false;
		String mailSession = BdConstants.getInstance().MAIL_SESSION;
		String to = BdConstants.getInstance().DESTINO_CORREO_NOTIF;
		String subject = BdConstants.getInstance().ASUNTO_CORREO_NOTIF;
		String bodyContent = BdConstants.getInstance().CUERPO_CORREO_NOTIF;
