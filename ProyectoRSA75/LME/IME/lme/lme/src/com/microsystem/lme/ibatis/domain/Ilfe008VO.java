package com.microsystem.lme.ibatis.domain;

public class Ilfe008VO {
	private int ideope;
	private int numlic;
	private String diglic;
	private int afirut;
	private String afirutdv;
	private int adscrito;
	private String codtipsu;

	public Ilfe008VO() {
	}

	public int getAdscrito() {
		return adscrito;
	}

	public void setAdscrito(int adscrito) {
		this.adscrito = adscrito;
	}

	public int getAfirut() {
		return afirut;
	}

	public void setAfirut(int afirut) {
		this.afirut = afirut;
	}

	public String getAfirutdv() {
		return afirutdv;
	}

	public void setAfirutdv(String afirutdv) {
		this.afirutdv = afirutdv;
	}

	public String getCodtipsu() {
		return codtipsu;
	}

	public void setCodtipsu(String codtipsu) {
		this.codtipsu = codtipsu;
	}

	public String getDiglic() {
		return diglic;
	}

	public void setDiglic(String diglic) {
		this.diglic = diglic;
	}

	public int getIdeope() {
		return ideope;
	}

	public void setIdeope(int ideope) {
		this.ideope = ideope;
	}

	public int getNumlic() {
		return numlic;
	}

	public void setNumlic(int numlic) {
		this.numlic = numlic;
	}

}
