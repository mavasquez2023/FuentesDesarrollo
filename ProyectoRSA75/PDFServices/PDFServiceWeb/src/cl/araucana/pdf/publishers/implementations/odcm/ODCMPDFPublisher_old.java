 ...
 * @version 1.0
 */
public class ODCMPDFPublisher_old extends PDFPublisher {
	 ...
	 */	
	public ODCMPDFPublisher_old(String docType, int docVersion,
	...
	public void flush() throws PDFPublisherException {
		...
		synchronized (ODCMPDFPublisher_old.class) {
			indexFileNo = ++ODCMPDFPublisher_old.indexFileNo;
		}
		...
		try {
			while (iterator.hasNext()) {
				...
				if (nIndexedDocs % usedPartitionSize == 0) {
					 ...
					 */
					out.println("FIELD NAMES BEGIN:");

					for (int i = 0; i < fields.length; i++) {
						out.println(fields[i].getName());
					}

					out.println("FIELD NAMES END:");
				}
...
*/
	...
	private int executeARSCommand(String[] args, boolean stdout,
		...
		synchronized (ODCMPDFPublisher_old.class) {
			cmdNo = ++ODCMPDFPublisher_old.cmdNo;
		}
