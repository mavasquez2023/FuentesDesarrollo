...
/*
 * @(#) LocateInstruction.java    1.0 08-04-2008
 *
 ...
 * @version 1.0
 */
public class LocateInstruction extends Instruction {

	private Expression y;
	private Expression x;

	public LocateInstruction(String label, Expression y, Expression x) {
		super(FPGCode.LOCATE, label, true);

		if (y.subType != FPGCode.TYPE_INT || x.subType != FPGCode.TYPE_INT) {
			throw new IllegalArgumentException(
					"Expression types must be 'int'.");
		}
	...
	public void execute(CompiledPDFObject compiledObject,
		...
		int cy = compiledObject.getIntValue(y);
		int cx = compiledObject.getIntValue(x);

		compiledObject.write(
				compiledObject.getXYBase() + cy + " " + cx + " Tm\n");
	}
}
