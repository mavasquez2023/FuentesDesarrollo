

package cl.araucana.fpg.compiled;


/*
 * @(#) CodeSegment.java    1.0 07-04-2008
 *
 * Este código fuente pertenece a la Caja de Compensación de Asignación Familiar
 * La Araucana (C.C.A.F.). Su utilización y reproducción es confidencial y está
 * restringido a los sistemas de información propios de la institución.
 */


import cl.araucana.fpg.FPGException;

import cl.araucana.fpg.compiled.instructions.Instruction;


/**
 * ...
 *
 * <BR>
 *
 * <TABLE BORDER="1" WIDTH="100%" CELLPADDING="3" CELLSPACING="0" SUMMARY="">
 *    <TBODY>
 *        <TR BGCOLOR="#CCCCFF" CLASS="TableHeadingColor">
 *            <TH ALIGN="left" COLSPAN=4> <FONT SIZE="+2">
 *                 <B>Registro de Mantenciones</B></FONT>
 *            </TH>
 *        </TR>
 *
 *        <TR>
 *            <TD align="center"> <B>Fecha</B> </TD>
 *            <TD align="center"> <B>Versión</B> </TD>
 *            <TD> <B>Autor</B> </TD>
 *            <TD align="left"><B>Descripción</B></TD>
 *        </TR>
 *
 *        <TR BGCOLOR="white" CLASS="TableRowColor">
 *            <TD> 07-04-2008 </TD>
 *            <TD align="center">  2.0 </TD>
 *            <TD> Germán Pavez I. <BR> gpavez@hotmail.com </TD>
 *            <TD> Versión inicial. </TD>
 *        </TR>
 *    </TBODY>
 * </TABLE>
 *
 * <BR>
 *
 * @author Germán Pavez I. (gpavez@hotmail.com)
 *
 * @version 1.0
 */

public class CodeSegment {

	protected Instruction[] instructions;

	public void execute(CompiledPDFObject compiledObject) throws FPGException {

		FlowControl flowControl = new FlowControl();
		int pc = 0;

		do {
			flowControl.flow = FlowControl.NEXT;

			instructions[pc].execute(compiledObject, flowControl);

			switch (flowControl.flow) {
				case FlowControl.NEXT:
					pc++;

					break;

				case FlowControl.JUMP:
					pc = flowControl.pc;

					break;

				case FlowControl.END:

					return;

				default:

					throw new FPGException(
							  "Unexpected flow control "
							+ "[" + flowControl.flow + "]");
			}
		} while (true);
	}
}
