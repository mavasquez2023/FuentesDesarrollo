...
<html>
	...
	function Aceptar(){
		...
		window.parent.frames['frmResultado'].document.all.NombreIsapre.value = form1.NombreIsapre.value;
	...
	function BajarPDF(){
		...
		if (form1.NomIsapre.selectedIndex != 0) {
			strCondicion = strCondicion + " AND NombreIsapre LIKE ''" + form1.NomIsapre.value + "%''";
	    }
		...
		window.parent.frames['frmDownload'].document.all.inTipo.value = "5";
...
<body leftmargin="3" topmargin="3" marginwidth="3" marginheight="3" onload="window.open('PlanillaIsapre_Obj.htm','frmResultado');window.open('Download.htm','frmDownload');sizeRows();">
	
<FORM id="form1">
<input type="hidden" value="Planillas Isapres Holding:Planillas Isapres" name="_folder">				<font class="titulo">Solicitud Planillas Isapres</font>
				<br/>
				<br/>
			...
			<table width="650" border="0" cellspacing="5" cellpadding="0" id="tabla_cont">
					...
					<tr>
						...
						<td align="left" valign="middle" nowrap="nowrap">Isapre :</td>
						<td align="left" valign="middle">
							<select name='NombreIsapre' id='NombreIsapre' size='1' style='font-family:Arial;font-size:7pt;' onChange='rut_razon()'>
								<%if (!usuario.equals("entidad")){ %>
								<option value=''>--Todas--</option>
								<%} %>
								<c:forEach var="nomisapre" items="${ISAPRE}">
								<option value='${nomisapre.value}'>${nomisapre.nombre}</option>
								</c:forEach>
								<%if (!usuario.equals("entidad")){ %>
								<option value='CRUZ DEL NORTE'>CRUZ DEL NORTE</option>
								<option value='FERROSALUD'>FERROSALUD</option>
								<option value='ISAPRE CHUQUICAMATA'>ISAPRE CHUQUICAMATA</option>
								<option value='ISAPRE ING SALUD S.A'>ISAPRE ING SALUD S.A</option>
								<option value='ISAPRE OPTIMA'>ISAPRE OPTIMA</option>
								<option value='ISAPRE RIO BLANCO'>ISAPRE RIO BLANCO</option>
								<option value='MAS VIDA'>MAS VIDA</option>
								<option value='NORMEDICA'>NORMEDICA</option>
								<option value='PROMEPART'>PROMEPART</option>
								<option value='SAN LORENZO ISAPRE LTDA.'>SAN LORENZO ISAPRE LTDA.</option>
								<%} %>
							</select>
						</td>
