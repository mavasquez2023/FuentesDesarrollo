...
<html>
	<head>
		<title>Planillas Afiliado Voluntario</title>
	...
	function Aceptar(){
		...
		window.parent.frames['frmResultado'].document.all.NombreAFP.value = form1.NombreAFP.value;
		...
		window.parent.frames['frmResultado'].document.all.RazonSocial.value = form1.RazonSocial.value;		
	...
	function BajarPDF(){
		...
		if (form1.RazonSocial.selectedIndex != 0) {
			strCondicion = strCondicion + " AND RazonSocial LIKE ''" + form1.RazonSocial.value + "%''";
	    }	    
...
<body leftmargin="3" topmargin="3" marginwidth="3" marginheight="3" onload="window.open('PlanillaCotAPV_Obj.htm','frmResultado');window.open('Download.htm','frmDownload');sizeRows();">
	
		<FORM id="form1">
		<input type="hidden" value="Planillas AFPs AV Holding:Planillas AFPs AV" name="_folder">
					<font class="titulo">Solicitud Planillas Afiliado Voluntario</font>
					<br/>
					<br/>
			...
			<table width="650" border="0" cellspacing="5" cellpadding="0" id="tabla_cont">
				...
				<tr>
					...
					<td align="left" valign="middle" nowrap="nowrap">AFP :</td>
					<td align="left" valign="middle"><!-- <div id="divAFP"></div> --> 
						<select name='NombreAFP' id='NombreAFP' size='1' style='font-family:Arial;font-size:7pt;' onChange='rut_razon()'>
							<%if (!usuario.equals("entidad")){ %>
								<option value=''>--Todas--</option>
							<%} %>
							<c:forEach var="nomafp" items="${AFP}">
								<option value='${nomafp}'>${nomafp}</option>
							</c:forEach>
							<c:if test="${TipoUsuario=='empresa'}">
								<option value='BANSANDER S.A.'>BANSANDER S.A.</option>
								<option value='ING AFP SANTA MARIA'>ING AFP SANTA MARIA</option>
				...
				<tr>
					...
					<td align="left" valign="middle">${tipoUsuario}
						<!-- <div id="divRut"></div> -->
						<c:choose>
