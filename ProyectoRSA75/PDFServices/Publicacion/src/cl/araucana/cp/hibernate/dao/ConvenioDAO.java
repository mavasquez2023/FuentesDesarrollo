...
public class ConvenioDAO
{
	...
	public List getConveniosEmpresa(boolean flag, int idEmpresa) throws DaoException
	{
		try
		{
			...
			return crit.addOrder(Order.desc("descripcion")).list();
		} catch (Exception ex)
		{
	 ...
	 * @throws DaoException
	 */
	public List getConvenios(int idGrupoConvenio) throws DaoException {
		try {
			List lista = this.session.createCriteria(ConvenioVO.class)
			.add(Restrictions.eq("idGrupoConvenio", new Integer(idGrupoConvenio))).list();
			return lista;
		} catch (Exception ex) {
			logger.error("Error en ConvenioDAO.getConvenios::" + ex);
			throw new DaoException("Error en ConvenioDAO.getConvenios::", ex);
		}		
	}
