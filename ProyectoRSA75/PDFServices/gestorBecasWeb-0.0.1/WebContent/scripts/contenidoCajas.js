...
function soloPuntajePSU(objCampo, newVals)
{
	var vals="1234567890" + newVals;
	validaFormato(objCampo,vals,false,true);

	if(parseInt(objCampo.value) <= 850) {
		var puntajeLenguaje = parseInt(document.getElementsByName("puntajeLenguaje")[0].value) || 0;
		var puntajeMatematica = parseInt(document.getElementsByName("puntajeMatematica")[0].value) || 0;
		var puntajePromedio = Math.round((puntajeLenguaje + puntajeMatematica) / 2);
		if (puntajePromedio > 0 && puntajePromedio <= 850 && parseInt(objCampo.value) <= 850) {
			document.getElementsByName("puntajePromedio")[0].value = puntajePromedio;
			document.getElementsByName("puntajePromedioHidden")[0].value = puntajePromedio;
		} 
	} else {
			if(objCampo.name == "puntajeLenguaje") {
				alert("Debe ingresar puntaje de lenguaje y comunicación, y no debe superar los 850 puntos")
			} else {
				alert("Debe ingresar puntaje de matemática y no debe superar los 850 puntos")
			}
			objCampo.value = "";
...
function soloDecimales2(objCampo) {
	var vals="1234567890,";
	validaFormato(objCampo,vals,false,true);
	$("#calificacionIN").val(function(index, value) {
