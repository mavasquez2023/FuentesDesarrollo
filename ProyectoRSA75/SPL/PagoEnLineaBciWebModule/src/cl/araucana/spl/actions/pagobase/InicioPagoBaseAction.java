...
public abstract class InicioPagoBaseAction extends AppAction {
	...
	protected abstract String preIngresoCompra(HttpServletResponse response, Transaccion trx);
	...
	protected ActionForward doTask(User user, ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			//Se invoca Preingreso banco
			String resultado_preingreso= preIngresoCompra(response, trx);
			ActionForward forward=null;
			if(resultado_preingreso.equals("OK")){
				forward = redireccionCgi(request, response, mapping, trx);
				forward=null;
				if (forward != null) {
					return forward;
				}
			}else{
				forward= mapping.findForward("precompra");
				return forward;
			}
		} catch (Exception ex) {
