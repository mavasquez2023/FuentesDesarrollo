...
public class InicioPagoBciAction extends InicioPagoBaseAction {
	...
	protected String preIngresoCompra(HttpServletResponse resp, Transaccion trx) {
		String salida= "";
		try {
			...
			//RedirectPrepago redirect= new RedirectPrepago(resp, url_preingreso);
			//redirect.start();
	
			//URL url = new URL(url_preingreso);
			HttpURLConnection conn= (HttpURLConnection)new URL(url_preingreso).openConnection();
			 int responseCode = conn.getResponseCode();

			 if (200 <= responseCode && responseCode <= 299) {
			        logger.info("Codigo respuesta exitoso: " + responseCode);
			    } else {
			    	logger.warn("Codigo respuesta no válido: " + responseCode);
			    }
			...
			logger.info("Salida banco= " + salida);
			reader.close();

		} catch (Exception e) {
			logger.warn("Error invocando URL Preingreso del banco: " + e.getMessage());
			//e.printStackTrace();
		}
