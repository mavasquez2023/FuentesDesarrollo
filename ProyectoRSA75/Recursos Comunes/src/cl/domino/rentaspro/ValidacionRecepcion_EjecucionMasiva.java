 ...
 *
 */
public class ValidacionRecepcion_EjecucionMasiva extends AgentBase {
	...
	public static void main(String[] args) {
		ValidacionRecepcion_EjecucionMasiva valida= new ValidacionRecepcion_EjecucionMasiva();
		valida.NotesMain();
				...
				if (param.getArchivo()!=null) {
					...
					if (formato.equals("TXT")||formato.equals("CSV")||formato.equals("XLS")||formato.equals("ZIP")){
						if(formato.equals("ZIP")){
							...
							try {
								int rutemp= Integer.parseInt(rutempresa.substring(0, rutempresa.indexOf(".")));
	...
	public void enviarMail(int conerror_envio, Vector mailEncargados, Vector mensaje, ParametrosEnvio paramEnvio) {
		...
		try {			
			...
			if(conerror_envio==0){
				...
				body.append("Señor Empleador: su archivo ha sido recibido exitosamente. <BR>");
				body.append("Le informaremos dentro de las próximas horas el resultado del proceso.<br>");
