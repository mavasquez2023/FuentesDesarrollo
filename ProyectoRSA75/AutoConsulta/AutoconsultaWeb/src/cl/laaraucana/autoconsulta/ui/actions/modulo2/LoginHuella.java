...
public class LoginHuella extends Action
{
  ...
  if (session.getAttribute("chkUser") != null) {
    ...
    try {rutUsuario = Long.parseLong(user.substring(0, user.indexOf('-')).trim()); } catch(Exception ex) {rutUsuario=0;}
    if (rutUsuario==0) {
    	session.invalidate();
  ...
  System.out.println("---> getDatosUsuario user [" + rutUsuario + "]");  
  usuario = delegate.getDatosUsuario(rutUsuario);
    ...
    UserPrincipal urPrincipal = new UserPrincipal(String.valueOf(usuario.getRut()),"");
    ...
    session.removeAttribute("esSoloEncargado");
  ...
  if (usuario == null) {
    ...
    session.setAttribute("security.message", "errors.security.nouser");
      ...
      System.out.println("Usuario Encargado?: " + usuario.isEsEncargadoEmpresa());
      ...
      System.out.println("no es empresa, es delegado");
      if (!usuario.isEsEmpresa()) {
        ...
        if ((empresas != null) && (empresas.size() > 0))
        {
            ...
            (empresaACargo.length() == 0))
          {
            ...
            if ( ! (
            	...
            	session.setAttribute("esSoloEncargado","ok");
            }
