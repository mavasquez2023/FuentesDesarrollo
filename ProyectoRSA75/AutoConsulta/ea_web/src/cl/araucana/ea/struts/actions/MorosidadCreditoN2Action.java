...
import cl.araucana.ea.ctacte.dto.ResumenMorosidadCreditoCuentaCorrienteTO;
 ...
 * @author
 */
public class MorosidadCreditoN2Action extends Action {
	...
	public ActionForward execute(
		...
		try {
			...
			codigoConcepto = ((Integer)
					PropertyUtils.getSimpleProperty(form, "codigoConcepto")).intValue();
			periodo = ((Long)
					PropertyUtils.getSimpleProperty(form, "periodo")).longValue();
		} catch (Exception e) {
			StringBuffer msg = new StringBuffer();
			msg.append("\r\n\tAtrapado en: ").append(this.getClass().getName()).append("#execute");
		...
		try {
			...
			Collection detalles = delegate.getCreditosMorososCuentaCorriente(new RutTO(rutEmpresa, ""), codigoOficina,	codigoSucursal,	codigoConcepto, new PeriodoTO(periodo));
			ResumenMorosidadCreditoCuentaCorrienteTO resumen = delegate.getResumenCreditosMorososCuentaCorriente(detalles);
			...
			request.setAttribute("periodo", new PeriodoTO(periodo));
