...
public class Encargado {
	...
	public void setDocumentTypesCountForAllEmpresas(int period) {
		Collection empresasValues = empresas.values();
		Iterator iterator = empresasValues.iterator();
		...
		while (iterator.hasNext()) {
			...
			for (int i = 0; i < documentTypes.length; i++) {
				...
				int documentCount =
						documentController.getDocumentCount(
								documentTypeName,
								period,
								rutEmpresa);

				empresa.setDocumentCount(documentTypeName, documentCount);
			}			
		}
	}
}
