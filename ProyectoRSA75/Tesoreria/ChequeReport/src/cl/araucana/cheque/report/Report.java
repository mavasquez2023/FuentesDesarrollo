...
import cl.araucana.cheque.thread.EstadisticasThread;
...
public String createReport(int folio, ParamURL paramurl, EstructuraCarpeta carpetas){
	TesoDAO tesodao= null;
	String mensaje=null;
	try {
		log.info("Conectándose a Tesorería");
		tesodao= new TesoDAO();
		...
		//Instanciando Thread para guardar estadísticas
		param_map.put("cola", paramurl.getCola());
		param_map.put("dispositivo", paramurl.getDispositivo());
		param_map.put("usuario", paramurl.getCajero());
		param_map.put("ipremota", paramurl.getIpRemota());
		param_map.put("username", paramurl.getUsername());
		
		EstadisticasThread ethread= new EstadisticasThread(param_map);
		ethread.start();
		...
		if(param_map.size()>0){
			...
			log.debug("Monto en letras" + montoletra);
			param_map.put("montoletra", montoletra);

			//Se carga diseño del template
			log.debug("Compilando Jasper Report");
			...
			log.info("Exportando PDF, ruta:" + Parametros.getInstance().getRutaPDF()+ paramurl.getRuta() + folio + ".pdf");
			//JasperExportManager.exportReportToPdfFile(jPrint, Parametros.getInstance().getUnidad()+ pathout + folio + ".pdf");

			OutputStream output= carpetas.getOutput(Parametros.getInstance().getRutaPDF() + paramurl.getRuta() + folio + ".pdf");
			...
			mensaje= "Folio no existe en Tesorería: " + folio;
			return mensaje;
		}
		return "1";
	} catch (Exception e) {
...
//		Instanciando Thread para guardar actualizar estadísticas
		EstadisticasThread ethread= new EstadisticasThread(folio, estado, mensaje);
		ethread.start();
		if(tesodao!=null){
