 ...
 * @author
 */
public class OpcionesRangoFuturoAction extends Action {
	...
	public ActionForward execute(
		...
		Logger logger = Logger.getLogger(OpcionesRangoFuturoAction.class);
		...
		logger.debug("indice:"+(String)dynaValidatorActionForm.get("indiceFuturo"));
		logger.debug("opcion:"+(String)dynaValidatorActionForm.get("opcionFuturo"));
		int opcion=(int)Integer.parseInt((String)dynaValidatorActionForm.get("opcionFuturo"));
		int indice=0;
		String index=(String)dynaValidatorActionForm.get("indiceFuturo");
		...
		if (cobertura.getRangoFuturo() == null)
			cobertura.setRangoFuturo(new VigenciaRango());
		cobertura.getRangoFuturo().setInicioVigencia(formatter.parse(inicioVig));
		if (!finVig.equals(""))
			cobertura.getRangoFuturo().setFinVigencia(formatter.parse(finVig));
		...
		switch(opcion){			
			case 1:		cobertura.getRangoFuturo().addRango(rango,cobertura.getTipoCobertura());	
						break;
			case 2:		cobertura.getRangoFuturo().removeRango(indice);
						cobertura.getRangoFuturo().addRango(rango,cobertura.getTipoCobertura());
						break;
			case 3:		cobertura.getRangoFuturo().removeRango(indice);		
						break;
			case 4:		ServicesCoberturasDelegate delegate=new ServicesCoberturasDelegate();
						delegate.actualizaRangosFuturosCobertura(cobertura);		
						target="success";
						break;
			default:	break;
		}
		...
		String referer="/getListaRangoFuturo.do";
		request.getSession(false).setAttribute("referer",referer);
		ArrayList listaRangos=cobertura.getRangoFuturo().getRangos();
