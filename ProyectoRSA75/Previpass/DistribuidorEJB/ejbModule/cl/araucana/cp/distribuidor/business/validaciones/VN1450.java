...
public class VN1450 extends Validacion 
{
	private static Logger log = Logger.getLogger(VN1450.class);
	/*
	 * 1 parametro = VN1450: remuneracion imponible Isapre
	 * 
	 * Mensajes
	...
	private int validaMonto(CotizanteVO cotizante, int monto){
		...
		{
			if (this.logear)
				log.info("validacion VN1450 ERR:remuneracion imponible:invalido:" + monto + "::");
		...
		{       
			...
			if (monto > topeINP)
			{
				...
				if (this.logear)
					log.info("validacion VN1450 ERR:remuneracion imponible sobre el tope: montoRecibido:" + monto + ":tope calculadoINP:" + topeINP + "::");
		...
		} else if (cotizante.getTipoPrevision() == Constants.TIPO_PREVISION_AFP)//AFP
		{
			...
			if (monto > topeAFP)
			{
				...
				if (this.logear)
					log.info("validacion VN1450 ERR:remuneracion imponible sobre el tope: montoRecibido:" + monto + ":tope calculadoAFP:" + topeAFP + "::");
				return 131; // Remuneracion imponible mayor al tope permitido
			}
		}
	...
	public VN1450(HashMap parametrosNegocio, Session session, ConvenioVO datosConvenio)
	{
		super(parametrosNegocio, session, datosConvenio);
	}

	public VN1450(HashMap parametrosNegocio, Utils setter, Session session, ConvenioVO datosConvenio, List parametros, List listaConceptos)
	{
		super(parametrosNegocio, setter, session, datosConvenio, parametros, listaConceptos);
	}

	public VN1450(HashMap parametrosNegocio, Utils setter, Session session, ConvenioVO datosConvenio, List parametros, List listaConceptos, List listaMapeosValidaciones)
	{
