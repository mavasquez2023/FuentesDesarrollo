...
//				clillo 3-12-14 por Reliquidación
	...
	public List getDetalleSecciones(long idCodigoBarra, int tipoSeccion) throws DaoException
	{
		try
		{
			Class tipo = SeccionVO.class;
			
			return this.session.createCriteria(tipo).add(Restrictions.eq("idCodigoBarra", new Long(idCodigoBarra)))
													.add(Restrictions.eq("idTipoSeccion", new Integer(tipoSeccion))).list();
			
		} catch (Exception ex)
		{
			log.error("Error en CotizanteDAO.getDetalleSecciones");
			throw new DaoException("CotizanteDAO:getDetalleSecciones:", ex);
		}
	}
}