...
public class VN490 extends Validacion
{
	private static Logger log = Logger.getLogger(VN490.class);
	/*
	 * 1 parametro = VN490: entidad ahorro APV 4
	 * 
	 * Mensajes
	 * 		1693: Codigo Entidad de Ahorro Previsional Voluntario invalido
	 * 		1703: Codigo Entidad de Ahorro Prev. Volun. no aparece en mapeo
...
//			clillo 03/03/15 si empresa envía varios ceros se considera como uno solo (es decir, sin APV)
			...
			{
				if (this.logear) 
					log.info("validacion VN490:codigo recibido vacio, se asigna codigo 'sin APV'::");
        	...
        	{
        		if (this.logear) log.info("validacion VN490 ERR:no se encontro APV1:codigo recibido:" + codigo + "::");
        	...
        	else
        	{
        		...
        		cotizante.addApv(mApv.getId());
        	   	if (this.logear) log.info("validacion VN490 OK: APV1:codigo recibido:" + codigo + ":entidad:" + mApv.getId() + "::");
        	}
        	return this.COD_CUMPLE_VALIDACION;
		} catch(Exception e)
		{
			log.error("error validacion VN490::", e);
	...
	public int validaFromWEB(CotizanteVO cotizante)
	{
		try
		{
			...
			{
				this.resultado = "fin";
				if (this.logear) log.info("validacion VN490 : no se valida mas::");
				//cotizante.addApv(Constants.SIN_APV);
			...
			{
				if (this.logear) log.info("validacion VN490 ERROR getIdEntApv no encontrada:" + apv.getIdEntidadApv() + "::");
			  	return 169;//Monto APV invalido
			}
			if (apv.getIdEntidadApv() == Constants.SIN_APV)//no hay valor, no se valida nada mas
			{
				this.resultado = "fin";
				if (this.logear) log.info("validacion VN490 : codigo:" + apv.getIdEntidadApv() + ": no se valida mas::");
				//cotizante.addApv(Constants.SIN_APV);
				cotizante.setApvList(Utils.limpiaListaApv(cotizante.getApvList()));
				return this.COD_CUMPLE_VALIDACION;				
			}
			this.resultado = "";
			if (this.logear) log.info("validacion VN490 OK::");
			return this.COD_CUMPLE_VALIDACION;
		} catch(Exception e)
		{
			log.error("error validacion VN490::", e);
	...
	public VN490(HashMap parametrosNegocio, Session session, ConvenioVO datosConvenio)
	{
		super(parametrosNegocio, session, datosConvenio);
	}

	public VN490(HashMap parametrosNegocio, Utils setter, Session session, ConvenioVO datosConvenio, List parametros, List listaConceptos)
	{
		super(parametrosNegocio, setter, session, datosConvenio, parametros, listaConceptos);
	}

	public VN490(HashMap parametrosNegocio, Utils setter, Session session, ConvenioVO datosConvenio, List parametros, List listaConceptos, List listaMapeosValidaciones)
	{
