...
public class VN470 extends Validacion
{
	private static Logger log = Logger.getLogger(VN470.class);
	/*
	 * 1 parametro = VN470: entidad ahorro APV 3
	 * 
	 * Mensajes
	 * 		1692: Codigo Entidad de Ahorro Previsional Voluntario invalido
	 * 		1702: Codigo Entidad de Ahorro Prev. Volun. no aparece en mapeo
...
//			clillo 03/03/15 si empresa envía varios ceros se considera como uno solo (es decir, sin APV)
			...
			if (codigo.equals(""))
			{
				if (this.logear) log.info("validacion VN4370:codigo recibido vacio, se asigna codigo 'sin APV'::");
        	...
        	{
        		if (this.logear) log.info("validacion VN470 ERR:no se encontro APV1:codigo recibido:" + codigo + "::");
        	...
        	else
        	{
        		...
        		cotizante.addApv(mApv.getId());
        	   	if (this.logear) log.info("validacion VN470 OK: APV1:codigo recibido:" + codigo + ":entidad:" + mApv.getId() + "::");
        	}
		...
		{
			log.error("error validacion VN470::", e);
	...
	public int validaFromWEB(CotizanteVO cotizante)
	{
		try
		{
			...
			{
				this.resultado = "fin";
				if (this.logear) log.info("validacion VN470 : no se valida mas::");
				//cotizante.addApv(Constants.SIN_APV);
			...
			{
				if (this.logear) log.info("validacion VN470 ERROR getIdEntApv no encontrada:" + apv.getIdEntidadApv() + "::");
			  	return 169;//Monto APV invalido
			}
			if (apv.getIdEntidadApv() == Constants.SIN_APV)//no hay valor, no se valida nada mas
			{
				this.resultado = "fin";
				if (this.logear) log.info("validacion VN470 : codigo:" + apv.getIdEntidadApv() + ": no se valida mas::");
				//cotizante.addApv(Constants.SIN_APV);
				cotizante.setApvList(Utils.limpiaListaApv(cotizante.getApvList()));
				return this.COD_CUMPLE_VALIDACION;				
			}
			this.resultado = "";
			if (this.logear) log.info("validacion VN470 OK::");
			return this.COD_CUMPLE_VALIDACION;
		} catch(Exception e)
		{
			log.error("error validacion VN470::", e);
	...
	public VN470(HashMap parametrosNegocio, Session session, ConvenioVO datosConvenio)
	{
		super(parametrosNegocio, session, datosConvenio);
	}

	public VN470(HashMap parametrosNegocio, Utils setter, Session session, ConvenioVO datosConvenio, List parametros, List listaConceptos)
	{
		super(parametrosNegocio, setter, session, datosConvenio, parametros, listaConceptos);
	}

	public VN470(HashMap parametrosNegocio, Utils setter, Session session, ConvenioVO datosConvenio, List parametros, List listaConceptos, List listaMapeosValidaciones)
	{
