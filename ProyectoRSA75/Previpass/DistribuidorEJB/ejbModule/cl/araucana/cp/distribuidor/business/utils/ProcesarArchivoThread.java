...
public class ProcesarArchivoThread extends Thread {	
    ...
    public void run() { 
    	...
    	try { 
			...
			//logger.info("\n\Inicio validación VNs Cargas Familiares:" + stressInicio.getTime() + "::");
			...
			for (Iterator it = nominas.keySet().iterator(); it.hasNext(); ) {
				...
				if (cotizantes != null || cotizantes.size() > 0) {
					for (Iterator it2 = cotizantes.iterator(); it2.hasNext() && numeroValidaciones <= 100;) {
						...
						//Se busca el ID_CAUSA máximo de los registros ya existentes en CAUSAAVISOCXP
						//int contador = 1;
...
//								clillo 3-12-14 por Reliquidación
				...
				if (listaCausaAviso.size() > 0)
					evento.setComentarios("Proceso: " + tipoProceso + ", Empresa: " + rutEmpresa + ", Convenio: " + idConvenio + ". Validación: ERRONEA");
				else
					evento.setComentarios("Proceso: " + tipoProceso + ", Empresa: " + rutEmpresa + ", Convenio: " + idConvenio + ". Validación: CORRECTA");
