...
public class VN520 extends Validacion
{
	private static Logger log = Logger.getLogger(VN520.class);
	/*
	 * 1 parametro = VN520: monto ahorro APV 5, requiere: 510
	 * 
	 * Mensajes
	 *		1714: Monto APV debe existir si existe Codigo de Entidad APV 
	 *		1724: Monto APV invalido
	 *		1734: Monto APV debe ser menor que tope
	 * 		2354: CODIGO ENTIDAD APV DEBE EXISTIR SI EXISTE MONTO APV
	 */
	public int valida(CotizanteVO cotizante)
	{
		try
		{
			...
			if (valor < 0)
			{
				if(this.logear) log.info("validacion VN520 ERR: Ahorro invalido:" + c.getValor() + "::");
			...
			{
				if(this.logear) log.info("validacion VN520 ERR: Ahorro APV mayor a cero:" + valor + ":pero no se informa entidad:");
			...
			if (valor > topeAPV)
			{
				if(this.logear) log.info("validacion VN520 ERR: Ahorro APV:" + valor + ": mayor a tope:" + topeAPV + "::");
			...
			{	
				if(this.logear) log.info("validacion VN520 OK con APV_INVALIDO-->SIN APV");
			...
			{
				if(this.logear) log.info("validacion VN520 ERR: Ahorro debe existir:" + c.getValor() + "::");
				return 1710 + cotizante.getApvList().size() - 1;//Monto APV debe existir si existe Codigo de Entidad APV
			}

			if (apv.getIdEntidadApv() == Constants.SIN_APV && valor == 0)
			{
				if(this.logear) log.info("validacion VN520 OK:");
				return this.COD_CUMPLE_VALIDACION;
			}

			if (this.logear) log.info("validacion VN520 OK: Monto APV5:" + valor + "::");
			return this.COD_CUMPLE_VALIDACION;
		} catch(Exception e)
		{
			log.error("error validacion VN520::", e);
	...
	public int validaFromWEB(CotizanteVO cotizante)
	{
		...
		{			
			...
			if (apv != null)
			{
				if (apv.getAhorro() < 0)
				{
					if (this.logear) log.info("validacion VN520 ERROR Ahorro invalido:" + apv.getAhorro() + "::");
					return 172;//Monto APV invalido
				} else if (apv.getAhorro() == 0)
				{
					if (this.logear) log.info("validacion VN520 ERROR Ahorro debe existir:" + apv.getAhorro() + "::");
...
//				topeAPV *= new Float((String)this.parametrosNegocio.get("" + Constants.PARAM_UF_ACTUAL)).floatValue();
				...
				{
					if(this.logear) log.info("validacion VN520 ERR: Ahorro APV mayor a tope:" + topeAPV + "::");
	...
	public VN520(HashMap parametrosNegocio, Session session, ConvenioVO datosConvenio)
	{
		super(parametrosNegocio, session, datosConvenio);
	}

	public VN520(HashMap parametrosNegocio, Utils setter, Session session, ConvenioVO datosConvenio, List parametros, List listaConceptos)
	{
		super(parametrosNegocio, setter, session, datosConvenio, parametros, listaConceptos);
	}

	public VN520(HashMap parametrosNegocio, Utils setter, Session session, ConvenioVO datosConvenio, List parametros, List listaConceptos, List listaMapeosValidaciones)
	{
