...
public class VN480 extends Validacion
{
	private static Logger log = Logger.getLogger(VN480.class);

	/*
	 * 1 parametro = VN480: monto ahorro APV 3, requiere: 470
	 * 
	 * Mensajes 1712: Monto APV debe existir si existe Codigo de Entidad APV 1722: Monto APV invalido 1732: Monto APV debe ser menor que tope 2352: CODIGO ENTIDAD APV DEBE EXISTIR SI EXISTE MONTO APV
	 */
	public int valida(CotizanteVO cotizante)
	{
		try
		{
			...
			if (valor < 0)
			{
				if(this.logear) log.info("validacion VN480 ERR: Ahorro invalido:" + c.getValor() + "::");
			...
			{
				if(this.logear) log.info("validacion VN480 ERR: Ahorro APV mayor a cero:" + valor + ":pero no se informa entidad:");
			...
			if (valor > topeAPV)
			{
				if(this.logear) log.info("validacion VN480 ERR: Ahorro APV:" + valor + ": mayor a tope:" + topeAPV + "::");
			...
			{	
				if(this.logear) log.info("validacion VN480 OK con APV_INVALIDO-->SIN APV");
			...
			{
				if(this.logear) log.info("validacion VN480 ERR: Ahorro debe existir:" + c.getValor() + "::");
				return 1710 + cotizante.getApvList().size() - 1;//Monto APV debe existir si existe Codigo de Entidad APV
			}

			if (apv.getIdEntidadApv() == Constants.SIN_APV && valor == 0)
			{
				if(this.logear) log.info("validacion VN480 OK:");
				return this.COD_CUMPLE_VALIDACION;
			}

			if (this.logear)
				log.info("validacion VN480 OK: Monto APV3:" + valor + "::");
			return this.COD_CUMPLE_VALIDACION;
		} catch (Exception e)
		{
			log.error("error validacion VN480::", e);
	...
	public int validaFromWEB(CotizanteVO cotizante)
	{
		try
		{
			ApvVO apv = cotizante.getApv(2);
			if (apv != null)
			{
				if (apv.getAhorro() < 0)
				{
					if (this.logear)
						log.info("validacion VN480 ERROR Ahorro invalido:" + apv.getAhorro() + "::");
					return 172;// Monto APV invalido
				} else if (apv.getAhorro() == 0)
				{
					if (this.logear)
						log.info("validacion VN480 ERROR Ahorro debe existir:" + apv.getAhorro() + "::");
...
//				topeAPV *= new Float((String) this.parametrosNegocio.get("" + Constants.PARAM_UF_ACTUAL)).floatValue();
				...
				{
					if (this.logear)
						log.info("validacion VN480 ERR: Ahorro APV mayor a tope:" + topeAPV + "::");
	...
	public VN480(HashMap parametrosNegocio, Session session, ConvenioVO datosConvenio)
	{
		super(parametrosNegocio, session, datosConvenio);
	}

	public VN480(HashMap parametrosNegocio, Utils setter, Session session, ConvenioVO datosConvenio, List parametros, List listaConceptos)
	{
		super(parametrosNegocio, setter, session, datosConvenio, parametros, listaConceptos);
	}

	public VN480(HashMap parametrosNegocio, Utils setter, Session session, ConvenioVO datosConvenio, List parametros, List listaConceptos, List listaMapeosValidaciones)
	{
