 ...
 * Ventana - Preferencias - Java - Estilo de código - Plantillas de código
 */
public class GenerarArchivosImpresionAction extends Action {
	...
	public ActionForward execute(
		...
		try{
			...
			logger.fine("Entrando Action Generar Archivo ASICOM");
			...
			logger.info("Generar Archivos Impresión, Periodo= " + periodo + ", cierre=" + cierre);
			//Invocando una hebra para Generar Cheques
			if(!paramTO.getProcesosActivos().containsKey("ASICOM:" + cierre)){
				GenerarArchivosImpresionThreads asicomThreads= new GenerarArchivosImpresionThreads(Integer.parseInt(periodo), Integer.parseInt(cierre), formapago,  emails);
				paramTO.addProceso("ASICOM:" + cierre, Today.getFecha_Hora());
				paramTO.addThread("ASICOM:" + cierre, asicomThreads);
				asicomThreads.start();
		...
		System.out.println("Fin GenerarArchivosEntidadesAction, forward= "+ forward);
