...
public class Utils
{
	...
	private static Pattern ppto = Pattern.compile("[.]");
	...
	public int procesaTexto(boolean reemplazo, String txt)
	{
		if (txt != null)
		{
			...
			this.valor = ppto.matcher(this.valor).replaceAll(" ");
	...
	{
		//Se está limpiando la línea afuera.
		//line = corrigeDobleSeparador(mapeosConcep, line);
		...
		for (Iterator it2 = listaConceptos.iterator(); it2.hasNext();) // asigna valor a cada campo
		{
			...
			} else
			{
				...
				if (mc.getTipoSeparador() == Constants.TIPO_SEPARADOR_POSICION) {
					c.setValor(line.substring(mc.getPosicion() - 1, Math.min(mc.getPosicion() + mc.getLargo() - 1, line.length())).trim());
				//SEPARACIÓN POR CARACTER
				} else {
					String[] elementos = line.split(String.valueOf(mc.getCaracterSeparador()));
					c.setValor(elementos[ mc.getPosicion()-1 ]);
				}
	...
	public static HashMap parseoValores(List listaConceptos, HashMap mapeosConcep, String line, boolean isPrevired, boolean isProdCaja)	{
		HashMap mapeoValores = new HashMap();
		...
		for (Iterator it2 = listaConceptos.iterator(); it2.hasNext();) { // asigna valor a cada campo
			...
			if (mc == null || (mc.getPosicion() == 0 && mc.getLargo() == 0) || line.length() < mc.getPosicion()) {
				if(isProdCaja){
					...
					if (c.getId() == 15){
						c.setValor("30");
						mapeoValores.put(c.getNombre().trim(), "30");
					}
				}else{
					c.setValor("");
					mapeoValores.put(c.getNombre().trim(), "");
				...
				//SEPARACIÓN POR CARACTER
					...
					c.setValor(elementos[ mc.getPosicion()-1 ]);
					if (isPrevired) {
						if (c.getId() == Constants.CONCEPTO_MES) {
							// El Mes está con formato "MMAAAA", se dejan solo los dos primeros caracteres.
							//clillo 27/9/12 Se corrige obtención del mes, si no viene se define como '00' ya que no existe VN para período
							//String mes = c.getValor().substring(0, 2);
							String mes = c.getValor();
							if(mes.length()== 6){
							...
							c.setValor(mes);
						} else if (c.getId() == Constants.CONCEPTO_RUT) {
							// Se Concatena el DV al RUT. Se Asume que el DV está en la posición inmediatamente siguiente.
							c.setValor(c.getValor() + "-" + elementos[mc.getPosicion()]);
						} else if (c.getId() == Constants.CONCEPTO_RUT_DEP_AFPV) {
							// Se Concatena el DV al RUT. Se Asume que el DV está en la posición inmediatamente siguiente.
							if(!c.getValor().equals("0") && !c.getValor().equals(""))
								c.setValor(c.getValor() + "-" + elementos[mc.getPosicion()]);
							else
								c.setValor("");
						} else if (c.getId() == Constants.CONCEPTO_APELLIDOS) {
							// Se Concatena el Ap. Materno al Paterno. Se Asume que el Ap. Materno está en la posición inmediatamente siguiente.
							c.setValor(c.getValor() + " " + elementos[mc.getPosicion()]);
						} else if (c.getId() == Constants.CONCEPTO_APELLIDOS_DEP_AFPV) {
							// Se Concatena el Ap. Materno al Paterno. Se Asume que el Ap. Materno está en la posición inmediatamente siguiente.
							c.setValor(c.getValor() + " " + elementos[mc.getPosicion()]);
						} else if (c.getId() == Constants.CONCEPTO_ENTIDAD_EXCAJA) {
							// Se están informando Entidad Ex-Caja (EC) y Regimen Impositivo (RI) juntos con formato "ECRI" (4 caracteres).
							//clillo 27/9/12 Se corrige obtención de codExCaja
							//String codExCaja = c.getValor().substring(0, 2);
							String codExCaja = c.getValor();
							if(codExCaja.length()>2){
								codExCaja= codExCaja.substring(0, 2);
							}
							c.setValor(codExCaja);
						} else if (c.getId() == Constants.CONCEPTO_REGIMEN_IMPOSITIVO) {
							// Se están informando Entidad Ex-Caja (EC) y Regimen Impositivo (RI) juntos con formato "ECRI" (4 caracteres).
							// Se asume que Regimen Impositivo informa la posición cero.
							//clillo 27/9/12 Se corrige obtención de codExCaja
							//String codRegImp = c.getValor().substring(2, 4);
							String codRegImp = c.getValor();
							if(codRegImp.length()>=4){
								codRegImp= codRegImp.substring(2, 4);
							}
							c.setValor(codRegImp);
						}
					}
	 ...
	 * @return
	 */
	public static String corrigeDobleSeparador(HashMap mapeosConcep, String line) {

		MapeoConceptoVO mc = (MapeoConceptoVO) mapeosConcep.get("" + 2); //Concepto "RUT", es obligatorio por lo que todas las nóminas lo traen
	 ...
	 * @return
	 */
	public static String corrigeLineaConSeparador(HashMap mapeosConcep, String linea, int cantidadConceptos) {
		
		linea = corrigeDobleSeparador(mapeosConcep, linea);
		MapeoConceptoVO mc = (MapeoConceptoVO) mapeosConcep.get("" + Constants.CONCEPTO_RUT); //Es obligatorio por lo que debe estar definido. Solo importa rescatar el caracter y tipo de separador
		
		if (mc.getTipoSeparador() == Constants.TIPO_SEPARADOR_CARACTER && cantidadConceptos > 0) {
