...
public class SeccionVO extends AuditableVO
{
	private static Logger logger = Logger.getLogger(SeccionVO.class);
	private static final long serialVersionUID = -3361721910924672786L;
	...
	static private int diaFinPagoCaja;
	...
	public SeccionVO(int numTrabajadores, int tipoPago, int idTipoSeccion, int finPagoCaja)
	{
		super();
		setDiaFinPagoCaja(finPagoCaja);
		this.idTipoSeccion = idTipoSeccion;
		this.numTrabajadores = numTrabajadores;
		Calendar now = Calendar.getInstance();
		int dia= now.get(Calendar.DATE);
		if(dia > finPagoCaja && dia <=15 && (idTipoSeccion== 1 || idTipoSeccion==20 || idTipoSeccion== 40 || idTipoSeccion== 6)){
			this.tipoPago = Constants.EST_SECCION_INDEF;
		}else{
	...
	public SeccionVO(SeccionVO seccionVO, ComprobanteVO cmp, int finPagoCaja)
	{
		super();
		
		setDiaFinPagoCaja(finPagoCaja);
		this.idCodigoBarra = 0;
		this.idTipoSeccion = seccionVO.getIdTipoSeccion();
		this.mm1 = seccionVO.getMm1();
		this.mm2 = seccionVO.getMm2();
		this.mm3 = seccionVO.getMm3();
		this.mm4 = seccionVO.getMm4();
		this.mm5 = seccionVO.getMm5();
		this.mm6 = seccionVO.getMm6();
		this.mm7 = seccionVO.getMm7();
		this.mm8 = seccionVO.getMm8();
		this.mm9 = seccionVO.getMm9();
		this.mm10 = seccionVO.getMm10();
		this.mm11 = seccionVO.getMm11();
		this.mm12 = seccionVO.getMm12();
		this.numTrabajadores = seccionVO.getNumTrabajadores();
		
		Calendar now = Calendar.getInstance();
		int dia= now.get(Calendar.DATE);
		if(dia > finPagoCaja && dia <=15 && (seccionVO.getIdTipoSeccion()== 1 || seccionVO.getIdTipoSeccion()==20 || seccionVO.getIdTipoSeccion()== 40 || seccionVO.getIdTipoSeccion()== 6)){
			this.tipoPago = Constants.EST_SECCION_INDEF;
		}else{
			this.tipoPago = seccionVO.getTipoPago();
		}
		
		List detalles = new ArrayList();
		if (seccionVO.getDetallesSeccion() != null)
		{
			for (Iterator it = seccionVO.getDetallesSeccion().iterator(); it.hasNext();)
			{
				DetalleSeccionVO secc = new DetalleSeccionVO((DetalleSeccionVO) it.next(), this);
				detalles.add(secc);
			}
		}
		this.detallesSeccion = detalles;
		this.configPDF = new ConfigPDFVO(seccionVO.getConfigPDF());
	...
	public DetalleSeccionVO getDetalleSeccion(int idDetalleSeccion)
	{
		if (this.detallesSeccion != null)
			for (Iterator itDetSecc = this.detallesSeccion.iterator(); itDetSecc.hasNext();)
			{
				DetalleSeccionVO detalle = (DetalleSeccionVO) itDetSecc.next();
	...
	public void sumaTotalesCaja()
	{
		this.numTrabajadores = 0;
		this.m = new long[12];
		boolean conMoviminetos= false;
		if (this.detallesSeccion != null)
		{
			List detCeros = new ArrayList();
			for (Iterator it = this.detallesSeccion.iterator(); it.hasNext();)
	...
	public void setDetallesSeccion(int idDetalleSeccion, int numTrabajadores, double[] mTmp)
	{
		...
		{
			this.detallesSeccion = new ArrayList();
			this.detallesSeccion.add(new DetalleSeccionVO(this.idTipoSeccion, idDetalleSeccion, numTrabajadores, this.tipoPago, mTmp));
		} else
		{
			boolean flag = false;
			for (int i = 0; i < this.detallesSeccion.size(); i++)
			{
				DetalleSeccionVO det = (DetalleSeccionVO) this.detallesSeccion.get(i);
	...
	public String toString()
	{
		...
		/*
		 * if (this.detallesSeccion != null) for (int i = 0; i < this.detallesSeccion.size(); i++) { DetalleSeccionVO det = (DetalleSeccionVO) this.detallesSeccion.get(i); str += "\t" +
	...
	public boolean equals(Object obj)
	{
		...
		if (getClass() != obj.getClass())
			return false;
		final SeccionVO other = (SeccionVO) obj;
	...
	public long calculaTotal()
	{
		long suma = 0;
		refreshListaM();
		System.out.println("En calculaTotal, largo TOTAL_MX_SECCION=" + Constants.TOTAL_MX_SECCION.size());
	...
	/**
	 * @return el diaFinPagoCaja
	 */
	public static int getDiaFinPagoCaja() {
		return diaFinPagoCaja;
	}

	/**
	 * @param diaFinPagoCaja el diaFinPagoCaja a establecer
	 */
	public static void setDiaFinPagoCaja(int diaFinPagoCaja) {
		SeccionVO.diaFinPagoCaja = diaFinPagoCaja;
	}
}
