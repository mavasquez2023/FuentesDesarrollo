...
public class CotizacionREVO extends CotizacionVO
{
	...
	//Campos de Depósito Convenido
	private int depositoConvenido;
	private int tipoRegimenPrev; //(1) INP, (2) AFP, (0) Ninguno
	...
	public CotizacionREVO(CotizacionREVO otro)
	{
		...
		this.idEntDep = otro.getIdEntDep();
	...
	public boolean isReforma(Utils setter)
	{
		if (!setter.setString("rutDependiente").equals("") || !setter.setString("nombreDependiente").equals("") || !setter.setString("apellidoDep").equals("")){
			//17-05-2012 GMALLEA Se limpia el rut ya se esta mandando con 00000000
			//28-09-2012 clillo Se corrige código anterior ya que si rut viene vacío se cae
			String rutDependienteFull = setter.setString("rutDependiente").trim();
			//Se revisa si viene rut con guión para solo tomar parte numérica 
			int pos= rutDependienteFull.indexOf('-');
			if(pos>-1){
				rutDependienteFull= rutDependienteFull.substring(0, pos);
			}
			//se verifica si parte numérica es mayor a cero para determinar que dato es un rut
			try {
				if(Integer.parseInt(rutDependienteFull)>0){
					return true;
				}
			} catch (NumberFormatException e) {
				//dato no es un Rut y se retornará false
			}
			/*
			...
			int rutDependienteNuemric = Integer.parseInt(rutDependiente.trim());
	...
	public HashMap getParametrosOrdenados()
	{
		...
		parametros.put("52", String.valueOf(this.depositoConvenido));
		parametros.put("53", String.valueOf(this.tipoRegimenPrev));
		parametros.put("54", String.valueOf(this.tasaPactada));
		parametros.put("55", String.valueOf(this.indemAporte));
		parametros.put("56", String.valueOf(this.indemInicio));
		parametros.put("57", String.valueOf(this.indemTermino));
		parametros.put("58", String.valueOf(this.numPeriodos));
		parametros.put("59", String.valueOf(this.idEntDep));
	...
	public int getNumeroPeriodoAFP() {
		return numeroPeriodoAFP;
	}

	public void setNumeroPeriodoAFP(int numeroPeriodoAFP) {
		this.numeroPeriodoAFP = numeroPeriodoAFP;
	}
