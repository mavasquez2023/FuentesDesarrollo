...
function recalculaSalud()
{
	...
	{
		//document.getElementById('saludObligISAPRE').value = formatNumero(Math.round(montoReli * tasaSalud));
		document.getElementById('saludObligFONASA').value = '0';
		document.getElementById('aporteCaja').value       = '0';
		//document.getElementById('fonasaCheck').checked	= false;
	} if (document.getElementById('idEntSaludReal').value == idFONASA ||
	      document.getElementById('idEntSaludReal').value == sinEntidadSalud) //FONASA O SIN ENTIDAD
	{
		...
		//	document.getElementById('aporteCaja').value = formatNumero(Math.round(montoReli * aporteCaja));
		//}
		document.getElementById('saludObligISAPRE').value = '0';
		//if (document.getElementById('fonasaCheck').checked)
		//	document.getElementById('saludObligFONASA').value = formatNumero(Math.round(montoReli * tasaSalud));
	}
	calculaTotales('INP');
}
...
//DETALLES
	...
	var montoReli = creaNumero(document.getElementById('montoReli').value);
	if (montoReli < 0)
...
*/
	...
	if (tipoPrevision == "AFP")
	{
		if (tieneTP == 1)
		{
			...
			if(montoTrabPesado > 0 && document.getElementById('idTasaTraPesa').value != -1 && montoReli > 0)
			{
