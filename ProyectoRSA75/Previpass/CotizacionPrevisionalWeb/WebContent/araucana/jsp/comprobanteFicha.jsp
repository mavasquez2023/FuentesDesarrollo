	...
	<tr>
		...
		<table width="100%" border="0" cellpadding="1" cellspacing="5" class="textos-formularios3">
			<tr> 
          	...
          	<tr>
          		<td colspan="3">&nbsp;</td>
          		<td align="center"><html:submit property="operacion"  value="${filtro}" onclick="javascript:limpiar();" styleClass="btn-destacado"/></td>
         	</tr>
      	</table>
	                  		...
	                  		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" bordercolor="#CCCCCC">
								<c:if test="${seccion.nombre == 'C.C.A.F.'}">
									<c:if test="${seccion.sfe !=''}">
										<tr>
											<td height="19" colspan="3" align="right" valign="middle" class="barra_tablas">(Saldo Favor Empresa) $${seccion.sfe}</td>
										</tr>
									</c:if>
									<c:if test="${seccion.sfi !=''}">
										<tr>
											<td height="19" colspan="3" align="right" valign="middle" class="barra_tablas">(Saldo Favor Institución) $${seccion.sfi}</td>
										</tr>
									</c:if>
								</c:if>
    ...
    <logic:notPresent name="msg">
	    <tr> 
			<td height="33" valign="top" align="center">
		  		<input name="operacion" type="button" class="btn_grande" value="${imprimirResumen}" onclick="javascript:imprimirResumen();"/>
		  		<logic:equal name="DetalleActionForm" property="puedePagar" value="1">
			  		<c:if test="${bloqueoPagoCaja == 0}" >
				  		<input name="operacion" type="submit" class="btn_grande" value="${pagoCaja}" onclick="javascript:limpiar();"/>
				  	</c:if>
				  	<c:if test="${bloqueoPagoCaja == 1}" >
	           		...
	           		<c:if test="${bloqueoPagoLinea == 0}" >
			  			<input name="operacion" type="submit" class="btn_grande" onclick="validaPagoCero(); return false;" value="${pagoLinea}"/>
			  		</c:if>
