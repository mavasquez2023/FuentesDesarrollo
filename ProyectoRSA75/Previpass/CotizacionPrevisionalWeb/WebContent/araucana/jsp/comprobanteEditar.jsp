									...
									<logic:equal name="filaSeccion" property="pagableIndividual" value="true">
		                          		<logic:equal name="filaSeccion" property="dnp" value="true">
					                  		...
					                  		<td class="${estilo}" width="12%">
					                  			<nested:checkbox  title="Selecciona todos No Pago" property="chkDNP" value="3" onclick="javaScript:cambiaRadios(this)" styleClass="checkBox"/>
					                  		</td>
				                  		</logic:equal>
									...
									<logic:notEqual name="filaSeccion" property="pagableIndividual" value="true">
										...
										<logic:notEqual name="filaSeccion" property="mostrar3" value="true">
			                          		...
			                          		<logic:notEqual name="filaSeccion" property="dnp" value="true">
					            	      		...
					            	      		<td class="${estilo}" width="12%" ><div align="center">
					            	      			<c:choose>
					            	      				<c:when test="${filaSeccion.glosa!='OTROS PAGOS'}">
					                	  					<nested:radio property="tipoPago" value="3" onclick="javaScript:cambiaRadios(this)"  styleClass="radioButton"/>
					                	  				</c:when>
				            	      			...
				            	      			<tr id="DET${filaSeccion.idTipoSeccion}${idLineaDet}">
		            			                   	<td class="${estilo}" colspan="5">
			                        			       	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" bordercolor="#CCCCCC">
									                         ...
									                         <tr>
							            	      				<td>
							            	      					<c:if test="${filaSeccion.idTipoSeccion == 5}">
																	<c:if test="${filaSeccion.sfe !=''}">
																	<tr>
																		<td height="19" colspan="3" align="right" valign="middle" class="barra_tablas">(Saldo Favor Empresa) $${filaSeccion.sfe}</td>
																	</tr>
																	</c:if>
																	<c:if test="${filaSeccion.sfi !=''}">
																	<tr>
																		<td height="19" colspan="3" align="right" valign="middle" class="barra_tablas">(Saldo Favor Institución) $${filaSeccion.sfi}</td>
																	</tr>
																	</c:if>
																	</c:if>
																	<c:if test="${filaSeccion.idTipoSeccion == 3}">
																	<c:if test="${filaSeccion.sfe !=''}">
																	<tr>
																		<td height="19" colspan="3" align="right" valign="middle" class="barra_tablas">(Saldo Favor Empresa) $${filaSeccion.sfe}</td>
																	</tr>
		                  	...
		                  	<td align="center" valign="middle">
	                      		...
	                      		<nested:equal property="puedeGuardarOPagar" value="true">
		                      		<c:if test="${bloqueoEdicionNom == 0}" >
                      		    		<input type="button" class="btn4" onclick="javascript:isPagoComprobante(this);" value="${guardar}" name="operacion" id="botonGuaradar" disabled="disabled"/>
		                      		</c:if>
								...
								<c:if test="${bloqueoPagoCaja == 0}" >
									<html:submit property="operacion" value="Declarar No Pago" styleClass="btn4" styleId="botonDeclararNoPago"/>
	                      		</c:if>
	                      		<c:if test="${bloqueoPagoCaja == 1}" >
		                      		<html:button property="operacion" value="Declarar No Pago" styleClass="btn4" styleId="botonDeclararNoPago" onclick="alert('\n El plazo válido para declarar y no pagar ha finalizado')"/>
	                      		</c:if>
...
//Cambia el estado de los radio button de la misma seccion y columna que el checkBox chkBtn
function isPagoComprobante(chkBtn) {
	var isSelect="";
	var inputs = document.forms[0].getElementsByTagName("input");
	for (var i = 0; i < inputs.length; i++) {
        if ((inputs[i].type == undefined) || (inputs[i].type != "radio")){
	...
	if(isSelect == "ok"){
		...
		input.setAttribute("name", "operacion");
