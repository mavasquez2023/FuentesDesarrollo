...
public class ConvenioDAO
{
	...
	public List getGruposConveniosAdmin(int idPersona) throws DaoException 
	{
		try 
		{
			...
			List listaEmpresas = this.session.createCriteria(EmpresaVO.class).add(Restrictions.eq("idAdmin", new Long(idPersona))).add(Restrictions.eq("tipo", Constants.TIPO_EMPRESA)).list();
	...
	public List getAllConvenios(int idUsuario, Collection colEmps) throws DaoException
	{
		try
		{
			...
			List listEmpresaConvenio = this.session.createCriteria(EmpresaVO.class).add(Restrictions.in("idEmpresa",colEmps)).add(Restrictions.like("tipo", Constants.TIPO_EMPRESA)) .list();
			...
			if (encVO != null && encVO.getHabilitado() == Constants.COD_HABILITACION_ENCARGADO)
			{
				...
				if (!listaEmp.isEmpty())
				{
					...
					List listaEmps = this.session.createCriteria(EmpresaVO.class)
						...
						//TODO GMALLEA Se agrega la restriccion tipo para que retorne solo las empresas
						.add(Restrictions.like("tipo", Constants.TIPO_EMPRESA))
						.list();
	...
	public String errorCargaFamiliarOtraCaja(char tipoProceso, int rutEmpresa, int idConvenio, String idCotizPendiente) {
		...
		String textoError = "LA EMPRESA PRESENTA CARGA PARA OTRA CAJA";
		...
		try {
			...
			String query = "SELECT c.texto" +
							" FROM " + tipo + " AS c" + 
						   " WHERE c.idTipoCausa      = " + Constants.ERROR_CARGAFAM_EN_OTRA_CAJA + 
							 " AND c.rutEmpresa       = " + rutEmpresa +
							 " AND c.idConvenio       = " + idConvenio +
							 " AND c.idCotizPendiente = " + idCotizPendiente;
			Query q = this.session.createQuery(query).setMaxResults(1);
			List listado = q.list();

			if ( listado.size() == 0 ) {

				//Si no fue un AVISO, buscar si está en una nómina con ERROR
				if (tipoProceso == 'R')
					tipo = "CausaREVO";
	  			if (tipoProceso == 'G')
					tipo = "CausaGRVO";
				...
				q = this.session.createQuery(query).setMaxResults(1);
				listado = q.list();				
			...
			for (int i=0 ; i < listado.size() ; i++) {
				textoError = String.valueOf(listado.get(i)).trim();
			}
			
			return textoError;
		} catch (Exception e) {
			logger.error("Error en ConvenioDAO:errorCargaFamiliarOtraCaja", e);
			return textoError;//throw new DaoException("ConvenioDAO:errorCargaFamiliarOtraCaja:", e);
		}
	}
}