...
public class EmpresaDAO
{
	...
	public EmpresaVO getEmpresa(int rutEmpresa) throws DaoException
	{
		...
		} catch (Exception ex)
		{
			...
			throw new DaoException("Empresa "+ rutEmpresa +" no existe en el sistema.", ex);
		}
	}
	 ...
	 * @author clillo
	 * 
	 * obtiene sucrsal matriz empresa
	 * 
	 * @param idEmpresa
	 * @param idConvenio
	 * @return int
	 * @throws DaoException
	 */
	public String getSucursalMatrizEmpresa(int idEmpresa)throws DaoException{
		try{
			StringBuffer hqlQuery = new StringBuffer("SELECT idCasaMatriz FROM "+EmpresaVO.class.getName()+" WHERE idEmpresa= "+idEmpresa);

			Query query = this.session.createQuery(hqlQuery.toString());

			String sucursal = (String)query.uniqueResult();

			return sucursal;
		}catch (Exception ex)
		{
			logger.error("Error en EmpresaDAO:getSucursalMatrizEmpresa", ex);
			ex.printStackTrace();
			throw new DaoException("EmpresaDAO:getSucursalMatrizEmpresa idEmpresa: " + idEmpresa , ex);
		}
	}
	 ...
	 * @throws DaoException
	 */
	public List getListaConceptos(int idGrupoConvenio) throws DaoException
	{
		try
		{
			logger.info("EmpresaDAO:getListaConceptos:idGrupoConvenio" + idGrupoConvenio  + "::");
			StringBuffer sb = new StringBuffer();
			sb.append("SELECT a.idConcepto, a.posicion, c.nombre, c.largoMax ");
			sb.append("FROM " +  MapeoConceptoVO.class.getName() + " as a, " +  GrupoConvenioVO.class.getName() + " as b, " +  ConceptoVO.class.getName() + " as c ");
			sb.append("WHERE a.idMapa= b.idMapaNomRem ");
			sb.append("AND a.idConcepto=c.id " );
			sb.append("AND a.tipoProceso= c.tipoProceso ");
			sb.append("AND b.idGrupoConvenio= ? ");
		...
		{
			logger.error("Error en EmpresaDAO:getListaConceptos", ex);
			ex.printStackTrace();
			throw new DaoException("EmpresaDAO:getListaConceptos idGrupoConvenio: " + idGrupoConvenio , ex);
		}
	}
	...
	public List getEmpresasPermisos(int idPersona) throws DaoException
	{
		try
		{
			...
			//TODO GMALLEA Se agrega la restriccion tipo para que retorne solo las empresas
			...
			if (adminVO != null && adminVO.getHabilitado() == Constants.COD_HABILITACION_ADMIN)// La persona es administrador, y esta habilitado
					conjuntoEmpresas.addAll(this.session.createCriteria(EmpresaVO.class).add(Restrictions.eq("idAdmin", new Long(idPersona))).add(
						Restrictions.eq("habilitada", new Integer(Constants.COD_HABILITACION_EMPRESA))).add(Restrictions.like("tipo", Constants.TIPO_EMPRESA)).list());
