...
public class HibernateUtil
{
	...
	private static String archivoConfig = "cl/araucana/cp/hibernate/beans/hibernate.cfg.xml";
	...
	static
	{
		configsMaps.put("SPL", "cl/araucana/cp/hibernate/beans/SPLhibernate.cfg.xml");
		configsMaps.put("TES", "cl/araucana/cp/hibernate/beans/TEShibernate.cfg.xml");
		configsMaps.put("TESCRONTAB", "cl/araucana/cp/jcrontab/TEShibernate.cfg.xml");
		configsMaps.put("CPCRONTAB", "cl/araucana/cp/jcrontab/hibernate.cfg.xml");
	}
	...
	public static Session getSession(String key) throws InfrastructureException
	{
		...
		try
		{
			...
			// Habilita el filtro para habilitacion (solo conexion DB2 CPE)
			if (key.equals("") || key.equals("TES"))
			{
				s.enableFilter("filtroHabilitacion");
				s.enableFilter("filtraHabilitacion");
			}
	...
	public static void disableInterceptor(String key)
	{
		try
		{
			log.info("\n\ninterceptor:" + configuration.getInterceptor().getClass().getName() + "::");
			Interceptor inter = ((Configuration)configMap.get(key)).getInterceptor();
			if (inter instanceof MiscInterceptor)
				((MiscInterceptor) inter).disable();
		} catch (Exception e)
		{}
	}

	public static void enableInterceptor(String key)
	{
		try
		{
			log.info("\n\ninterceptor:" + configuration.getInterceptor().getClass().getName() + "::");
