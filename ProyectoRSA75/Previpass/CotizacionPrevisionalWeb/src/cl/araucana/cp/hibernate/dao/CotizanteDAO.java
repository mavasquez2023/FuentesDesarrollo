...
//	clillo 3-12-14 por Reliquidación
	...
	public CotizanteVO getCotizante(int idEmpresa, int idConvenio, char tipoProceso, int rutTrabajador, int periodo)
	{
		...
		{
			int borrar=0;
			log.debug("CotizanteDAO:getCotizante:idEmpresa" + idEmpresa + ":idConvenio:" + idConvenio + ":tipoProceso:" + tipoProceso + ":rutTrabajador:" + rutTrabajador + ":periodo" + periodo + "::");
//			clillo 3-12-14 por Reliquidación
			//CotizanteVO cotizante = (CotizanteVO) this.session.get(CotizanteVO.class, new CotizanteVO(idEmpresa, idConvenio, rutTrabajador));
			CotizanteVO cotizante = (CotizanteVO) this.session.get(CotizanteVO.class, new CotizanteVO(idEmpresa, idConvenio, rutTrabajador, periodo));
			if (cotizante != null)
			{
...
//				clillo 3-12-14 por Reliquidación
				if (tipoProceso == 'A'){
					...
					argTypes[3]= Integer.class;

					argValues= new Object[4];
					argValues[0]= new Integer(idEmpresa);
					argValues[1]= new Integer(idConvenio);
					argValues[2]= new Integer(rutTrabajador);
					argValues[3]= new Integer(periodo);
				}
				//fin cambio
...
//	clillo 3-12-14 por Reliquidación
	...
	public CotizanteVO getCotizante(int idEmpresa, int idConvenio, int rutTrabajador, int periodo) throws DaoException
	{
		try
		{
...
//			clillo 12-5-14 por Reliquidación
			...
			return (CotizanteVO) this.session.get(CotizanteVO.class, new CotizanteVO(idEmpresa, idConvenio, rutTrabajador, periodo));
		} catch (Exception ex)
		{
			log.error("Error en CotizanteDAO:getCotizante", ex);
			throw new DaoException("CotizanteDAO:getCotizante idEmpresa: " + idEmpresa + ", idConvenio: " + idConvenio + ":rutTrabajador:" + rutTrabajador + ":periodo:" + periodo, ex);
		}
	}
	...
	public List getCotizantesNomina(int idEmpresa, int idConvenio, char tipoProceso) throws DaoException
	{
		try
		{
			...
			for (Iterator it = cotizaciones.iterator(); it.hasNext();)
			{
				...
				cotizante = (CotizanteVO) this.session.get(CotizanteVO.class, new CotizanteVO(idEmpresa, idConvenio, cotizacion.getIdCotizante(), cotizacion.getPeriodo()));
				if (cotizante != null)
				{
					cotizante.setCotizacion(cotizacion);
	...
	public List getListaCotizantes(int regIni, int cantidad, int idEmpresa, int idConvenio, char tipoProceso) throws DaoException
	{
		try
		{
			...
			sb.append("select cot from " + CotizanteVO.class.getName() + " as cot ");
			sb.append("WHERE cot.rutEmpresa = ? AND cot.idConvenio = ? AND cot." + CotizanteVO.getTipoTiene(tipoProceso) + " > 0 ");
			sb.append("AND 0 = (SELECT count(aviso.idCotizPendiente) FROM " + CotizacionVO.getTipoCausaAviso("" + tipoProceso).getName() + " as aviso ");
			sb.append( " WHERE cot.rutEmpresa = aviso.rutEmpresa " );
			sb.append(" AND cot.idConvenio = aviso.idConvenio ");
			sb.append(" AND cot.idCotizante = aviso.idCotizPendiente ");
			//clillo 5/12/14 por Reliquidación
			if(tipoProceso=='A'){
				sb.append(" AND cot.periodo= aviso.periodo ");
			}
			sb.append(")");
...
//				clillo 3-12-14 por Reliquidación
				...
				cotPend = new CotizacionPendienteRAVO(idEmpresa, idConvenio, getCotizPend, periodo);
			else
...
//			clillo 3-12-14 por Reliquidación
			if (tipoProceso == 'A'){
				crit.add(Restrictions.eq("periodo", new Integer(cotPend.getPeriodo())));
			}
			//fin
...
//			clillo 3-12-14 por Reliquidación
			if (tipoProceso == 'A'){
				crit.add(Restrictions.eq("periodo", new Integer(periodo)));
			}
			//fin
			List result = crit.list();
			if (result != null)
...
//	clillo 3-12-14 por Reliquidación
	...
	public List getCausasAvisosCargas(char tipoProceso, int idEmpresa, int idConvenio, int idCotizante, int periodo) throws DaoException
	{
		try
		{
			...
			crit.add(Restrictions.eq("idConvenio", new Integer(idConvenio)));
			crit.add(Restrictions.eq("idCotizPendiente", new Integer(idCotizante)));
			crit.add(Restrictions.in("idTipoCausa", Constants.AVISOS_CARGASFAM));
//			clillo 12-5-14 por Reliquidación
			if (tipoProceso == 'A'){
				crit.add(Restrictions.eq("periodo", new Integer(periodo)));
			}
			...
			log.info("\nCotizanteDAO:getCausasAvisosCargas:encontrados n avisos:" + lista.size() + "::");
			return lista;
		} catch (Exception ex)
		{
			log.error("Error en CotizanteDAO:getCausasAvisosCargas", ex);
			throw new DaoException("CotizanteDAO:getCausasAvisos:", ex);
		}
	}	
...
//	clillo 3-12-14 por Reliquidación
	...
	public List getCausasAvisosPendientes(char tipoProceso, int idEmpresa, int idConvenio, int idCotizante, int periodo) throws DaoException {	
		try {
			...
			log.info("\nCotizanteDAO:getCausasAvisosPendientes::" + idEmpresa + "::" + idConvenio + "::" + idCotizante + "::" + periodo + "::");
			Class tipo = null;
			if (tipoProceso == 'R')
				tipo = CausaREVO.class;
			if (tipoProceso == 'G')
	...
	public int getNumAvisos(int rutEmpresa, int convenio, char tipoProceso, String filtro) throws DaoException
	{
		...
		{
			StringBuffer sb = new StringBuffer();
			sb.append("select count(cot.idCotizante) from " + CotizanteVO.class.getName() + " as cot ");
			sb.append("WHERE cot.rutEmpresa = ? AND cot.idConvenio = ? AND cot." + CotizanteVO.getTipoTiene(tipoProceso) + " > 0 ");
			sb.append("AND 0 < (SELECT count(aviso.idCotizPendiente) FROM " + CotizacionVO.getTipoCausaAviso("" + tipoProceso).getName() + " as aviso ");
			sb.append( " WHERE cot.rutEmpresa = aviso.rutEmpresa " );
			sb.append(" AND cot.idConvenio = aviso.idConvenio ");
	...
	public int getNumAprobadosAll(int idUser, HashMap filtros) throws DaoException
	{
		try
		{
			...
			sb.append("AND (cot.rutEmpresa in (SELECT emp.idEmpresa FROM cl.araucana.cp.distribuidor.hibernate.beans.EmpresaVO as emp WHERE emp.idAdmin = ? AND emp.tipo = '"+Constants.TIPO_EMPRESA+"') "
	...
	public int getNumAvisosAll(int idUser, HashMap filtros) throws DaoException
	{
		try
		{
			...
			sb.append("(cot.rutEmpresa in (SELECT emp.idEmpresa FROM cl.araucana.cp.distribuidor.hibernate.beans.EmpresaVO as emp WHERE emp.idAdmin = ? AND emp.tipo = '"+Constants.TIPO_EMPRESA+"') "
					+ "	    OR 0 < (SELECT count(perm.idEncargado) FROM cl.araucana.cp.distribuidor.hibernate.beans.EncargadoConvenioVO as perm WHERE "
					+ "					cot.rutEmpresa = perm.idEmpresa AND cot.idConvenio = perm.idConvenio AND perm.idEncargado = ?)) ");
	...
	public int getNumPendientesAll(int idUser, HashMap filtros, List tiposNomina) throws DaoException
	{
		try
		{
			...
			for (Iterator it = tiposNomina.iterator(); it.hasNext();)
			{
				...
				sb.append("(pend.rutEmpresa in (SELECT emp.idEmpresa FROM cl.araucana.cp.distribuidor.hibernate.beans.EmpresaVO as emp WHERE emp.idAdmin = ? AND tipo = '"+Constants.TIPO_EMPRESA+"') "
						+ "	    OR 0 < (SELECT count(perm.idEncargado) FROM cl.araucana.cp.distribuidor.hibernate.beans.EncargadoConvenioVO as perm WHERE "
	...
	public List getCausasVO(char tipoProceso, CotizacionPendienteVO cotPend) throws DaoException {
		try {
			...
			crit = this.session.createCriteria(tipoAviso).add(Restrictions.eq("rutEmpresa", new Integer(cotPend.getRutEmpresa())));
			//clillo 23-12-14 Se modifica citeria para poder traer los mensajes de Avisos
			//crit.add(Restrictions.eq("idConvenio", new Integer(cotPend.getIdConvenio()))).add(Restrictions.eq("idCotizPendiente", new Integer(cotPend.getIdCotizPendiente())));
			crit.add(Restrictions.eq("idConvenio", new Integer(cotPend.getIdConvenio()))).add(Restrictions.eq("idCotizPendiente", new Integer(cotPend.getIdCotizante())));
			...
			throw new DaoException("CotizanteDAO:getCausasVO:", ex);
		}
	}
	 ...
	 * @throws DaoException
	 */
	public int totalCotizante(int idEmpresa, int idConvenio){
		
		StringBuffer hqlQuery = new StringBuffer("SELECT COUNT(rutEmpresa) FROM "+CotizanteVO.class.getName()+" WHERE rutEmpresa= "+idEmpresa+" AND idConvenio= "+idConvenio);
		
		Query query = this.session.createQuery(hqlQuery.toString());
		
		int totalCotizante = ((Integer)query.uniqueResult()).intValue();
		
		return totalCotizante;
	}
}