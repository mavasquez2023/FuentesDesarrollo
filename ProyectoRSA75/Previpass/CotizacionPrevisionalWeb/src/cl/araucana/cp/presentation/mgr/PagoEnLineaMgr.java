...
// SECURITY_SUPPORT
	...
	public PagoEnLinea preparaPago(String pagador, List listaCodBarras, Session session)
	{
		...
		try
		{
			...
			listaParams.add(new Integer(Constants.PARAM_URL_RETORNO_SPL));
			listaParams.add(new Integer(Constants.PARAM_URL_NOTIFICACION_SPL));
			
			ParametrosHash paramHash = this.parametrosDao.getParametrosHash(listaParams);
			String urlRetorno = paramHash.get("" + Constants.PARAM_URL_RETORNO_SPL);
			String urlNotificacion = paramHash.get("" + Constants.PARAM_URL_NOTIFICACION_SPL);
