...
public class ServletNotificacion extends HttpServlet 
{
	...
	static Logger log = Logger.getLogger(ServletNotificacion.class);
	...
	{
		log.info("ServletNotificacion: entrando a GET");
 ...
 */
			...
			if (pago != null && pago.getPagado() == 1)
			{
				...
				//clillo 2/12/14 Se comenta update LibroBanco y Curse en Tesorería a petición de Marco.
		...
		{
			e.printStackTrace();
			log.error("NotificacionSPLAction: ERROR al recibir notificacion:", e);
			if (tx != null)
				tx.rollback();
			if (txSPL != null)
				txSPL.rollback();
