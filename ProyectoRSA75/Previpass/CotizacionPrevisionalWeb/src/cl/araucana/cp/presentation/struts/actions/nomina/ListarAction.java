...
public class ListarAction extends AppAction
{
	...
	protected ActionForward doTask(User user, ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		try
		{
		    ...
		    request.getSession().setAttribute("tipoNomina", tipoNomina);
			/*
			...
			for (Iterator it = conveniosLectura.iterator(); it.hasNext();)
			{
				...
				//TODO GMALLEA validacion si es empres muestra la nomina...
				if (nomina == null || (idEstado != 0 && nomina.getIdEstado() != idEstado || !nomina.getEmpresa().getTipo().equals(Constants.TIPO_EMPRESA)))
					continue;
				nominas.add(nomina);
			...
			if (nominas.size() > 0)
			{
				...
				for (Iterator it = nominas.iterator(); it.hasNext();)
				{
					...
					if (mostrarTotal && nom.getIdCodigoBarras() > 0 && (nom.getIdEstado() == Constants.EST_NOM_PAGADO 			   ||
																		nom.getIdEstado() == Constants.EST_NOM_PAGADO_PARCIALMENTE ||
																		nom.getIdEstado() == Constants.EST_NOM_POR_PAGAR           ||
