...
public class ReceptorMgr
{
	...
	private void unzip(byte[] zippedData, int idEnviador) throws DaoException, Exception
	{
		...
		try
		{
			...
			while ((zipEntry = zipInput.getNextEntry()) != null)
			{
				...
				name = decodeZipEntryName(name);//formato: rutEmpresa+dv.T00
				...
				try
				{	
					int largorut= tokens[0].length();
					//Como rut viene con DV se separan para validar
					int rut = Integer.parseInt(tokens[0].substring(0, largorut-1));					
					dvRutEmpresa= tokens[0].substring(largorut-1).charAt(0);
					rutEmpresa = rut;
					
					//se calcula DV para comprar con el informado
					char dv_calculado= Utils.generaDV(rut);
					
					
					if (this.empresaDao.getEmpresa(rutEmpresa) == null || dvRutEmpresa!= dv_calculado )//se verifica que la empresa exista
					{
						zipInput.closeEntry();
				...
				try
				{
					...
					//Validación de RUT del Cotizante mal mapeado
					if (isRutMalMapeado(data, nombreNomina, rutEmpresa)) // Validación de Mapeo del RUT
					{
						this.receiptReport.addRejectedFile(idNomina, nombreNomina, (String) Constants.ESTADO_RECHAZO.get("" + Constants.EST_RECH_RUT_MAL_MAPEADO));
						guardaDescriptor(nombreNomina, rutEmpresa, Constants.EST_RECH_RUT_MAL_MAPEADO, (int) zipEntry.getSize(), (int) zipEntry.getCompressedSize());
						continue;
					}
	...
	public boolean isNominaPagada(NominaVO nomina)
	{
		...
		if (estadoPago == Constants.EST_NOM_PAGADO				||
			estadoPago == Constants.EST_NOM_PAGADO_PARCIALMENTE ||
			estadoPago == Constants.EST_NOM_PRECURSADA			||
			estadoPago == Constants.EST_NOM_PREPAGADA)
			return true;
		return false;
	}
	...
	private boolean isPeriodoIncorrecto(byte[] data, String nombreNomina, int rutEmpresa) throws Exception{
		...
		try {
			ParametroVO parametroVO = this.parametroMgr.getParametro(Constants.PARAM_PERIODO);
			...
			parametroVO = this.parametroMgr.getParametro(Constants.PARAM_CANT_LINEAS);
			int cantLineas = 10;
			try {
				cantLineas = Integer.parseInt(parametroVO.getValor().trim());
			} catch (Exception e) {}
			...
			//TODO GMALLEA 02/05/2012 Se agrega la validacion para que en las nominas de tipo (R) verifique el periodo, si es distinto no valida....
			if (tipoNomina.equals("R") || tipoNomina.equals("r")){
				//Busca periodo incorrecto solo en la primera línea
				while ((line = br.readLine()) != null && cantLineas > 0) {
					...
					//SEPARACIÓN POR CARACTER
						...
						//Si Grupo Convenio es con Separador y Formato Previred, el Mes viene dado como "MMAAAA"
						if (grupoConvenioVO.isPrevired())
							mesPeriodoString = ""+periodoInformado;
					}
	...
	/**
	 * Determina si el RUT del cotizante está mal mapeado.
	 * 
	 * @param data
	 * @param nombreNomina
	 * @param rutEmpresa
	 ...
	 * @throws Exception
	 */
	private boolean isRutMalMapeado(byte[] data, String nombreNomina, int rutEmpresa) throws Exception{
		...
		boolean rutMalMapeado = false;
		try {
			
			ParametroVO parametroVO = this.parametroMgr.getParametro(Constants.PARAM_CANT_LINEAS);
			...
			MapeoConceptoVO mapeoConceptoVO = this.mapeosMgr.getMapeoConcepto(tipoNomina, idMapaNom, Constants.CONCEPTO_RUT);

			//Busca RUT mal mapeado solo en las primeras 10 líneas
			while ((line = br.readLine()) != null && cantLineas > 0) {
				...
				//SEPARACIÓN POR POSICIÓN
				if (mapeoConceptoVO.getTipoSeparador() == Constants.TIPO_SEPARADOR_POSICION) {
					rut = line.substring(mapeoConceptoVO.getPosicion() - 1, Math.min(mapeoConceptoVO.getPosicion() + mapeoConceptoVO.getLargo() - 1, line.length())).trim();
				//SEPARACIÓN POR CARACTER
				} else {
					String[] elementos = line.split(String.valueOf(mapeoConceptoVO.getCaracterSeparador()));
					rut = elementos[ mapeoConceptoVO.getPosicion()-1 ];
				}

				// En caso de que el RUT hubiese tenido guión o espacio en blanco
				String rutSinBlancos = "";
				for (int x=0; x < rut.length(); x++) {
					if (rut.charAt(x) != ' ' && rut.charAt(x) != '-')
						rutSinBlancos += rut.charAt(x);
				}

				rut = (!rutSinBlancos.equals("") ? rutSinBlancos.substring(0, rutSinBlancos.length() - 1) : "");
			...
			bais.close();
			br.close();
			return rutMalMapeado;
		} catch (Exception e) {
			logger.error("problemas al obtener el rut", e);
			return false;
		}
	}
