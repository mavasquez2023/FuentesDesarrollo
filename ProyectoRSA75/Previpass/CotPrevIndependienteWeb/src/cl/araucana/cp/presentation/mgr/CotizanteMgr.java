...
public class CotizanteMgr
{
	...
	{
		return this.cotizanteDao.getCotizante(new Integer(idEmpresa).intValue(), new Integer(idConvenio).intValue(), new Integer(rutTrabajador).intValue());
	}
	...
	public HashMap getNivelErrorTipoCausa(List causas) throws DaoException
	{
		...
		for (Iterator it = causas.iterator(); it.hasNext();)
		{
			...
			if (tiposCausa.containsKey(idCausa))
			{
				...
				if(tc.getId() != 3492 && tc.getId() != 338){//Si es id 3492 o 338 Se omite el error ya que es de tipo independiente
	...
	public List guardaCotizante(char tipoProceso, int idCotizPend, String idPersona, String oldRut, CotizanteVO cotizante, HashMap causaAvisos) throws Exception
	{
		...
		try {
			//jlandero. Se respalda el folio y total del comprobante de pago. Previo a cualquier modificación de éste.
	...
	{
		return this.cotizanteDao.getCausasAvisosPendientes(tipoProceso, idEmpresa, idConvenio, idCotizante);
	}
