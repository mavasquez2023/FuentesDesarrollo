...
public class ComprobanteMgr
{
	 ...
	 * @throws Exception
	 */
	public long guardaListaEdicionComprobantes(long codigoBarra, List listaSecciones) throws DaoException, Exception
	{
		...
		return this.comprobanteDao.guardaNuevoComprobante(comprobante);
	}
	...
	{	
		//clillo 13-10-2017
		ComprobanteVO c= this.comprobanteDao.getComprobante(idCodigoBarra);
		c.setDiaFinPagoCaja(30);
		ComprobanteVO cmp = new ComprobanteVO(c);
		cmp.setDiaFinPagoCaja(30);
...
//				pago.setPagador("" + empresa.getIdEmpresa());
				...
				for (Iterator it2 = mapeoTesoreria.iterator(); it2.hasNext();)
				{
					...
					//INICIO GMALLEA 
						if(mapeo.getAplicaPara().trim().equals(Constants.CONCEPTO_TESORERIA_APLICA_PARA_IND)){
							
							this.log.info("concepto Independiente:" + mapeo.getIdConceptoTesoreriaInd() + "::");
							...
							this.log.info("obteniendo monto:" + monto + ":idMonto:" + mapeo.getIdMontoDetSeccion() + ":tipoSeccion:" + mapeo.getIdTipoSeccion() + ":idConcepto:"
									+ mapeo.getIdConceptoTesoreriaInd() + "::");
							if (totalCodigo.containsKey("" + mapeo.getIdConceptoTesoreriaInd()))
								monto += ((Long) totalCodigo.get("" + mapeo.getIdConceptoTesoreriaInd())).longValue();
							totalCodigo.put("" + mapeo.getIdConceptoTesoreriaInd(), new Long(monto));
							...
							this.log.info("concepto Ambos:" + mapeo.getIdConceptoTesoreriaInd() + "::");
							long montoInd = comprobante.getMonto(mapeo.getIdTipoSeccion(), mapeo.getIdDetalleSeccion(), mapeo.getIdMontoDetSeccion());
							this.log.info("obteniendo monto:" + montoInd + ":idMonto:" + mapeo.getIdMontoDetSeccion() + ":tipoSeccion:" + mapeo.getIdTipoSeccion() + ":idConcepto:"
									+ mapeo.getIdConceptoTesoreriaInd() + "::");
							if (totalCodigo.containsKey("" + mapeo.getIdConceptoTesoreriaInd()))
								montoInd += ((Long) totalCodigo.get("" + mapeo.getIdConceptoTesoreriaInd())).longValue();
							totalCodigo.put("" + mapeo.getIdConceptoTesoreriaInd(), new Long(montoInd));
							this.log.info("\ttotal seccion:" + mapeo.getIdTipoSeccion() + ":montoAcumulado por codigo tes:" + montoInd + ":tct.getIdMonto():" + mapeo.getIdMontoDetSeccion() + "::");
						}
				}
				this.log.info("\n\nmontos por concepto:");
	 ...
	 * @throws Exception
	 */
	public String generarArchivos(List listaIds, String tipoAdmin, String operacion) throws Exception
	{
		...
		String fechaLimiteCaja = paramHash.get("" + Constants.PARAM_FIN_PAGO_CAJA_IND);
		...
		for (Iterator it = listaComprobantes.iterator(); it.hasNext();)
		{
			...
			if (listaComprobantes.size() > 1 && "".equals(nombreArchivo))
				nombreArchivo = Constants.RUTA_CMPS + "Comprobante_" + tipoProceso + "_" + paramHash.get("" + Constants.PARAM_PERIODO_INDEPENDIENTE) + ".pdf";
			else if ("".equals(nombreArchivo))
				nombreArchivo = Constants.RUTA_CMPS + "Comprobante" + Utils.formatRut(rutEmpresa) + "_" + tipoProceso + "_"
						+ (idConvenio < 10 ? "0" : "") + idConvenio + "_" + paramHash.get("" + Constants.PARAM_PERIODO_INDEPENDIENTE) + ".pdf";
		...
		boolean result = constructor.createPDFtoPago(true, nombreArchivo, listaComprobantes, listaEmpresas, listaNominas, listaRepresentantes, listaConvenios, tipoAdmin, listadetalleCajaInforme);
	...
	public boolean cargaOnDemand(List listaComprobantes)
	{
		try
		{
			...
			for (Iterator it = listaComprobantes.iterator(); it.hasNext();)
			{
				...
				indices.add(paramHash.get("" + Constants.PARAM_PERIODO_INDEPENDIENTE) + "/" + convenio.getIdGrupoConvenio() + "/" + empresa.getIdEmpresa() + "/" + (empresa.getRazonSocial().trim().length() < 25 ? empresa.getRazonSocial().trim() :  empresa.getRazonSocial().trim().substring(0, 25))+ "/" + idConvenio + "/"
