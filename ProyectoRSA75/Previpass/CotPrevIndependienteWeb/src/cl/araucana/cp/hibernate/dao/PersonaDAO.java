...
public class PersonaDAO
{
	...
	public List getListaEncargados(Collection colEmpAdmins, HashMap filtros) throws DaoException
	{
		...
		try
		{
			...
			if (idsEncargados.size() == 0)
				return new ArrayList();
//			//TODO GMALLEA  Se agrega la resticcion independiente para que retorne los encargados de los independientes.
			List listEncargado = this.session.createCriteria(EncargadoVO.class).add(Restrictions.in("idEncargado", idsEncargados)).add(Restrictions.like("independiente", new Integer(1))).list();
	...
	public void borraPermEncargado(int idEncargado,int idAdmin) throws DaoException
	{
		try
		{
			...
			query.setString(1, Constants.TIPO_EMPRESA_INDEPENDIENTE);
	...
	public List getListaEmpresas(Collection col, HashMap filtros) throws DaoException
	{
		...
		try
		{
			...
			//TODO GMALLEA 10-08-2012 Se comenta ya que los independiente siempre es de tipo proceso Remuneracion
	...
	public List getListaEmpresasIn(Collection col) throws DaoException
	{
		try
		{
			...
			return this.session.createCriteria(EmpresaVO.class).add(Restrictions.in("idEmpresa", col)).addOrder(Order.asc("idEmpresa")).add(Restrictions.like("tipo", Constants.TIPO_EMPRESA_INDEPENDIENTE)).list();
		} catch (Exception ex)
		{
	...
	public boolean isAdminEmpresa(String idPersona) throws DaoException
	{
		try
		{
			...
			if (lista.size() > 0)	
			{//TODO GMALLEA SI ES INDEPENDIENTE NO ES NECESARIO QUE TENGA EMPRESAS ASOCIADAS
	...
	public List getListTipoPersona(String rut) throws DaoException
	{
		...
		try
		{
			 ...
			 listTipoPesona = this.session.createCriteria(AdministradorVO.class).add(Restrictions.eq("idAdmin", new Integer(rut))).add(
					Restrictions.eq("independiente", new Integer(1))).list();
			 ...
			 if(listTipoPesona.size() == 0){
				 listTipoPesona= this.session.createCriteria(EncargadoVO.class).add(Restrictions.eq("idEncargado", new Integer(rut)))
					 																.add(Restrictions.eq("independiente",  new Integer(1))).list();
			}
