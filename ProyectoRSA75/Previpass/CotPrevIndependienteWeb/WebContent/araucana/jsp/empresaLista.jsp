...
<table width="590" border="0" cellspacing="0" cellpadding="0">
	<tr valign="bottom"> 
    	<td width="38%" height="30" bgcolor="#FFFFFF" class="titulo">
			<strong>B&uacute;squeda de Independientes</strong>
		</td>
	</tr>
	<tr>
		<td>
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tabla-datos">
				...
				<tr>
			    	<td style="display:none"><strong>Grupo de Convenios:</strong></td>
				    <td style="display:none">
						<html:select property="opcGrupoConvenio" styleClass="campos" >
								<html:optionsCollection property="gruposConvenio" />
						</html:select>
					</td>
					<td style="display:none"><strong>Tipo Proceso: </strong></td>
					<td style="display:none">
            	...
            	<tr valign="bottom"> 
              		...
              		<td width="31%" align="right" bgcolor="#FFFFFF">
						...
						<nested:equal property="esAdminEmpresa" value="true">
						  &nbsp;<html:button property="operacion" styleClass="btn6" value="Crear Independiente" onclick="javascript:creaEmpresa(this);"/>
						</nested:equal>
	              	</td>
				</tr>
	         </table>
		                                ...
		                                <td align="center" width="5%" nowrap="nowrap" class="${estilo}">
		                                	<div align="center">
			                                	<a href="<c:url value='/SucursalesLista.do?accion=admin&subAccion=empresas&subSubAccion=sucursalesLista&rutEmpresa=${filaConsulta.idEmpresa}'/>">
			                                		<img alt="Listar Direcciones" title="Listar Direcciones" src="<c:url value="/img/btnListaSucursal.gif" />" width="14" height="13" border="0" />
			                                	</a>
			                                </div>
										</td>
		                                <td align="center" width="5%" align="center" nowrap="nowrap" class="${estilo}" style="display:none">
		                                	<div align="center">
		                                	...
		                                	<div align="center">
												<nested:equal property="editable" value="true">
				                                	<a href="<c:url value="/DetalleEmpresa.do?accion=admin&subAccion=empresas&subSubAccion=empresaEditar&rutEmpresa=${filaConsulta.idEmpresa}" />">
				                                		<img src="<c:url value="/img/ico_hojap.gif" />" alt="Modificar Independiente" title="Modificar Independiente" width="14" height="13" border="0"/>
				                                	</a>
				                                </nested:equal>
							...
							<nested:empty property="consulta">
								<tr>
									<td align="left" nowrap="nowrap" class="textos_formularios" colspan="6">
										<c:choose>
		             						<c:when test="${FLG_Busqueda == '0'}">No hay Independientes administrados por el usuario.</c:when>
...
<!--
	function validaFormulario(f)
	{
		...
		if (validaReq(document.getElementById("razonSocial")) &&
			...
			sMsje += "* Caracteres inválidos en la razón social del Independiente a buscar\n";
		else if (document.getElementById("razonSocial").value.length > 0 &&
				 document.getElementById("razonSocial").value.length < 3)
			sMsje += "* Debe ingresar al menos 3 caracteres en la razón social del Independiente a buscar\n";

		if (sMsje == "") {
			if (validaReq(document.getElementById("rutEmpresa")) &&
				!validaDV(document.getElementById("rutEmpresa").value))
				sMsje += "* Dígito verificador inválido para el rut el Independiente a buscar\n";
		}
