...
public class CertificadoAction extends Action {
	...
	private static String basedir = Configuraciones.getConfig("carpeta.jasper.recursos") ;
	private static String baserecursos = basedir + "/resources/";
	private static String baseireport = basedir + "/ireport/"; 
	private static String style = baserecursos + "resultado.css";
	...
	public static String generatePDFwithJasper(TestForm testForm, CreditoVO value) {
		try {
			JasperReport jasperReport = (JasperReport) JRLoader.loadObjectFromFile(baseireport + "simulacion.jasper");
			...
			JasperReport jasperReport1 = (JasperReport) JRLoader.loadObjectFromFile(baseireport + "simulacion.jasper");
			...
			JasperReport jasperReport2 = (JasperReport) JRLoader.loadObjectFromFile(baseireport + "resumen.jasper");
	...
	public static String generatePDFwithJasperTest(TestForm testForm) {
		try {
			...
			// Or if you want bytes create ByteArrayOutputStream
			// ByteArrayOutputStream out = new ByteArrayOutputStream();
			String pdfname = baseireport + "resultado " + testForm.getRut() + " .pdf";
			FileOutputStream output = new FileOutputStream(pdfname);
			//exporter.setExporterOutput(new SimpleOutputStreamExporterOutput(output));
			exporter.setParameter(JRExporterParameter.JASPER_PRINT_LIST, jasperPrints);
			exporter.setParameter(JRExporterParameter.OUTPUT_STREAM, output);
