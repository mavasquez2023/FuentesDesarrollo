...
public class GetCreditosVigentesAction extends Action {
	...
	private static Logger logger =
		Logger.getLogger(GetCreditosVigentesAction.class);

	public static final String GLOBAL_FORWARD_consultaCreditoVigente =
		"consultaCreditoVigenteSAP";
		...
		if (usuario.isEsEmpresa() || usuario.isEsEmpresaPublica()) {
			...
			do {
				...
				if (textRutAfiliado != null && textRutAfiliado.length() > 0) {
					...
					target = GLOBAL_FORWARD_definirEmpleado;
					session.setAttribute("volverA", "getCreditosVigentes");
					return mapping.findForward(target);
				}
			} while (
				!empleadoEncontrado
...
*/
		...
		target = GLOBAL_FORWARD_consultaCreditoVigente;
		return mapping.findForward(target);
	}
}
