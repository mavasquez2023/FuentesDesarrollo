package cl.araucana.autoconsulta.ui.actions.recuperacionesIntegracion;
...
public class RedireccionarInformeAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping,	ActionForm form,
		...
		String server = FileSettings.getValue(servicesFile,
				"/services-settings/recuperaciones/server-ip/");
		...
		if (session.getAttribute("uc") != null) {				
			if(accion != null && server != null){ 
				...
				try{
					...
					// envía el RutEmpresa
					url += "&rutEmpresa="+user.getRut()+"-" + user.getDv();
