...
public class GetAfiliacion extends Action
{
    ...
    throws Exception
  {
		...
		String sRutAfiliado = (String)session.getAttribute("afiliado.rut");
		lRutAfiliado = Long.parseLong(sRutAfiliado);
		String sDigAfiliado = (String)session.getAttribute("afiliado.digito");
		...
		try
		{
		  ...
		  System.out.println("rutFullAfiliado [" + lRutAfiliado + "][" + sDigAfiliado + "]");
		  afiliacion = proxy.obtenerDataCertificado(String.valueOf(lRutAfiliado), sDigAfiliado);
		...
		catch (Exception be)
		{
		  if ((wsErrorCode != 0) && (!wsErrorMsg.equals(""))) {
		    request.setAttribute("message", "Error al obtener informacion del Certificadox" + sDigAfiliado);
