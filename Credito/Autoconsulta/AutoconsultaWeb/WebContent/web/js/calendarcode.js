/*
 * This version was modified to support spanish language.
*/
/*
...
// GLOBAL variables
var scImgPath = cPath+'images/calendario/';
...
function spiffyCalManager() {
	...
	// *** HOLIDAYS ***************************
	this.Holidays = new Array("Ene-1", "Abr-18", "May-1", "May-21", "Jun-16", "Jun-29", "Ago-15", "Sep-18", "Sep-19", "Oct-12", "Nov-1", "Dic-8", "Dic-25");
	this.HolidaysDesc = new Array("Año Nuevo", "Viernes Santo", "Día del Trabajo", "Glorias Navales", "Corpus Christi", "San Pedro y San Pablo", "Ascensión de la Virgen", "Fiestas Patrias", "Glorias del Ejército", "Día de la Raza", "Todos los Santos", "Inmaculada Concepción", "Navidad");
	//*****************************************
...
'yyyy-MM-dd'
	...
	function validateDate(eInput, bRequired, dStartDate, dEndDate,InputName){

	validarFechaCalendario(eInput,InputName);
	return;
		...
		alert('Fomato de fecha Invalido');
...
function ctlSpiffyCalendarBox(strVarName, strFormName, strTextBoxName, strBtnName, strDefaultValue, intBtnMode) {
	...
	switch (intBtnMode) {
		...
		case 0 :
			...
			strBtnImg=strDefBtnImgPath+'ic_calendar.gif';
			img_DateBtn_UP.src=strDefBtnImgPath+'ic_calendar.gif';
			img_DateBtn_OVER.src=strDefBtnImgPath+'ic-calendar2.gif';
			img_DateBtn_DOWN.src=strDefBtnImgPath+'ic_calendar.gif';
			img_DateBtn_DISABLED.src=strDefBtnImgPath+'ic_calendar.gif';			
			strBtnW = '29';
			strBtnH = '18';
			break; 
		case 1 :
	...
	this.minYearChoice=dteToday.getFullYear()-100;
	this.maxYearChoice=dteToday.getFullYear()+1;
	...
	function setSelectedDate(strWhat) {
		...
		if (!calMgr.isDate(quote(strWhat),quote(this.dateFormat))) {
			...
			//eval('document.'+this.formName+'.'+this.textBoxName).className = "TextBlanco";
		}
		else {
	...
	function writeControl() {
		...
		if (!(this.useDateRange)) {
			strTemp+=' onChange="calMgr.validateDate(document.'+this.formName+'.'+this.textBoxName+','+this.varName+'.required,'+this.varName+'.minDate,'+this.varName+'.maxDate, nombre);" onKeypress="return acceptNum(event); onBlur="calMgr.formatDate(document.'+this.formName+'.'+this.textBoxName+','+this.varName+'.dateFormat);" ';
		}
		else {
		...
		if (!scIE) {
			strTemp=' href="javascript:calClick();" ';
		}
		else {
			strTemp='';
		}
