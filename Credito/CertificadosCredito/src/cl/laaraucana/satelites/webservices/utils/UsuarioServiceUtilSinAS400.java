...
public class UsuarioServiceUtilSinAS400 {
		...
		try {
			...
			if (salidaSAP.getCodigoError().equals(ConstantesRespuestasAPP.COD_RESPUESTA_ERROR)) {
				...
				for (SalidaAfiliadoVO detSap : salidaSAP.getListaAfiliado()) {
					...
					if(!listaRoles.contains(obtenerTipoAfiliadoSap(detSap.getRol()))){
						listaRoles.add(obtenerTipoAfiliadoSap(detSap.getRol()));
					}
					if(detSap.getRutEmpresa()!=null && obtenerTipoAfiliadoSap(detSap.getRol()).equals("Trabajador")){
				...
				afiliado.setListaRoles(listaRoles);
	...
	public static String obtenerTipoAfiliadoSap(String entrada){
		String salida = "";
