		...
		try {
			...
			for (CreditoVO creditoVO : listaCreditos) {
				if (creditoVO.getOperacion().equals(paramFolio)) {
					...
					try {
						codebar = ManagerCupon
								.crearCuponDePago(creditoVO, rutDeudor, Constantes.getInstancia().TIPO_PAGO_CAJA, total)
								.get("codigoBarra");
						...
						logger.error("Error al generar cupon de pago:" + creditoVO + " " + rutDeudor + " "
								+ Constantes.getInstancia().TIPO_PAGO_CAJA + " " + total);
					...
					hash.put("timbre",
							ResourceBundle.getBundle("resources.application").getString("path.timbreAgua.img"));
					break;
				}
			}
