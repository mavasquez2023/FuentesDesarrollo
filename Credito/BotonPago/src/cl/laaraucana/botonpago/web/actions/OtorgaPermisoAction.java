...
public class OtorgaPermisoAction extends DispatchAction {
	...
	public ActionForward busca(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			ArrayList<String> roles = new ArrayList<String>(
					urConnection.getUserRoles(id, Constantes.getInstancia().LDAP_APLICACION));
	...
	public ActionForward update(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			@SuppressWarnings("unchecked")
			ArrayList<String> oldRoles = new ArrayList<String>(
					urConnection.getUserRoles(formu.getId(), Constantes.getInstancia().LDAP_APLICACION));
				...
				for (String newRol : newRoles) {
					if (allRoles.contains(newRol) && !oldRoles.contains(newRol)) {
						if (newRol.equals(Constantes.getInstancia().LDAP_ROL_DEUDOR)) {
							...
							if (salida.isDeudor()) {
								urConnection.assignAppRole(formu.getId(), Constantes.getInstancia().LDAP_APLICACION,
										newRol);
							} else {
