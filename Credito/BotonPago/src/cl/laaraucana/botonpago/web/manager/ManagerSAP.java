...
public class ManagerSAP {
	...
	public SalidaCreditosVO obtenerCreditos(String rut) throws Exception {
		...
		for (SalidaCompContHeaderVO salidaQueryCompCont : listaBanking) {
			...
			if (!salidaContract.getCodigoError().equals(Constantes.getInstancia().APP_COD_SUCCESS)) {
				...
				SalidaListaQueryContractCashFlowVO salidacash = (SalidaListaQueryContractCashFlowVO) clienteCashFlow
						.call(entradaCashFlow);
				...
				if (!salidacash.getCodigoError().equals(Constantes.getInstancia().WS_COD_SUCCESS)) {
					...
					for (SalidaQueryContractCashFlowVO cuota : cuotas) {
						...
						if (cuota.getEstadoCuota().trim().equalsIgnoreCase("moroso")) {
							...
							totalDeuda += Integer.parseInt(Calculo.getCuota(cuota.getMontoCapital(),
									cuota.getMontoInteres(), cuota.getMontoServAdic(), cuota.getMontoAbono(),
									cuota.getMontoGravamenes()));

						}
						if (cuota.getEstadoCuota().trim().equalsIgnoreCase("vigente")
								&& Util.isFechaMenorA(cuota.getFechaVencCuota(), fecha_pago_futuro)) {
						...
						if (cuota.getNroCuota().equals("999")
								&& !cuota.getEstadoCuota().toLowerCase().equals("cancelada")) {
							totalDeuda = Integer
									.parseInt(Calculo.getCuotaEPO(cuota.getTotalCuota(), cuota.getMontoAbono()));
							cuotasCanceladas = 0;
							break;
						}
					}
					...
					Map<String, Integer> map = Calculo.getCondGCob(gastosCobranza,
							salidaQueryCompCont.getContractBegin());
					...
					credito.setProducto(Util.replace(salidaContract.getComercialLine(),
							Constantes.getInstancia().BUNDLE_LINEACOMERCIAL_SAP));
					credito.setOperacion(salidaQueryCompCont.getContractNumber());
					...
					credito.setPorcentCondGravamen(
							String.valueOf(Constantes.getInstancia().CONDONACION_GRAVAMENES.get(0).getPorcen()));
	...
	public SalidaCuotasVO obtenerCuotas(String folio) throws Exception {
		...
		if (!salidaCuotas.getCodError().equals(Constantes.getInstancia().WS_COD_SUCCESS)) {
			...
			ArrayList<SalidaQueryContractCashFlowVO> listacash = (ArrayList<SalidaQueryContractCashFlowVO>) salidacash
					.getListaCuotas();
			...
			for (SalidaQueryContractCashFlowVO salidaCashFlowVO : listacash) {
				...
				if ((salidaCashFlowVO.getEstadoCuota().trim().equalsIgnoreCase("vigente")
						&& Util.isFechaMenorA(salidaCashFlowVO.getFechaVencCuota(), fecha_pago_futuro)
						|| salidaCashFlowVO.getEstadoCuota().trim().equalsIgnoreCase("moroso"))) {
...
//					cuota.setMontoDeuda(salidaCashFlowVO.getTotalCuota());
					...
					cuota.setMontoDeuda(Calculo.getCuota(salidaCashFlowVO.getMontoCapital(),
							salidaCashFlowVO.getMontoInteres(), salidaCashFlowVO.getMontoServAdic(),
							salidaCashFlowVO.getMontoAbono(), salidaCashFlowVO.getMontoGravamenes()));

					listacuotas.add(cuota);
				}
				if (salidaCashFlowVO.getNroCuota().equals("999")
						&& !salidaCashFlowVO.getEstadoCuota().toLowerCase().equals("cancelada")) {
					CuotasVO cuota = new CuotasVO();
					cuota.setNcuota(salidaCashFlowVO.getNroCuota());
					cuota.setFechaVencimiento(salidaCashFlowVO.getFechaVencCuota());
					cuota.setEstadoCuota(salidaCashFlowVO.getEstadoCuota());
...
//					cuota.setMontoDeuda(salidaCashFlowVO.getTotalCuota());
					cuota.setMontoAbono(salidaCashFlowVO.getMontoAbono());
					cuota.setMontoDeuda(
							Calculo.getCuotaEPO(salidaCashFlowVO.getTotalCuota(), salidaCashFlowVO.getMontoAbono()));
	...
	public SalidaDeudorVO esDeudorSapYpStatus(String rut) throws Exception {
		 ...
		 * entradaBpStatus.setRutCliente(rut); SalidaListaQueryBPVO salidaBpStatusVO =
		 * (SalidaListaQueryBPVO) clienteBpStatus.call(entradaBpStatus);
		 * 
		 * if (Constantes.getInstancia().APP_COD_SUCCESS.equals(salidaBpStatusVO.
		 * getCodigoError())) { SalidaQueryBPVO salidaBP = isAfiliado(salidaBpStatusVO);
		 ...
		 * salidaDeudor.setRutDeudor(rut); } else {
		 * salidaDeudor.setCodSalida(salidaBpStatusVO.getCodigoError());
		 * salidaDeudor.setMsjSalida(salidaBpStatusVO.getMensaje()); return
		 * salidaDeudor; }
		 */
		// if (!esAfiliado) {
		EntradaCompContHeaderVO entrada = new EntradaCompContHeaderVO();
		...
		/*
		 * } else { salidaDeudor.setCodError(Constantes.getInstancia().APP_COD_SUCCESS);
		 * salidaDeudor.setDeudor(false); salidaDeudor.setMensaje("usuario no deudor");
		 * }
		 */
		return salidaDeudor;
