...
patrones['tipo_direccion'] = /[a-zA-Z0-9-/ #áéíóúÁÉÍÓÚñÑäëïöüÄËÏÖÜ]/;
patrones['tipo_numerico'] = /\d/;
patrones['tipo_rut'] = /[kK0-9\-]/;
patrones['tipo_serie'] = /[Aa0-9]/;
...
antipatrones['tipo_direccion'] = /^[a-zA-Z0-9-/ #áéíóúÁÉÍÓÚñÑäëïöüÄËÏÖÜ]/g;
antipatrones['tipo_numerico'] = /[^0-9]/g;
antipatrones['tipo_rut'] = /[^kK0-9\-]/g;
antipatrones['tipo_serie'] = /[^Aa0-9]/g;
...
function automatizar_init() {
  ...
  $(':text, :password, textarea, select').each(function (i, el) {
    ...
    $(el).blur(function () {
      ...
      if ($(this).prop("tagName") == "SELECT" && $(this).parent().hasClass('select')) {
    ...
    if (!$(el).data('requerido')) {
      $(el).blur(function () {
        if ($(this).hasClass("tipo_telefono") && this.value == "+56") {
      ...
      if ($(el).hasClass(key)) {
        ...
        $(el).bind('paste', function (event) {
          ...
          if ($(el).attr('maxlength')) {
            pasteValue = pasteValue.substr(0, parseInt($(el).attr('maxlength')));
          }
...
function aplicarEventos(el) {
  if($(el).data("automatizado")){ return; }
  $(el).data("automatizado",true);
  ...
  if (tipo == "tipo_telefono") {
    $(el).attr("maxlength", 12);
    $(el).blur(function () {
      if (!$(this).data("requerido") && this.value == $(this).data("valueRequerido")) {
        aplicarClases(this, '');
      } else {
...
function bloquearCampo(el, val) {
  ...
  if ($(el).val()) {} else if ($(el).data('placeholder')) {
    $(el).val($(el).data('placeholder'));
  } else {
    $(el).val("");
  }
  ...
  $(el).keydown(function (event) {
    ...
    if (obtenerPosicionCursor($(el)[0]) == val.length) { //bloquear el backspace
      ...
      if (this.selectionStart === 0 && this.selectionEnd === this.value.length) {} else if (code === 46) {
        continuar = false;
      }
    }
  ...
  $(el).keyup(function (event) {
    ...
    if (this.value.indexOf($(this).data("valueRequerido")) != 0) {
      ...
      } else if ($(this).data("textoAntes").indexOf($(this).data("valueRequerido")) == 0) {
        this.value = $(this).data("textoAntes") + this.value;
      } else {
        this.value = $(this).data("valueRequerido") + $(this).data("textoAntes");
      }
    }
  });
  $(el).blur(function () {
...
function validarNombre(nombre) {
  ...
  if (nombres.length > 1 && nombres[0] != "" && nombres[1] != "" && nombre.length > 4) {
    estado = 'valido';
  }
  return estado;
}
