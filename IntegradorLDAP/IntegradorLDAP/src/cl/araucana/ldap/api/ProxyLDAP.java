...
public class ProxyLDAP 
{
	...
	{	
		...
		{	user=getUser(username);
			...
			String apellido= persona.getApellidoPaterno().trim();
			if(apellido.length()>20){
				apellido= apellido.substring(0, 20);
			}
			String apellidom= persona.getApellidoMaterno().trim();
			if(apellidom.length()>20){
				apellidom= apellidom.substring(0, 20);
			}
			if (user != null)
			{
