...
function consultarUsuario() {
	...
	$
			.getJSON(
					...
					ocultarCargando();
					if(!$("#nuevo").val()=="true" && rol_usuario=="Ejecutivo"){
...
if(rol_usuario=="Agente"){
	...
	$("#agregar_encargado").css('visibility', 'visible');
						...
						if (data != null) {
							...
							*/
							...
							if(data.empresa.observaciones!=null && data.empresa.observaciones!=""){
								...
								$("#agregar_encargado").css("visibility", "hidden");
							}
...
function consultarAnexo() {
		if($("#rutanexo").val()==""){
			marcarCampoError("rutanexo", "RUT Empresa");
			return;
		}
		if(!validarRutIngresado("rutanexo", $("#rutanexo").val())){
			return;
		}
		$("#rutanexo").attr('disabled', 'disabled');
		
		$("#agregar_encargado_anexo").css('visibility', 'visible');
		$("#consultar_encargados_anexo").css('visibility', 'visible');
		var url = "ActionServlet?action=CANXBD&rutempresa="
				+ $("#rutanexo").val() 
				+ "&rand=" + Math.random();
		;
		//openWindow(url, "blank", 800, 600);
		mostrarCargando(texto_cargando_informacion);
		...
		$
				.getJSON(
						...
						function(data) {
							...
							if (data != null) {
								$("#detalle").html("");
								$("#razonSocialAnexo").val(data.empresa.razonSocialEmpresa);

								$("#estado").val(data.empresa.estado);
								if(data.empresa.observaciones!=null && data.empresa.observaciones!=""){
									mostrarInfoError(data.empresa.observaciones);
									$("#consultar_encargados").css("visibility", "hidden");
									$("#agregar_encargado_anexo").css("visibility", "hidden");
									$("#consultar_encargados_anexo").css('visibility', 'hidden');
								}
...
function consultarEncargadosAnexo() {
	if($("#rutanexo").val()==""){
		marcarCampoError("rutanexo", "RUT Empresa");
		return;
	}
			...
			.getJSON(
					...
					function(data) {
						...
						if (data != null) {
							...
							if (data.usuarios_registrados != null) {
								$("#detalle").append("<table align=\"center\" class=\"tabla_formulario\" cellspacing=\"2\">");
								$("#detalle").append("<tr class=\"tr_formulario\"><td class=\"titulo_campo\" align=\"center\">Encargados Anexo</td><td class=\"titulo_campo_check\" align=\"center\">Todas las Oficinas</td><td class=\"titulo_campo_check\" align=\"center\">Acción</td></tr>");
								$.each(data.usuarios_registrados, 
									function(indice, usuario){
									linea= "<tr class=\"tr_formulario\"><td class=\"data_campo_ancho\" style=\"text-align: left;\">" + usuario.RUT + " " + usuario.NOMBRE + "</td><td class=\"data_campo_ancho\">" + usuario.ALLOFFICE + "</td><td class=\"data_campo_check\"><a href=\"#\" title=\"Editar Oficinas\" onclick=\"editarOficinasAnexo('" + usuario.RUT + "', '" + rutempresa + "', '" + usuario.NOMBRE + "', '" + usuario.ALLOFFICE + "');\"><span class=\"glyphicon glyphicon-pencil\"></span></a>&nbsp;&nbsp;<a href=\"#\" title=\"Eliminar RUT\" onclick=\"addUsuarioAnexo('" + usuario.RUT + "','E');\"><span class=\"glyphicon glyphicon-remove-circle\"></span></a></td></tr>"
									$("#detalle").append(linea);
								});
								linea= "<tr class=\"tr_formulario\"><td class=\"data_campo\">Ingrese RUT:&nbsp;<input type=\"text\" name=\"newuseranx\" id=\"newuseranx\" maxlength=\"13\" onBlur=\"validarRutIngresado(this.id,this.value);\" onKeyPress=\"keyRut();\" onKeyUp=\"formateaRut(this)\"/></td>"+
									"<td class=\"data_campo_check\"></td><td class=\"data_campo_check\"><a href=\"#\" title=\"Agregar RUT\" onclick=\"addUsuarioAnexo('', 'N');\"><span class=\"glyphicon glyphicon-ok-circle\"></span></a>"+
																	"&nbsp;&nbsp;<a href=\"#\" title=\"Subir Lista\" onclick=\"cargarArchivoANX('N');\"><span class=\"glyphicon glyphicon-list-alt\"></span></a></td></tr>";
								$("#detalle").append(linea);
								linea= "<tr class=\"tr_formulario\"><td class=\"data_campo\"><div id=\"upfile_e\" style=\"visibility: hidden\"><input type=\"file\" name=\"listusu_anx\" id=\"listusu_anx\" value=\"\" />&nbsp;<a href=\"#\" onclick=\"ayuda();\"><img src=\"images/help.jpg\" title=\"Formato archivo\"></a></div></td>"+
								"<td class=\"data_campo_check\" colspan=\"2\"><div id=\"upfile_anx\" style=\"visibility: hidden\"><input type=\"submit\" name=\"Subir\" value=\"Subir\" onclick=\"submitFileANX();\" /></div></td></tr>"
								$("#detalle").append(linea);
								$("#detalle").append("</table");
								abrir_detalle(600, 350);
							}else{
								$("#detalle").html("No existen Encargados de Empresa");
							}
						}
					...
					function(data) {
						if (data != null) {
							if(data.estado=="1"){
								...
								mostrarInfoError("ERROR, operación no realizada, usuario NO afiliado o sin información");
							}
...
function addUsuarioAnexo(user, estado) {
	var ventana= true;
	if(user!=""){
		ventana=false;
	}
	rol_usuario= $("#rol_usuario").val();
		...
		if(estado=="N" && user==""){
			user= $("#newuseranx").val();
			if(!validarRutIngresado("newuseranx", user)){
				return;
			}
		}
		if(user==""){
			marcarCampoError("newuseranx");
			return;
		}

		var url = "ActionServlet?action=GUANXBD" 
				+ "&rutEmpresa=" + $("#rutanexo").val() 
				+ "&userid=" + user
				+ "&estado=" + estado
				+ "&rand=" + Math.random();
		if(ventana==true){
...
function guardarAnexo() {
	rol_usuario= $("#rol_usuario").val();
		...
		var url = "ActionServlet?action=GANXBD&rutempresa="
				+ $("#rutempresa").val() 
				+ "&razonSocialEmpresa=" + $("#razonSocialEmpresa").val()
				+ "&nuevoe=" + $("#nuevoe").val()
			/*	+ "&afiliada=" + afiliado
...
function descargarAuditoria(){
	var dias= $("#diasAuditoria").val();
	
	var url = "ActionServlet?action=DOWNAUDI&diasAuditoria="
			+ dias
			+ "&rand=" + Math.random();
	;
	window.location= url;
	...
	/*$
			.getJSON(
					...
					function(data) {
						...
						if (data != null) {
							if(data.auditoria=="1"){
								mostrarInfo("Archivo generado exitosamente!");
						
							}else{
								mostrarInfoError("ERROR, Archivo No generado, consulte log.");
							}
						}
					//ocultarCargando();
