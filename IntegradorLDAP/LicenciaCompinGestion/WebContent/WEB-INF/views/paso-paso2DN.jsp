...
function drawStacked() {
      ...
      data.addColumn('number', 'Oficinas');

      data.addRows([
		<c:forEach var="reg" varStatus="vs" items="${oficinas}">
		<c:if test='${distribucion=="R"}'>
		['${reg.rango}', ${reg.totalLicencias}]<c:if test="${!vs.last }">,</c:if>
		</c:if>
		<c:if test='${distribucion=="P"}'>
		['${reg.percentil}', ${reg.totalLicencias}]<c:if test="${!vs.last }">,</c:if>
		</c:if>					
		</c:forEach>
      ...
      var options = {
        title: 'Cantidad de Oficinas por Número de Licencias Recibidas para el Periodo ${periodoActual}',
        isStacked: true,
        hAxis: {
          title: 'Rango Cantidad Licencias'
        },
        vAxis: {
 ...
 <g>
  ...
  <ellipse ry="9.24249" rx="8.96572" cy="20.6879" cx="48.9012" stroke-width="8" stroke="#f2f7f7" fill="none" id="svg_22"/>
			...
			<div class="col xs12 lg10">
					...
					<div class="row justify-center-xs justify-start-sm">
						...
						<div class="col xs12 sm12 md4 oculto-xs block-md">
							<div
								...
								<div class="pasos__data">
									<span>Paso 2</span>CARGA x OFICINA
								</div>
							</div>
						</div>
						<div class="col xs12 sm12 md4">
							<div
								...
								<div class="pasos__data">
									<span>Paso 3</span>LICENCIAS x OFICINA
								</div>
							</div>
						</div>
					</div>
				...
				<p style="margin-top: 10px">Carga de las Oficinas en base a Cantidad de Licencias Recibidas</p>
				<form class="form" id="paso2DN" action="paso2DN.do" method="GET" style="margin-top: 10px">
						<div class="col xs12 lg3">
							...
							<div class="form__grupo" data-animacion="placeholder"
								data-comentario="">
								<select name="periodo" id="periodo" onChange="validarPaso2DN();">
				...
				<form class="form" id="paso2" action="paso2.do" method="GET" style="margin-top: -50px">
						<div class="col xs12 lg6">
							<p class="adicional" style="font-size: medium;">Selecciona
								tipo Grafico:
							</p>
							<p style="margin-top: 10px">
								<input type="radio" name="tipografico" id="tipografico" value="F" onClick="validarPaso2();">&nbsp;Frecuencia
								&nbsp;&nbsp;<input type="radio" name="tipografico" id="tipografico" value="N" checked="checked">&nbsp;Distribución Normal
							</p>
						</div>
						<div class="col xs12 lg10">
						...
						<div class="col xs12 lg8">
							<div class="btn__grupo text-align-right-xs" style="margin-top: -20px">
								<a id="botonIL" style="margin-left: 20px"
									...
									>Volver</a>&nbsp;&nbsp;
								<a id="botonIL" style="margin-left: 20px"
									class="btn btn--primario" href="<c:url value='/paso3.do' />"
									>Siguiente</a>
							</div>
						</div>
