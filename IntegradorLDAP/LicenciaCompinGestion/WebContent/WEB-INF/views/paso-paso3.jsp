 ...
 <g>
  ...
  <ellipse ry="9.24249" rx="8.96572" cy="20.6879" cx="48.9012" stroke-width="8" stroke="#f2f7f7" fill="none" id="svg_22"/>
			...
			<div class="col xs12 lg10">
				...
				<form class="form" id="paso3" action="paso3.do" method="GET" style="margin-top: 10px">
					<div class="row">
						
						<div class="col xs12 lg3">
						<p class="adicional"
									style="font-size: medium;">
						Selecciona otro Rango:</p>
							<div class="form__grupo" data-animacion="placeholder" data-comentario="">
								<select name="rango" id="rango" onChange="validarPaso3();">
								
									<c:forEach var="entry" varStatus="vs" items="${rango_select}" >
										<option value="${entry.value }" 
										<c:if test="${entry.value==rangoActual }"> selected="selected" </c:if>
										>${entry.descripcion }</option>
									</c:forEach>
								</select>
							</div>
						</div>
						...
						<div class="col xs12 lg6">
							<div class="btn__grupo text-align-right-xs" style="margin-top: -20px">
								<a id="botonIL" style="margin-left: 20px"
									class="btn btn--secundario" href="<c:url value='/paso2DN.do' />"
									>Volver</a>
							</div>
						</div>
					</div>
