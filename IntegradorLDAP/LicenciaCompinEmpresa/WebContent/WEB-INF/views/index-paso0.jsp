...
<script type="text/javascript">
	function validaSeleccion(rutemp) {
		$('#rutemp').val(rutemp);
		paso0.setAttribute('action', 'paso1.do');
		fln.preloader(1);
		paso0.submit();
	}
	
	function validaDescarga(rutemp) {
		$('#rutemp').val(rutemp);
		paso0.setAttribute('action', 'descargarArchivo.do');
 ...
 <g>
  ...
  <ellipse ry="9.24249" rx="8.96572" cy="20.6879" cx="48.9012" stroke-width="8" stroke="#f2f7f7" fill="none" id="svg_22"/>
			...
			<div class="col xs12 lg10">
						...
						<div class="col xs12 sm12 md4 oculto-xs block-md">
							<div
								...
								<div class="pasos__data">
									<span>Encargado</span>SELECCIONA UNA EMPRESA
								</div>
							</div>
						</div>
					</div>
				...
				<p style="margin-top: 10px">Este servicio permitirá subir las licencia de tus
					trabajadores en forma electrónica</p>

				<div class="col xs12 lg12">
							<div class="form__grupo" data-animacion="placeholder"
								data-comentario="">
								<p class="adicional" style="margin-top: 10px;margin-left:-15px;font-size: medium;">Para información e instrucciones de uso puede descargar instructivo 
								<br><a style="color:#2072BC" href="<c:url value='/img/Indicaciones_Licencia.pdf' />" target="_blank">descargar aquí</a></p>
							</div>								
				  </div>
				<form class="form" name="paso0" id="paso0" action="paso1.do" method="POST">
					<div class="row">
						<table align="center" class="tabla-creditos" style="width: 650px">
							<thead>
								<tr>
									<th class="certificadoLeft" style="height: 30px; vertical-align: middle;">RUT&nbsp;EMPRESA</th>
									<th class="certificadoLeft" style="height: 30px; vertical-align: middle;">NOMBRE EMPRESA</th>
									<th class="certificadoCenter" colspan="2" style="height: 30px; vertical-align: middle;">OPCIONES</th>
							...
							<tbody>
								<c:forEach var="entry" varStatus="vs" items="${empresas}">
									<tr>
										<td class="certificadoLeft" style="height: 30px; vertical-align: middle;">${entry.key}</td>
										<td class="certificadoLeft" style="height: 30px; vertical-align: middle;">${entry.value}</td>
										<td class="certificadoLeft" style="height: 30px; vertical-align: middle;"><input
											class="btn btn--secundario btn--small" id="seleccion"
											name="seleccion" type="button" value="Seleccionar"
											onclick="validaSeleccion('${entry.key}')" /></td>
										<td class="certificadoLeft" style="height: 30px; vertical-align: middle;"><input
											class="btn btn--secundario btn--small" id="seleccion"
											name="seleccion" type="button" value="Descargar" title="Descargar Licencias Pendientes"
											onclick="validaDescarga('${entry.key}')" /></td>
									</tr>
						 ...
						 <!--  <table align="center" class="" width="800">
							<tr height="45">
								<th align="left" width="150" style="font-size: 1.6em">RUT
									EMPRESA</th>
								<th width="450" style="font-size: 1.6em">NOMBRE EMPRESA</th>
								<th width="150" style="font-size: 1.6em">&nbsp;</th>
							</tr> 
							<c:set var="total" value="0"></c:set>
							<c:forEach var="entry" varStatus="vs" items="${empresas}">
								<tr height="55">
									<td width="150" class="text"
										style="font-size: 1.6em; color: gray;">${entry.key}</td>
									<td width="450" class="text"
										style="font-size: 1.6em; color: gray;">${entry.value}</td>
									<td class="text"><input
										class="btn btn--secundario btn--small" id="seleccion"
										name="seleccion" type="submit" value="Seleccionar"
										onclick="validaSeleccion('${entry.key}')" /></td>
								</tr>
								<div class="separador"></div>
							</c:forEach>
						</table> -->
						<input type="hidden" id="rutemp" name="rutemp" value="" />
