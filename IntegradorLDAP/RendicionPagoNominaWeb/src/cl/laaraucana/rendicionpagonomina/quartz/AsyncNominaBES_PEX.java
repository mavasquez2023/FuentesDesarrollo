...
public class AsyncNominaBES_PEX {
	private static final Logger logger = Logger.getLogger(AsyncNominaBES_PEX.class);
	
	@Value("${config.cron.bes.rendicion.pex.convenio}")
	private String convenios;
	
	@Autowired
	private ProcesaRendicionBESService processfileService;

	@Scheduled(cron = "${config.cron.bes.rendicion.pex}")
	public void asyncProcessNomina() {
		
		String estado = Configuraciones.getConfig("cronta.bes.estado");
		logger.info("Cron working, estado cronta BES PEX: " + estado);
