...
public class AsyncNominaBES_BECAS {
	private static final Logger logger = Logger.getLogger(AsyncNominaBES_BECAS.class);
	
	@Value("${config.cron.bes.rendicion.becas.convenio}")
	private String convenios;
	
	@Autowired
	private ProcesaRendicionBESService processfileService;

	@Scheduled(cron = "${config.cron.bes.rendicion.becas}")
	public void asyncProcessNomina() {
		
		String estado = Configuraciones.getConfig("cronta.bes.estado");
		logger.info("Cron working, estado cronta BES BECAS: " + estado);
