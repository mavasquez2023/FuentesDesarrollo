package cl.laaraucana.rendicionpagonomina.ibatis.vo;

import java.util.Date;

public class MandatoAS400Vo {

	private long idMandato;
	private long rutafi;
	private String dvafi;
	private String nombre;
	private String celular;
	private String telefono;
	private String email;
	private int codbanco;
	private String numcuenta;
	private int idtipcta;
	private int idtippro;
	private Date fechavig;
	private Date fechater;
	private Date obf002;
	private Date obf003;
	private String sajkm94;
	private String nombreBanco;
	private String desCuenta;
	private String estado;
	private String observaciones;
	/**
	 * @return the idMandato
	 */
	public long getIdMandato() {
		return idMandato;
	}
	/**
	 * @param idMandato the idMandato to set
	 */
	public void setIdMandato(long idMandato) {
		this.idMandato = idMandato;
	}
	/**
	 * @return the rutafi
	 */
	public long getRutafi() {
		return rutafi;
	}
	/**
	 * @param rutafi the rutafi to set
	 */
	public void setRutafi(long rutafi) {
		this.rutafi = rutafi;
	}
	/**
	 * @return the dvafi
	 */
	public String getDvafi() {
		return dvafi;
	}
	/**
	 * @param dvafi the dvafi to set
	 */
	public void setDvafi(String dvafi) {
		this.dvafi = dvafi;
	}
	/**
	 * @return the nombre
	 */
	public String getNombre() {
		return nombre;
	}
	/**
	 * @param nombre the nombre to set
	 */
	public void setNombre(String nombre) {
		this.nombre = nombre;
	}
	/**
	 * @return the celular
	 */
	public String getCelular() {
		return celular;
	}
	/**
	 * @param celular the celular to set
	 */
	public void setCelular(String celular) {
		this.celular = celular;
	}
	/**
	 * @return the telefono
	 */
	public String getTelefono() {
		return telefono;
	}
	/**
	 * @param telefono the telefono to set
	 */
	public void setTelefono(String telefono) {
		this.telefono = telefono;
	}
	/**
	 * @return the email
	 */
	public String getEmail() {
		return email;
	}
	/**
	 * @param email the email to set
	 */
	public void setEmail(String email) {
		this.email = email;
	}
	/**
	 * @return the codbanco
	 */
	public int getCodbanco() {
		return codbanco;
	}
	/**
	 * @param codbanco the codbanco to set
	 */
	public void setCodbanco(int codbanco) {
		this.codbanco = codbanco;
	}
	/**
	 * @return the numcuenta
	 */
	public String getNumcuenta() {
		return numcuenta;
	}
	/**
	 * @param numcuenta the numcuenta to set
	 */
	public void setNumcuenta(String numcuenta) {
		this.numcuenta = numcuenta;
	}
	/**
	 * @return the idtipcta
	 */
	public int getIdtipcta() {
		return idtipcta;
	}
	/**
	 * @param idtipcta the idtipcta to set
	 */
	public void setIdtipcta(int idtipcta) {
		this.idtipcta = idtipcta;
	}
	/**
	 * @return the idtippro
	 */
	public int getIdtippro() {
		return idtippro;
	}
	/**
	 * @param idtippro the idtippro to set
	 */
	public void setIdtippro(int idtippro) {
		this.idtippro = idtippro;
	}
	/**
	 * @return the fechavig
	 */
	public Date getFechavig() {
		return fechavig;
	}
	/**
	 * @param fechavig the fechavig to set
	 */
	public void setFechavig(Date fechavig) {
		this.fechavig = fechavig;
	}
	/**
	 * @return the fechater
	 */
	public Date getFechater() {
		return fechater;
	}
	/**
	 * @param fechater the fechater to set
	 */
	public void setFechater(Date fechater) {
		this.fechater = fechater;
	}
	/**
	 * @return the obf002
	 */
	public Date getObf002() {
		return obf002;
	}
	/**
	 * @param obf002 the obf002 to set
	 */
	public void setObf002(Date obf002) {
		this.obf002 = obf002;
	}
	/**
	 * @return the obf003
	 */
	public Date getObf003() {
		return obf003;
	}
	/**
	 * @param obf003 the obf003 to set
	 */
	public void setObf003(Date obf003) {
		this.obf003 = obf003;
	}
	/**
	 * @return the sajkm94
	 */
	public String getSajkm94() {
		return sajkm94;
	}
	/**
	 * @param sajkm94 the sajkm94 to set
	 */
	public void setSajkm94(String sajkm94) {
		this.sajkm94 = sajkm94;
	}
	/**
	 * @return the nombreBanco
	 */
	public String getNombreBanco() {
		return nombreBanco;
	}
	/**
	 * @param nombreBanco the nombreBanco to set
	 */
	public void setNombreBanco(String nombreBanco) {
		this.nombreBanco = nombreBanco;
	}
	/**
	 * @return the desCuenta
	 */
	public String getDesCuenta() {
		return desCuenta;
	}
	/**
	 * @param desCuenta the desCuenta to set
	 */
	public void setDesCuenta(String desCuenta) {
		this.desCuenta = desCuenta;
	}
	
	/**
	 * @return the estado
	 */
	public String getEstado() {
		return estado;
	}
	/**
	 * @param estado the estado to set
	 */
	public void setEstado(String estado) {
		this.estado = estado;
	}
	/**
	 * @return the observaciones
	 */
	public String getObservaciones() {
		return observaciones;
	}
	/**
	 * @param observaciones the observaciones to set
	 */
	public void setObservaciones(String observaciones) {
		this.observaciones = observaciones;
	}
	public String toString(){
		StringBuffer registro=	new StringBuffer();
		registro.append(this.idMandato);
		registro.append(";");
		registro.append(this.rutafi);
		registro.append(";");
		registro.append(this.dvafi);
		registro.append(";");
		registro.append(this.nombre.trim());
		registro.append(";");
		if(this.telefono.substring(0, 2).equals("56")){
			this.telefono= this.telefono.substring(2);
		}
		registro.append(this.telefono.trim());
		registro.append(";");
		if(this.celular.substring(0, 2).equals("56")){
			this.celular= this.celular.substring(2);
		}
		registro.append(this.celular.trim());
		registro.append(";");
		registro.append(this.email.trim());
		registro.append(";");
		registro.append(this.codbanco);
		registro.append(";");
		registro.append(this.numcuenta.trim());
		registro.append(";");
		registro.append(this.idtipcta);
		return registro.toString();

		
	}
	
	
}
