...
function ConfirmDialog(message) {
    ...
    .html('<div><h5>'+message+'?</h5></div>')
    .dialog({
        modal: true, title: 'Cesar Trabajador', zIndex: 10000, autoOpen: true,
        width: 'auto', resizable: false,
        buttons: {
            Si: function () {
...
function isKeyNum(){
     ...
     event.charCode==8 || event.charCode==46) return true;
     else return false;
}
//valida si tecla presionada es texto entre (A y Z) o (a y Z)
function isKeyText(){
	if((event.keyCode > 64 && event.keyCode<91) || (event.keyCode >96 && event.keyCode < 123) || event.keyCode==8 || event.keyCode ==46 || event.keyCode ==32 || (event.charCode > 64 && event.charCode<91) || (event.charCode >96 && event.charCode < 123) || event.charCode==8 || event.charCode ==46 || event.keyCode ==32) return true;
	else return false;
}
...
function keyText(){
	if (!isKeyText() && !( event.keyCode== 46 || event.keyCode== 32 || event.charCode== 46 || event.charCode== 32) ) event.returnValue = false;
}
//permite key pressed de telefono, vale decir: números, +  - ()
function keyFono(){
	if (!isKeyNum() && !( event.keyCode== 40 || event.keyCode== 41 || event.keyCode== 43|| event.keyCode== 45 || event.charCode== 40 || event.charCode== 41 || event.charCode== 43|| event.charCode== 45)) event.returnValue = false;
}
//permite key pressed de fecha, vale decir: números, slash y guión
function keyFecha(){
	if (!isKeyNum()  && !( event.keyCode== 45 || event.keyCode== 47 || event.charCode== 45 || event.charCode== 47)) event.returnValue = false;
}
//permite key pressed de email, numeros, texto y @ _ .
function keyEmail(){
	if (!isKeyNum() && !isKeyText() && !(event.keyCode== 46 || event.keyCode== 64 || event.keyCode== 95 || event.charCode== 46 || event.charCode== 64 || event.charCode== 95)) event.returnValue = false;
}
