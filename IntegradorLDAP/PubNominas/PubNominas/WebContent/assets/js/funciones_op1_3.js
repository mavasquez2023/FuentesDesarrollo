...
function verifyContent(){
	try{ 
		var variable;
		document.getElementById("iaccion").contentWindow.postMessage(variable, '*');
	}catch(e){
		console.error(e);
	}
	
	try{ 
		console.log("Body " + document.getElementById("iaccion").contentWindow.document.body.childNodes);
		console.log("Body " + document.getElementById("iaccion").contentWindow.document.body);
	}catch(e){
		console.error(e);
	}
...
return;
	...
	try{
		hayAlgo = (document.getElementById("iaccion").contentWindow.document.body!=null);
	}catch(e){
//		console.error(e);
		hayAlgo = false;
	}

	console.log('hay algo? ' + hayAlgo);
	
	if(hayAlgo){
		clearInterval(intervalId);
		const body = document.getElementById("iaccion").contentWindow.document.body;
		const tableSource = body.getElementsByClassName("table")[0];
		const tableTarget = document.getElementById("nombreColumna");
		const html = $(tableSource).html();		
		console.log(html);
		$(tableTarget).html(html);
		$(tableTarget).html(html);
		
		tableTarget.style.display = '';
	}
...
var formsPlanillas = function formsPlanillas(from) {
	...
	var opcionesAvanzada = document.getElementById("opciones-avanzadas");
	var codigoOficina = get("#codigo-oficina");
    var	codigoSucursal = get("#codigo-sucursal");
	/*
	var mesTermino = get("#mes-termino");
	var anoTermino = get("#ano-termino");
	
	var codigoOficina = get("#codigo-oficina");
	var codigoNomina = from == 'NC' ? get("#codigo-nomina") : null;
    var	tipoAhorro = from == 'NL' ? get("#tpo-ahorro") : null;
*/
/*
...
// console.log("elemento: "+$(".datosDo"));

console.log("Opcion: "+from);
console.log("opcionesAvanzada: " + opcionesAvanzada.checked);

console.log("periodo: ", periodo);
console.log("mes: ", mesInicio, " anio: ", anoInicio);
console.log('AllRuts: ', allRuts);

if(from=='PC'){ 
	if(periodo=='0'){
		periodo = anoInicio + mesInicio;
	}
	document.getElementById('Periodo').value = periodo;
	
	if(rutEmpresa.trim().length==0){
		if(allRuts.trim().length>0){
			document.getElementById('RutEmpresa').value = allRuts;
		}
	} else {
		if(allRuts==''){
			allRuts = document.getElementById('RutEmpresa').value;
		}
		rutEmpresa = $.Rut.quitarFormato(rutEmpresa);
	//console.log($('.RutEmpresa').val());
		rutEmpresa = rutEmpresa.substr(0,rutEmpresa.length-1);
		document.getElementById('RutEmpresa').value = rutEmpresa;
	}
} else {
	
}

if(opcionesAvanzada.checked){
	document.getElementById("CodigoOficina").value = codigoOficina;
	document.getElementById("CodigoSucursal").value = codigoSucursal;
}else{
	document.getElementById("CodigoOficina").value = "";
...
document.getElementById("datosDo").submit();


//intervalId = setInterval(verifyContent, 300);
...
try{ 
$.ajax( {
	url: "http://rasp.laaraucana.cl/PlanillaCotizacionesWeb/datos.do",
	cache : false,
	crossDomain: true,
	dataType: 'html',
	method :'POST', 
data: {
		  _display_fields : 'Periodo,Rut Empresa,Razon Social,Codigo Oficina,Codigo Sucursal',
          _folder : 'Comprobante Aporte Asfam PDF',
          ...
          _html: 'araucana.htm',
          Periodo: '202102',
		  RutEmpresa: '5355092 65154596 70016160 76033251 76042014 76098502 76125014 76134946 76196870 76232647 76243813 76258350 76473580 76833720 76879810 77085380 78454050 78507380 78954200 78968610 84794200 94226000 96562520 96755580 99579260',
		  RazonSocial: '%',
		  CodigoOficina:'',
		  CodigoSucursal:''
        } 
...
$.post( "http://rasp.laaraucana.cl/PlanillaCotizacionesWeb/datos.do", {
		  _display_fields : 'Periodo,Rut Empresa,Razon Social,Codigo Oficina,Codigo Sucursal',
          _folder : 'Comprobante Aporte Asfam PDF',
          _function: 'dochitlist',
          _max_hits: '200',
...
//	alert("tipo :"+tipo+" rutAfiliado: "+rutAfiliado+" Rut Empresa: "+ rutEmpresa+ " Periodo: "+ periodo);
  ...
  req.responseType = "blob";
      ...
      $.ajax({
        method: 'POST',
        url: '/PdfNominaDeCredito?_=' + new Date().getTime(),
        data: {
			index:f
		},
		xhrFields: {
...
//	alert("tipo :"+tipo+" rutAfiliado: "+rutAfiliado+" Rut Empresa: "+ rutEmpresa+ " Periodo: "+ periodo);
  ...
  $.ajax({
    method: 'POST',
    url: '/PdfCartaRenovacionCargas?_=' + new Date().getTime(),
    data: {
      tipo:tipo,
