        ...
        if (event.target.files.length) {
          if (!_formatos.length || _formatos[0].includes(event.target.value.split('.').pop())) {
            if (event.target.files[0].size < _size) {
            	$('.documento__item').get(0).classList.add('valido');
...
var validarPaso1 = function validarPaso1() {
  ...
  if ($('#celular').val().length == $('#celular').data('minlength') || _telefono== true) {
    _celular = true;
  }
  ...
  var _email = false;
  var largo_email= $('#email').val().length;
  var dominio= $('#email').val().substr(largo_email-3);
  if ( dominio== '.cl' || dominio== 'com' || dominio== 'net' || dominio== 'org' || dominio== '.es'){
	  _email = true;
  }
...
var validarPaso3 = function validarPaso3() {
  ...
  var adjunto = false;
  if ($('.documento__item').get(0).classList.contains('valido')) {
    adjunto = true;
