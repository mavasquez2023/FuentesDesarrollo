...
var validarPaso3 = function validarPaso3() {
	...
	var email= $('#email').val();
	var pais_email= email.substring(email.lastIndexOf(".")+1);
	if ( email!="" && (pais_email.length==2 || pais_email.length==3)) {
		_emailValidos = true;
	}
	validarFormulario('aprobar', {
		texto : 'Debe ingresar ',
		otrosCampos : {
			'Email v�lido': [_emailValidos, $('#email').parent()]
		},
		exito : function exito() {
			/*
			 * Ingresar metodo para procesar el formulario aquí Puede ser:
