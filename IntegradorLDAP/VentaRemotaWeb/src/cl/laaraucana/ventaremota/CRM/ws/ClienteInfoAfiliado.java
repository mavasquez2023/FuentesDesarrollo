...
import cl.laaraucana.ventaremota.vo.AnexoAfiliadoVO;
...
public class ClienteInfoAfiliado implements WSInterface {
	...
	public SalidainfoAfiliadoVO getDataAfiliado(String rut) throws AxisFault { 
		...
		try {
			...
			salidaListaVO.setMensaje("Error en servicio INFO_AFILIADO_OUT: compruebe el servicio");
		...
		if(respuesta!=null){
			...
			salidaListaVO.setMensaje("Carga de datos Cliente INFO_AFILIADO_OUT OK. Código error: 0");
		}else{
			salidaListaVO.setCodigoError(ConstantesRespuestasWS.COD_RESPUESTA_ERROR);				
