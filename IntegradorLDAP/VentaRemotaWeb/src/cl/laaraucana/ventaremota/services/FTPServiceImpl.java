...
import cl.laaraucana.ventaremota.ftp.exceptions.ErrorMessage;
import cl.laaraucana.ventaremota.ftp.exceptions.FTPErrors;
...
public class FTPServiceImpl implements FTPService {
	...
	public String downloadFileFromFTP(String ftpRelativePath, String name) throws FTPErrors {
		...
		try {
			...
			logger.info("recibiendo archivo desde " +ftpRelativePath);
			iStream = this.ftpconnection.retrieveFileStream(ftpRelativePath + lfile.get(0).getName());
			logger.info("creando archivo en carpeta local" + path + "\\" + lfile.get(0).getName());
