...
<body>
			...
			<div class="col xs12 lg10">
				<div class="pasos">
					<div class="row justify-center-xs justify-start-sm">
						<div class="col xs12 sm12 md3 oculto-xs block-md">
							<div
				...
				<c:if test="${hide=='show'}">
					<p>
						Ya casi hemos terminado.<br />Por seguridad, le solicitamos
						responder las siguientes preguntas.
					</p>

					<div class="separador"></div>
					<p style="font-size: 1.1em">Dispone de 5 minutos para
						responderlas.</p>
					<br />
				</c:if>
				<form class="form" id="paso_setpreguntas" action="validate.do"
					method="POST">
					<div class="row">
						<c:if test="${hide=='show'}">
							<div class="col xs12 lg12">

								<c:forEach items="${preguntas}" var="pre" varStatus="count">
									<p>${pre.pregunta}</p>
									<br />
									<ul>


										<c:forEach items="${pre.respuesta}" var="res">
											<li><p class="adicional--dos">
													<input type="radio" name="opcion${count.count}"
														id="opcion${count.count}" value="${res.codRespuesta}" />&nbsp;${res.respuesta}
												</p></li>
					...
					<div class="alerta alerta--error" id="errores_preguntas"
						style="display: none"></div>

					<div class="btn__grupo text-align-left-xs">
						<c:if test="${hide=='show'}">
							<a class="btn btn--primario" href="#"
								onClick="validarRespuestas(); return false;">Verificar<br>Resultados
							</a>
						</c:if>
						<c:if test="${respuestasValidas == -1 }">
							<div class="separador"></div>
							<div class="col xs12 md8 lg10">
            					<div class="respuesta text-align-center-xs">
              						<h4 class="respuesta__titulo color--secundario">Se ha equivocado en 2 o más respuestas. Favor reintente.</h4>
              					</div>
            					<div class="btn__grupo text-align-center-xs"><a id="reintentar" class="btn btn--primario" href="#">Reintentar</a>
           					 	</div>
         					 </div>
							
						</c:if>
						
						<c:if test="${respuestasValidas == 0}">
							<div class="separador"></div>
							<div class="col xs12 md8 lg10">
            					<div class="respuesta text-align-center-xs">
              						<h4 class="respuesta__titulo color--secundario">${mesage}</h4>
              					</div>
              					<div class="btn__grupo text-align-center-xs"><a class="btn btn--primario" href="<c:url value='/exit.do' />">Ir al Home</a>
           					 	</div>
         					 </div>
						</c:if>
						...
						<c:if test="${respuestasValidas == 1}">
							...
							<div class="col xs12 md8 lg10">
            					<div class="respuesta text-align-center-xs">
              						<h4 class="respuesta__titulo color--secundario">Sus respuestas no han sido correctas. No puede continuar.</h4>
              					</div>
         					 </div>
						</c:if>
						<c:if test="${respuestasValidas == -9 }">
							<div class="separador"></div>
							<div class="col xs12 md8 lg10">
            					<div class="respuesta text-align-center-xs">
              						<h4 class="respuesta__titulo color--secundario">Ha ocurrido un error. Favor reintente.</h4>
              					</div>
            					<div class="btn__grupo text-align-center-xs"><a id="reintentar_error" class="btn btn--primario" href="#" >Reintentar</a>
           					 	</div>
         					 </div>
						</c:if>
					</div>
				...
				<form id="rechazar-action" action="rechazar.do" method="post">
					<input type="hidden" name="max_intentos" id="max_intentos" value="1"/>
				</form>
	...
	<script>
			$('#reintentar').click(function() {
					fln.preloader(1);
					window.location="<c:url value='/question.do' />";
					
			});
			$('#reintentar_error').click(function() {
					fln.preloader(1);
					window.location="<c:url value='/question.do' />";
