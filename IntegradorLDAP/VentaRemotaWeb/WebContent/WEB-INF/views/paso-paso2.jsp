...
<script type="text/javascript">
	function ValidaDescarga(url){
		window.location=  "<%=request.getContextPath()%>/file.do?id=" + url ;
		$('#validaPDF').val("SI");
	}
</script>
</head>
<body>
	...
	<div class="container">
		...
		<div class="row">
			...
			<div class="col xs12 lg10">
				...
				<c:if test="${vacio == 'SI'}">
					<p>Estimado(a) ${nombre}, no tienes créditos para aprobar el
						día de hoy.</p>
					<p>&nbsp;</p>
					<p>&nbsp;</p>
					<p>&nbsp;</p>
					<div class="btn__grupo text-align-center-xs" id="div-boton"
						style="margin-left: -150px">
						<a class="btn btn--primario" href="<c:url value='/exit.do' />">Finalizar</a>
					</div>
				</c:if>
				<c:if test="${vacio == 'NO'}">
					<div class="col xs12 lg10">
						<c:if test="${tipoCredito!='Reprogramación' && tipoCredito!='Acuerdo de pago'}">
							<p style="text-align: justify;">Estimado(a) ${nombre}, RUT
								${rutUser}, le solicitamos revisar la documentación del crédito
								social que está solicitando.</p>
						</c:if>
						<c:if test="${tipoCredito=='Reprogramación' || tipoCredito=='Acuerdo de pago'}">
							<p style="text-align: justify;">Estimado(a) ${nombre}, RUT
								${rutUser}, le
								solicitamos revisar la documentación de la reprogramación que
								está solicitando</p>
						</c:if>
						<div class="separador"></div>

						<form class="form" id="paso2" action="paso3.do" method="POST">
							<p style="text-align: left;">Selecciona tipo de operación y descarga el <b>PDF</b> con la documentación</p>
							<table align="center" class="tabla-creditos">
								<thead>
									<tr>
										<th class="certificadoLeft" colspan="2">Tipo operación</th>
										<th class="certificadoLeft">Fecha</th>
										<th class="certificadoLeft">Monto</th>
										<th class="certificadoLeft">Cuota</th>
										<th class="certificadoLeft">Plazo</th>
										<th class="certificadoLeft">PDF</th>

									</tr>
								</thead>
								<tbody>
									<c:forEach items="${lista_creditos}" var="cre">
										<tr>
											<td class="certificadoLeft" style="text-align: center;"><input
												type="radio" id="credito" name="credito"
												value="${cre.idCredito}"></td>
											<td class="certificadoLeft">${cre.tipoCreditoNormal}</td>
											<td class="certificadoLeft">${cre.fechaOtorgamiento}</td>
											<td class="certificadoLeft">$${cre.montoSolicitado}</td>
											<td class="certificadoLeft">$${cre.montoCuota}</td>
											<td class="certificadoCenter">${cre.numeroCuotas}</td>
											<td class="certificadoCenter">
													<a href="#" onclick="ValidaDescarga('${cre.url}');return false;">
														<img src="<c:url value='/resources/img/descarga.png'/>"
															width="20px" height="20px" title="Descargar"
															alt="Descargar" />
													</a>
											</td>
							...
							<p style="text-align: justify;">
								<b>Por favor confirmar que recibió y leyó los siguientes
									documentos</b><br /> Marque cada uno y seleccione "Confirmo
								recepción y lectura".
							</p>
							...
							<c:if test="${tipoCredito!='Reprogramación' && tipoCredito!='Acuerdo de pago'}">
								...
								<table style="border: hidden">
									<tr style="border: hidden">
										<td style="border: hidden;"><input class="claseCheckBox"
											type="checkbox" id="check1"></td>
										<td style="border: hidden" class="certificadoLeft">Mandato
											único para suscribir Pagaré</td>
									</tr>
									<tr style="border: hidden">
										<td style="border: hidden"><input class="claseCheckBox"
											type="checkbox" id="check2"></td>
										<td style="border: hidden" class="certificadoLeft">Hoja
											Resumen y Contrato crédito social</td>
									</tr>
									<tr style="border: hidden">
										<td style="border: hidden"><input class="claseCheckBox"
											type="checkbox" id="check3"></td>
										<td style="border: hidden" class="certificadoLeft">Solicitud
											de crédito social</td>
									</tr>
									<tr style="border: hidden">
										<td style="border: hidden"><input class="claseCheckBox"
											type="checkbox" id="check4"></td>
										<td style="border: hidden" class="certificadoLeft">Contratación
											Seguro Desgravamen</td>
									</tr>
									<c:if test="${cesantia != 0 }">
										<tr style="border: hidden">
											<td style="border: hidden"><input class="claseCheckBox"
												type="checkbox" id="check5"></td>
											<td style="border: hidden" class="certificadoLeft">Contratación
												Seguro Cesantía</td>
										</tr>
									</c:if>
									<c:if test="${formaPago != 'Cheque' }">
									<tr style="border: hidden">
										<td style="border: hidden"><input class="claseCheckBox"
											type="checkbox" id="check6"></td>
										<td style="border: hidden" class="certificadoLeft">Mandato
											único para transferencia electrónica</td>
									</tr>
									</c:if>
									<c:if test="${contratoCR == 'NO' }">
									<tr style="border: hidden">
										<td style="border: hidden"><input class="claseCheckBox"
											type="checkbox" id="check7"></td>
										<td style="border: hidden" class="certificadoLeft">Contrato
											de Uso de Canales Remotos</td>
									</tr>
									</c:if>
									<c:if test="${deudorAlimenticio == 'SI' }">
									<tr style="border: hidden">
										<td style="border: hidden"><input class="claseCheckBox"
											type="checkbox" id="check8"></td>
										<td style="border: hidden" class="certificadoLeft">Registro
											Deudores Alimenticios</td>
									</tr>
									</c:if>
									<c:if test="${montoCC != '0' }">
									<tr style="border: hidden">
										<td style="border: hidden"><input class="claseCheckBox"
											type="checkbox" id="check9"></td>
										<td style="border: hidden" class="certificadoLeft">Mandato de Pago a Terceros</td>
									</tr>
									</c:if>
									<tr style="border: hidden">
										<td style="border: hidden"><input class="claseCheckBox"
											type="checkbox" id="check10"></td>
										<td style="border: hidden" class="certificadoLeft">Declaración 
										juarada de vigencia laboral</td>
									</tr>
									<tr style="border: hidden">
										<td style="border: hidden"><input class="claseCheckBox"
											type="checkbox" id="check11"></td>
										<td style="border: hidden" class="certificadoLeft">Informe
											de crédito social</td>
									</tr>
								</table>
							</c:if>
							<c:if test="${tipoCredito=='Acuerdo de pago'}">
								<table style="border: hidden">
									<tr style="border: hidden">
										<td style="border: hidden"><input class="claseCheckBox"
											type="checkbox" id="check1"></td>
										<td style="border: hidden" class="certificadoLeft">Acuerdo
											de Pago</td>
									</tr>
									<c:if test="${contratoCR == 'NO' }">
									<tr style="border: hidden">
							...
							<c:if test="${tipoCredito=='Reprogramación'}">
								<table style="border: hidden">
									<tr style="border: hidden">
										<td style="border: hidden"><input class="claseCheckBox"
											type="checkbox" id="check1"></td>
									...
									<c:if test="${tipoReprogramacion=='1'}">
									<tr style="border: hidden">
										<td style="border: hidden"><input class="claseCheckBox"
											type="checkbox" id="check2"></td>
										<td style="border: hidden" class="certificadoLeft">Contratación
							...
							<div class="col xs12 lg8">
								...
								<div class="btn__grupo text-align-center-xs" id="div-boton"
									...
									<a class="btn btn--primario" href="#"
										onClick="validarPaso2(); return false;">Acepto términos y condiciones</a>
								</div>
							</div>
							<input type="hidden" name="validaPDF" id="validaPDF" value="" />
							<input type="hidden" name="ipremota" id="ipremota" value="<%=request.getRemoteAddr() %>" />
						</form>
					</div>
				</c:if>
			</div>
	...
	<script type="text/javascript">
		//	$(document).on("click", '.container input', function() {
		//		$('#div-boton').show();
		//	});
	</script>
	<script>
		document.addEventListener("DOMContentLoaded", function(e) {
			var allChecksBoxes = document
					.querySelectorAll("input[type='checkbox'].claseCheckBox");
