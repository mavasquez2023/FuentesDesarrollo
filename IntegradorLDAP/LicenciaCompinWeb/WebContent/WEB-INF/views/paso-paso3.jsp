<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%>
<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	pageEncoding="ISO-8859-1"%>
<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="UTF-8" />
<meta name="viewport"
	content="width=device-width, initial-scale=1, viewport-fit=cover" />
<meta http-equiv="X-UA-Compatible" content="ie=edge" />
<meta name="format-detection" content="telephone=no" />
<title>Subir Licencia - La Araucana</title>
<link rel="stylesheet" href="assets_file/css/fln.css" />
<link rel="stylesheet" href="fonts/fln-icons.css" />
<link rel="stylesheet" href="assets_file/css/estilos.css" />

<script>
	WebFontConfig = {
		google : {
			families : [ 'Open Sans:300,400,500,700' ]
		}
	};
	(function() {
		var wf = document.createElement('script');
		wf.src = ('https:' == document.location.protocol ? 'https' : 'http')
				+ '://ajax.googleapis.com/ajax/libs/webfont/1.5.18/webfont.js';
		wf.type = 'text/javascript';
		wf.async = 'true';
		var s = document.getElementsByTagName('script')[0];
		s.parentNode.insertBefore(wf, s);
	})();
</script>

</head>
<body>
	<main>
	<div class="container">
		<div align="right">
			<a href="<c:url value='/exit.do' />"  style="text-decoration:none"><img alt="Salir" src="img/salir.png"  title="Cerrar Sesión" /><span class="pasos__data" style="font-size:1.6rem;color:gray;">&nbsp;Salir</span></a>
		</div>

		<div class="row">
			<div class="col xs12 lg2 text-align-center-xs">
				<svg width="155.30017318187797" height="139.99975464130628" xmlns="http://www.w3.org/2000/svg" class="svg svg--secundario">

 <g>
  <title>Subir Licencia - La Araucana</title>
  <rect height="402" width="582" y="-1" x="-1" fill="none" id="canvas_background"/>
 </g>
 <g>
  <title>Subir Licencia - La Araucana</title>
  <path d="m77.699997,85.400002c23.599998,0 42.700005,-19.099998 42.700005,-42.700001s-19.200005,-42.700001 -42.700005,-42.700001s-42.799995,19.1 -42.799995,42.700001s19.199997,42.700001 42.799995,42.700001zm0,-77.600001c19.299995,0 34.900002,15.600001 34.900002,34.900001s-15.599998,34.999996 -34.900002,34.999996s-34.999996,-15.699997 -34.999996,-34.999996c0,-19.300001 15.700001,-34.900001 34.999996,-34.900001z" id="svg_1"/>
  <path d="m135.600006,96.800003l0,0c-37.200005,-15 -78.800003,-15 -116,0l0,0c-11.900006,4.899994 -19.600006,15.199997 -19.600006,26.599998l0,16.599998l155.300003,0l0,-16.599998c0,-11.400002 -7.699997,-21.700005 -19.699997,-26.599998zm11.899994,35.399994l-139.7,0l0,-8.900002c0,-8.099998 5.8,-15.699997 14.8,-19.299995c35.300001,-14.200005 74.800001,-14.200005 110.200003,0c9,3.599998 14.799988,11.199997 14.799988,19.299995l0,8.900002l-0.099991,0z" id="svg_2"/>
  <line y2="127.31" x2="106.795" y1="105.73" x1="106.795" stroke-width="8" stroke="#bf0000" fill="none" id="svg_5"/>
  <line y2="116.255" x2="117.825" y1="116.255" x1="95.7204" stroke-width="8" stroke="#bf0000" fill="none" id="svg_6"/>
  <polyline points="74.6904,26.7804 74.6904,26.7804" stroke-width="7" stroke-linecap="round" fill="none" id="svg_10"/>
  <polyline points="49.4304,21.5154 49.4304,21.5154" stroke-width="7" stroke-linecap="round" stroke="#000a0a" fill="none" id="svg_20"/>
  <ellipse ry="20.5249" rx="20.5275" cy="20.9649" cx="48.9029" stroke-width="9" fill="#0f0f00" id="svg_34"/>
  <ellipse ry="9.24249" rx="8.96572" cy="20.6879" cx="48.9012" stroke-width="8" stroke="#f2f7f7" fill="none" id="svg_22"/>
 </g>
</svg>

			</div>
			<div class="col xs12 lg10">
				<div class="pasos">
					<div class="row justify-center-xs justify-start-sm">
						<div class="col xs12 sm12 md4 oculto-xs block-md">
							<div
								class="pasos__item flex-xs align-middle-xs justify-center-xs justify-start-md">
								<div class="pasos__icono">
									<svg width="59.999940559276474" height="59.999940559276474" xmlns="http://www.w3.org/2000/svg" class="svg">
 									<g>
  										<title>PASO 1</title>
  										<rect height="402" width="582" y="-1" x="-1" fill="none" id="canvas_background"/>
 									</g>
 									<g>
  										<title>Paso 1</title>
  										<rect height="60" width="60" y="0.002493" x="-0.012496" id="svg_1" class="svg__fondo"/>
  										<path d="m53.400002,12.3l-46.800002,0c-2.3,0 -4.1,1.8 -4.1,4.099999l0,27.199999c0,1.100002 0.4,2.100002 1.2,2.900002c0.8,0.799999 1.8,1.200001 2.9,1.200001l46.800002,0c2.299999,0 4.099998,-1.799999 4.099998,-4.100002l0,-27.199999c0,-2.2 -1.799999,-4.099999 -4.099998,-4.099999zm-46.800002,2.4l46.800002,0c0.899998,0 1.699997,0.8 1.699997,1.7l0,27.199999c0,0.900002 -0.799999,1.700001 -1.699997,1.700001l-46.800002,0c-0.9,0 -1.7,-0.799999 -1.7,-1.700001l0,-27.199999c0,-0.9 0.7,-1.7 1.7,-1.7z" id="svg_2"/>
 										<path d="m29.700001,26l20.700001,0c0.599998,0 1.199997,-0.5 1.199997,-1.200001c0,-0.599998 -0.5,-1.199999 -1.199997,-1.199999l-20.700001,0c-0.6,0 -1.200001,0.5 -1.200001,1.199999c0,0.300001 0.1,0.6 0.299999,0.800001c0.300001,0.299999 0.6,0.4 0.900002,0.4z" id="svg_3"/>
 										<path d="m29.700001,36.949921l20.700001,0c0.599998,0 1.199997,-0.499996 1.199997,-1.199993c0,-0.599995 -0.5,-1.199993 -1.199997,-1.199993l-20.700001,0c-0.6,0 -1.200001,0.499996 -1.200001,1.199993c0,0.299995 0.1,0.599995 0.299999,0.799992c0.300001,0.200001 0.6,0.400002 0.900002,0.400002z" id="svg_4"/>
  										<path d="m16.299999,28.800018c2.6,0 4.700001,-2.099998 4.700001,-4.699999c0,-0.200001 0,-0.300001 0,-0.5l0,0c0,-0.200001 -0.1,-0.5 -0.1,-0.700001c-0.1,-0.199999 -0.1,-0.4 -0.199999,-0.699999c-0.700001,-1.700001 -2.400002,-2.900002 -4.300001,-2.900002c-1.9,0 -3.599999,1.1 -4.299999,2.900002c-0.1,0.199999 -0.200001,0.4 -0.200001,0.699999c-0.099999,0.200001 -0.099999,0.5 -0.099999,0.700001c0,0.199999 0,0.299999 0,0.5c-0.2,2.6 1.9,4.699999 4.499999,4.699999zm-1.9,-7c0.1,-0.099998 0.3,-0.199999 0.5,-0.299999c0.1,0 0.200001,-0.1 0.3,-0.1c0.2,-0.1 0.400001,-0.1 0.5,-0.199999c0.2,0 0.400001,-0.1 0.599999,-0.1c1.6,0 2.900002,1.299999 2.900002,2.9c0,0.200001 0,0.4 -0.1,0.6c0,0.199999 -0.1,0.4 -0.200001,0.5c0,0.1 -0.1,0.199999 -0.1,0.299999l0,0c-0.099998,0.200001 -0.199999,0.300001 -0.299999,0.5c-0.6,0.700001 -1.4,1.1 -2.299999,1.1c-0.200001,0 -0.400001,0 -0.6,-0.1c-0.200001,0 -0.400001,-0.1 -0.5,-0.199999c-0.1,0 -0.200001,-0.1 -0.3,-0.1c-0.2,-0.1 -0.3,-0.200001 -0.5,-0.300001c-0.1,-0.099998 -0.3,-0.299999 -0.400001,-0.4c-0.099999,-0.1 -0.2,-0.299999 -0.299999,-0.5c0,-0.1 -0.1,-0.199999 -0.1,-0.299999c-0.1,-0.200001 -0.1,-0.4 -0.2,-0.5c0,-0.200001 -0.1,-0.4 -0.1,-0.6c0.2,-0.799999 0.5,-1.6 1.2,-2.200001z" id="svg_6"/>
  										<path d="m16.299999,40.21999c2.6,0 4.700001,-2.099998 4.700001,-4.700001c0,-0.199997 0,-0.299999 0,-0.5l0,0c0,-0.199997 -0.1,-0.5 -0.1,-0.699997c-0.1,-0.200001 -0.1,-0.400002 -0.199999,-0.700001c-0.700001,-1.699999 -2.400002,-2.899998 -4.300001,-2.899998c-1.9,0 -3.599999,1.1 -4.299999,2.899998c-0.1,0.200001 -0.200001,0.399998 -0.200001,0.700001c-0.099999,0.199997 -0.099999,0.5 -0.099999,0.699997c0,0.200001 0,0.300003 0,0.5c-0.2,2.600002 1.9,4.700001 4.499999,4.700001zm-1.9,-7c0.1,-0.099998 0.3,-0.200001 0.5,-0.299999c0.1,0 0.200001,-0.099998 0.3,-0.099998c0.2,-0.100002 0.400001,-0.100002 0.5,-0.200001c0.2,0 0.400001,-0.100002 0.599999,-0.100002c1.6,0 2.900002,1.300003 2.900002,2.900002c0,0.200001 0,0.400002 -0.1,0.599998c0,0.200001 -0.1,0.400002 -0.200001,0.5c0,0.100002 -0.1,0.200001 -0.1,0.300003l0,0c-0.099998,0.199997 -0.199999,0.299999 -0.299999,0.5c-0.6,0.699997 -1.4,1.099998 -2.299999,1.099998c-0.200001,0 -0.400001,0 -0.6,-0.099998c-0.200001,0 -0.400001,-0.100002 -0.5,-0.200001c-0.1,0 -0.200001,-0.100002 -0.3,-0.100002c-0.2,-0.099998 -0.3,-0.199997 -0.5,-0.299999c-0.1,-0.099998 -0.3,-0.299999 -0.400001,-0.399998c-0.099999,-0.100002 -0.2,-0.300003 -0.299999,-0.5c0,-0.100002 -0.1,-0.200001 -0.1,-0.300003c-0.1,-0.199997 -0.1,-0.399998 -0.2,-0.5c0,-0.199997 -0.1,-0.399998 -0.1,-0.599998c0.2,-0.799999 0.5,-1.599998 1.2,-2.200001z" id="svg_8"/>
 									</g>
									</svg>
								</div>
								<div class="pasos__data">
									<span>Paso 1</span>SELECCIONA OPCIÓN
								</div>
							</div>
						</div>
						<div class="col xs12 sm12 md4 oculto-xs block-md">
							<div
								class="pasos__item flex-xs align-middle-xs justify-center-xs justify-start-md">
								<div class="pasos__icono">
									<svg class="svg" xmlns="http://www.w3.org/2000/svg" width="60"
										height="60" viewBox="0 0 60 60">
										<rect class="svg__fondo" width="60" height="60" />
										<path
											d="M49.4 10.6C44.3 5.4 37.3 2.5 30 2.5c-7.3 0-14.3 2.9-19.4 8.1C5.4 15.7 2.5 22.7 2.5 30s2.9 14.3 8.1 19.4c5.2 5.2 12.1 8.1 19.4 8.1 7.3 0 14.3-2.9 19.4-8.1s8.1-12.1 8.1-19.4S54.6 15.7 49.4 10.6zM30 54.3C16.6 54.3 5.7 43.4 5.7 30S16.6 5.7 30 5.7 54.3 16.6 54.3 30 43.4 54.3 30 54.3z" />
										<path
											d="M40.9 21.2L26.6 35.4l-7.5-7.5c-0.6-0.6-1.6-0.6-2.3 0 -0.6 0.6-0.6 1.6 0 2.3l8.6 8.6c0.3 0.3 0.7 0.5 1.1 0.5 0.4 0 0.8-0.2 1.1-0.5l15.4-15.4c0.6-0.6 0.6-1.6 0-2.3C42.5 20.5 41.5 20.5 40.9 21.2z" /></svg>
								</div>
								<div class="pasos__data">
									<span>Paso 2</span>INGRESA TUS DATOS
								</div>
							</div>
						</div>
						<div class="col xs12 sm12 md4">
							<div
								class="pasos__item flex-xs align-middle-xs justify-center-xs justify-start-md activo">
								<div class="pasos__icono">
									<svg xmlns="http://www.w3.org/2000/svg" class="svg__fondo"
										width="55" height="60" viewBox="0 0 374.3 315">
										<defs>
										<rect width="374.3" height="315" /></defs>
										<clipPath>
										<use xlink:href="#SVGID_1_" /></clipPath>
										<path class="st0"
											d="M340.8 57h-73.5V30c0-16.5-13.4-30-30-30H137c-16.5 0-30 13.4-30 30v27H33.5C15 57 0 72 0 90.5v191C0 300 15 315 33.5 315h307.3c8.9 0 17.4-3.5 23.7-9.8 6.3-6.3 9.8-14.8 9.8-23.7v-191C374.3 72 359.3 57 340.8 57M245.3 30v27H129V30c0-4.4 3.6-8 8-8h100.3C241.7 22 245.3 25.6 245.3 30M352.4 90.5v25l-165.2 58.9L22 115.6V90.5c0-6.4 5.2-11.6 11.6-11.6h307.3C347.2 78.9 352.4 84.1 352.4 90.5M195.9 204.3h-17.4v-9.7l5.1 1.8c2.4 0.8 5 0.8 7.4 0l5-1.8V204.3zM22 138.9l134.5 47.8v28.5c0 6.1 4.9 11 11 11h39.4c6.1 0 11-4.9 11-11v-28.5l134.5-48v142.7c0 6.4-5.2 11.6-11.6 11.6H33.5c-6.4 0-11.6-5.2-11.6-11.6V138.9z" /></svg>
								</div>
								<div class="pasos__data">
									<span>Paso 3</span>ENVÍA TU LICENCIA
								</div>
							</div>
						</div>
					</div>
				</div>
				<p style="margin-top: 10px">Puedes subir imágenes o documentos, escoge "Licencia maternal" si la licencia es por tal motivo.</p>
				<form class="form" id="paso3" action="files.do" method="POST"
					enctype="multipart/form-data" style="margin-top: 10px">

					<div class="row">
						<div class="col xs12 lg12">
							<div class="alerta alerta--info" id="aviso"
								style="display: none; margin-top: -10px"></div>
						</div>
						<div class="col xs12 lg6"></div>
						<div class="col xs12 lg6">
							<div class="form__seleccion" style="margin-bottom: -5px">
								<div class="form__checkbox">
									<input type="checkbox" name="maternal" id="maternal" /> <label
										for="maternal">Licencia maternal</label>
								</div>
							</div>
						</div>
						<div class="col xs12 lg3">
								<div class="documento__item">
									<span class="documento__tipo"></span><span
										class="documento__nombre"><span
										class="documento__nombre__text">Adjuntar Licencia&nbsp;&nbsp;</span><span
										class="documento__icono fln-subir-light"></span><a
										class="documento__eliminar" href="#"
										onclick="limpiar(this); return false;"><span>+</span></a></span> <input
										class="documento__input" type="file" id="medica" name="medica"
										data-maxsize="5000"
										accept="image/jpg,image/png,image/jpeg,application/pdf,.doc,.docx,application/msword" />
								</div>
								<div class="ayuda ayuda--form">
									<span style="color: red">?</span>
									<div class="ayuda__container" style="max-width:600px;left:320px;top:-95px">
										<p>Recuerda que la imagen debe ser clara, legible y completa y por ambas caras de la licencia 
											La parte del diagnóstico debe venir visible.</p>
										<div class="separador--small"></div>
										<a href="img/Ejmplo_Licencia_Trabajador.png" target="_blank"><img
											src="img/Ejmplo_Licencia_Trabajador.png" alt="Licencia Trabajador" /> </a>
									</div>
								</div>
								<p id="file_error1"></p>
						</div>
						<div class="col xs12 lg3">
							<div class="form__grupo" data-animacion="placeholder"
								data-comentario="">
								<p class="adicional adicional--dos">Ver ejemplo a subir</p>
							</div>								
						</div>
						<div class="col xs12 lg6" style="margin-bottom: -15px">
							<div class="form__grupo" data-animacion="placeholder"
								data-comentario="">
								<p class="adicional adicional--dos">Sube tu licencia médica ABIERTA, con las firmas del doctor y tuya, con el diagnostico a la vista y legible (peso máximo del archivo 5M)</p>
								<label for="celular"></label>
							</div>
						</div>
						<div class="col xs12 lg3">
								<div class="documento__item">
									<span class="documento__tipo"></span><span
										class="documento__nombre"><span
										class="documento__nombre__text">Adjuntar ZonaC&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span><span
										class="documento__icono fln-subir-light"></span><a
										class="documento__eliminar" href="#"
										onclick="limpiar(this); return false;"><span>+</span></a></span> <input
										class="documento__input" type="file" id="zonac" name="zonac"
										data-maxsize="5000"
										accept="image/jpg,image/png,image/jpeg,application/pdf,.doc,.docx,application/msword" />
								</div>
								<div class="ayuda ayuda--form">
									<span style="color: red">?</span>
									<div class="ayuda__container" style="max-width:500px;left:280px;top:-360px">
										<p>Recuerda que la imagen debe ser clara, legible y completa, rellenada y firmada por tu empleador</p>
										<div class="separador--small"></div>
										<a href="img/ZonaC.png" target="_blank"><img
											src="img/ZonaC.png" alt="ZonaC Licencia" /> </a>
									</div>
								</div>
								<p id="file_errorC"></p>
						</div>
						<div class="col xs12 lg3">
							<div class="form__grupo" data-animacion="placeholder"
								data-comentario="">
								<p class="adicional adicional--dos">Ver ejemplo a subir</p>
							</div>								
						</div>
						<div class="col xs12 lg6" style="margin-bottom: -15px">
							<div class="form__grupo" data-animacion="placeholder"
								data-comentario="">
								<p class="adicional adicional--dos">Sube la parte trasera de la licencia ( Zona C completa ) rellenada por tu empleador (peso máximo del archivo 5M)</p>
								<label for="celular"></label>
							</div>
						</div>
						<div class="col xs12 lg6">
								<div class="documento__item">
									<span class="documento__tipo"></span><span
										class="documento__nombre"><span
										class="documento__nombre__text" style="font-size: 1.2em">Adjuntar Liquidación 1</span><span
										class="documento__icono fln-subir-light"></span><a
										class="documento__eliminar" href="#"
										onclick="limpiar(this); return false;"><span>+</span></a></span> <input
										class="documento__input" type="file" id="renta1" name="renta1"
										data-maxsize="5000"
										accept="image/jpg,image/png,image/jpeg,application/pdf,.doc,.docx,application/msword" />
								</div>
								<p id="file_error2"></p>
						</div>
						<div class="col xs12 lg6">
							<div class="form__grupo" data-animacion="placeholder"
								data-comentario="">
								<p class="adicional adicional--dos">Sube tu Liquidación 1 (peso máximo del archivo 5M)</p>

							</div>
						</div>
						<div class="col xs12">
							<div class="grupo-emails">
								<div class="row">
									<div class="col xs12 lg6">
											<div class="documento__item">
												<span class="documento__tipo"></span><span
													class="documento__nombre"><span
													class="documento__nombre__text" style="font-size: 1.2em">Adjuntar Liquidación
														2</span><span class="documento__icono fln-subir-light"></span><a
													class="documento__eliminar" href="#"
													onclick="limpiar(this); return false;"><span>+</span></a></span> <input
													class="documento__input" type="file" id="renta2"
													name="renta2" data-maxsize="5000"
													accept="image/jpg,image/png,image/jpeg,application/pdf,.doc,.docx,application/msword" />
											</div>
											<p id="file_error3"></p>
									</div>
									<div class="col xs12 lg6">
										<div class="form__grupo" data-animacion="placeholder"
											data-comentario="">
											<p class="adicional adicional--dos">Sube tu Liquidación 2 (peso máximo del archivo 5M)</p>

										</div>
									</div>

									<div class="col xs12 lg6">
											<div class="documento__item">
												<span class="documento__tipo"></span><span
													class="documento__nombre"><span
													class="documento__nombre__text" style="font-size: 1.2em">Adjuntar Liquidación
														3</span><span class="documento__icono fln-subir-light"></span><a
													class="documento__eliminar" href="#"
													onclick="limpiar(this); return false;"><span>+</span></a></span> <input
													class="documento__input" type="file" id="renta3"
													name="renta3" data-maxsize="5000"
													accept="image/jpg,image/png,image/jpeg,application/pdf,.doc,.docx,application/msword" />
											</div>
											<p id="file_error4"></p>
									</div>
									<div class="col xs12 lg6">
										<div class="form__grupo" data-animacion="placeholder"
											data-comentario="">
											<p class="adicional adicional--dos">Sube tu Liquidación 3 (peso máximo del archivo 5M)</p>
											<label for="confirmar-email"></label>
										</div>
									</div>
								</div>
							</div>
						</div>
						<div class="maternal-item" id="maternal-item"
							style="display: none">
							<div class="col xs12">
								<div class="grupo-emails">
									<div class="row">
										<div class="col xs12 lg6">
												<div class="documento__item">
													<span class="documento__tipo"></span><span
														class="documento__nombre"><span
														class="documento__nombre__text" style="font-size: 1.2em"> Adjuntar
															Liquidación 4</span><span
														class="documento__icono fln-subir-light"></span><a
														class="documento__eliminar" href="#"
														onclick="limpiar(this); return false;"><span>+</span></a></span>
													<input class="documento__input" type="file" id="renta4"
														name="renta4" data-maxsize="5000"
														accept="image/jpg,image/png,image/jpeg,application/pdf,.doc,.docx,application/msword" />
												</div>
												<p id="file_error5"></p>
										</div>
										<div class="col xs12 lg6">
											<div class="form__grupo" data-animacion="placeholder"
												data-comentario="">
												<p class="adicional adicional--dos">Sube tu Liquidación 4 (peso máximo del archivo 5M)</p>
												<label for="confirmar-email"></label>
											</div>
										</div>
										<div class="col xs12 lg6">
												<div class="documento__item">
													<span class="documento__tipo"></span><span
														class="documento__nombre"><span
														class="documento__nombre__text" style="font-size: 1.2em"> Adjuntar
															Liquidación 5</span><span
														class="documento__icono fln-subir-light"></span><a
														class="documento__eliminar" href="#"
														onclick="limpiar(this); return false;"><span>+</span></a></span>
													<input class="documento__input" type="file" id="renta5"
														name="renta5" data-maxsize="5000"
														accept="image/jpg,image/png,image/jpeg,application/pdf,.doc,.docx,application/msword" />
												</div>
												<p id="file_error6"></p>
										</div>
										<div class="col xs12 lg6">
											<div class="form__grupo" data-animacion="placeholder"
												data-comentario="">
												<p class="adicional adicional--dos">Sube tu Liquidación 5 (peso máximo del archivo 5M)</p>
												<label for="confirmar-email"></label>
											</div>
										</div>
										<div class="col xs12 lg6">
												<div class="documento__item">
													<span class="documento__tipo"></span><span
														class="documento__nombre"><span
														class="documento__nombre__text" style="font-size: 1.2em"> Adjuntar
															Liquidación 6</span><span
														class="documento__icono fln-subir-light"></span><a
														class="documento__eliminar" href="#"
														onclick="limpiar(this); return false;"><span>+</span></a></span>
													<input class="documento__input" type="file" id="renta6"
														name="renta6" data-maxsize="5000"
														accept="image/jpg,image/png,image/jpeg,application/pdf,.doc,.docx,application/msword" />
												</div>
												<p id="file_error7"></p>
										</div>
										<div class="col xs12 lg6">
											<div class="form__grupo" data-animacion="placeholder"
												data-comentario="">
												<p class="adicional adicional--dos">Sube tu Liquidación 6 (peso máximo del archivo 5M)</p>
												<label for="confirmar-email"></label>
											</div>
										</div>
									</div>
								</div>




							</div>

							
						</div>
						<div class="separador"></div>
						<div class="col xs12 lg12">
							<div class="form__grupo" data-animacion="placeholder"
								data-comentario="">
								<p class="adicional" style="margin-top: 15px">Para información e instrucciones de uso puedes descargar instructivo 
								<a style="color:#2072BC" href="<c:url value='/img/Indicaciones_Licencia.pdf' />" target="_blank">descargar aquí</a></p>
							</div>								
						</div>
						<div class="col xs12 lg12">
							<div class="alerta alerta--aviso" id="aviso2"
								style="display: none;position:absolute; left: 500px; top: -210px;""></div>
						</div>
						<div class="col xs6 lg12">
							<div class="form__seleccion" style="margin-top: -10px">
								<div class="form__checkbox">
									<input type="checkbox" name="terminos" id="terminos" /> <label
										for="terminos"><span style="font-size: 0.8em;text-align:justify;">
											Autorizo a C.C.A.F. La Araucana a utilizar los datos
											contenidos en la licencia médica que se adjunta y realizar
											los trámites necesarios para la validación en la Compin</span></label>
								</div>
							</div>
						</div>
						<div class="col xs6 lg16"></div>
						<div class="col xs6 lg16">
							<div class="btn__grupo text-align-left-xs" style="margin-right: 100px">
								<a class="btn btn--secundario" href="#"
									onClick="volverPaso2(); return false;">Volver</a>

								<a id="enviar-falso" class="btn btn--primario"
									href="#">Continuar</a> <a id="enviar"
									class="btn btn--primario" href="#"
									onClick="validarPaso3(); return false;" style="display: none">Continuar</a>
							</div>
						</div>
						
					</div>
					<c:if test='${error=="error-files"}'>
						<div class="alerta alerta--error">${errorMesj}</div>
					</c:if>
					<div class="alerta alerta--error" id="errores_paso2"
						style="display: none"></div>
				</form>
				<form class="form" id="paso2" action="volver_paso2.do" method="POST">
				 <input type="hidden" name="opcion" id="opcion"  value="IL" />
				</form>
					
				
			</div>
		</div>
	</main>
	 <div class="preloader-general" id="preloader-general" data-tipo="screen" style="display:none"></div>
	<script src="assets/js/polyfill.js"></script>
	<script src="assets/js/jquery-3.3.1.js"></script>
	<script src="assets/js/jquery-ui.js"></script>
	<script src="assets/js/automatizar.js"></script>
	<script src="assets/js/fln.js"></script>
	<script src="assets/js/funciones.js"></script>
	<script type="text/javascript">
		$('#maternal').click(function() {

			if ($('#maternal').is(':checked')) {

				$('#maternal-item').show();
			} else {
				$('#maternal-item').hide();
			}

		});
	</script>
	<script>
		avisar('<b><h5>Asegúrate de enviarnos:</h5></b>'
		+ '<ul style="margin-top:5px"><li>&nbsp;&nbsp;&nbsp;&nbsp;<img src="img/check.png" style="width:18px;height:18px;" />&nbsp;<b>Información correcta y legible</b></li>'
		+'<li style="margin-top:5px">De lo contrario <b>no</b> podremos <b>tramitar y pagar</b> tu licencia médica.</li></ul>'
		, 6000);
	</script>
	<script>
		$( "#enviar-falso" ).mouseover(function() {
  			avisar2('<b><h3>Antes de continuar chequea:</h3></b><br>'
		+ '<b><ul><li><img src="img/check.png" style="width:18px;height:18px;" />&nbsp;Licencia médica con firmas del médico y trabajador</li>'
		+ '<li><img src="img/check.png" style="width:18px;height:18px;" />&nbsp;El diagnóstico debe ser visible</li>'
		+ '<li><img src="img/check.png" style="width:18px;height:18px;" />&nbsp;Sección C de la licencia completada por empleador</li></ul></b>'
		, 10000);
		$( "#enviar-falso" ).hide();
		setTimeout('$( "#enviar" ).show();', 4000);
		
		
		});		

	</script>
</body>
</html>