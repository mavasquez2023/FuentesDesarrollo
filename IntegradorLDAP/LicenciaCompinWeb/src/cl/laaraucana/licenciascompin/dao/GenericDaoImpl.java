...
public class GenericDaoImpl<T extends Serializable> implements GenericDao<T> {
	...
	public int UpdateByEstado(Class<T> clazz, List<Long> lista ) {
		...
		return em
				.createQuery("update " + clazz.getName() + " c set c.estado='1' where c.id in (:ids) ")
				.setParameter("ids", lista).executeUpdate();
	}
	...
	public List<T> getAllByEstado(Class<T> clazz) {
		...
		return em
				.createQuery("select c from " + clazz.getName() + " c where c.estado = :estado order by ID ")
				.setParameter("estado", "0").getResultList();
	}
