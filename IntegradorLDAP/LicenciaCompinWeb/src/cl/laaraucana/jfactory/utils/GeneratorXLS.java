...
package cl.laaraucana.jfactory.utils;
...
public class GeneratorXLS {
	...
	public GeneratorXLS(String pathfile) throws FileNotFoundException{
			...
			for (Iterator iterator = objs.iterator(); iterator.hasNext();) {
				...
				for (int i = 0; i < metodos.length; i++) {
					if(metodos[i]!=null){
						...
						salidatemp.append("<td align='" + returnAlign(columnas[i]) + "'>" + columnas[i] + "</td>");
					}
				}
				salidatemp.append("</tr>");
				if(condicion){
...
//				Se rescata los títulos de las cabeceras de las columnas
				...
				for (int i = 0; i < metodos.length; i++) {
					if(metodos[i]!=null){
						...
						salida.print(columnas[i] + ";");
					}
				}
				salida.println("");
				numobj++;
			}
			if(objs.size()==0 && titulos!= null){
				for (int j= 0; j < titulos.length; j++ ) {
					//se rescata nombre de las columnas
					salida.print(titulos[j] + ";");
				}
