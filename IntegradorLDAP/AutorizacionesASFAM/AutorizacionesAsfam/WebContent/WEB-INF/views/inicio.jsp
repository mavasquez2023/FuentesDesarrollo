...
<title>Autorizaciones Asfam PDF -  La Araucana</title>
...
<body>
	...
	<div class="container">
		<h2>Autorizaciones Asfam PDF</h2>
		<br>
				...
				<tbody>
					<c:forEach items="${cabeceras}" var="cab" varStatus="numreg">
						<tr>
							...
							<td>${cab.rutAfiliado}-${cab.dvAfiliado}</td>
							<td>${cab.nombreAfiliado}</td>
							<td>${cab.oficina}</td>
							<td>${cab.sucursal}</td>
							<td><a 
								href="<c:url
							value='/reporteAUT.do?rutemp=${cab.rutEmpresa}&periodo=${cab.periodo}&rutafi=${cab.rutAfiliado}'/>"><button id="reg${numreg.count }" type="button" class="boton" onclick="lock(this, '${numreg.count}');" >descargar</button></a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
