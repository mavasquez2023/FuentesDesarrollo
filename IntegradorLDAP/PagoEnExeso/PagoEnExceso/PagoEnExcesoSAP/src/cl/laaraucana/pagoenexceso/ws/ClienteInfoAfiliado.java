...
import cl.laaraucana.pagoenexceso.vo.AnexoAfiliadoVO;
...
public class ClienteInfoAfiliado implements WSInterface {
	...
	public SalidainfoAfiliadoVO getDataAfiliado(String rut) throws AxisFault { 
		...
		String ep = Config.getConfig("ep.InfoAfiliado");
		...
		logger.debug("Consultando SI_INFO_AFILIADO_OUT Rut: " + rut);
...
//<==== Validacion Nueva	
		if(respuesta!=null){
			...
			salidaListaVO.setMensaje("Respuesta OK servicio SI_INFO_AFILIADO_OUT");
			logger.debug(salidaListaVO.getMensaje());
		}else{
			salidaListaVO.setCodigoError(ConstantesRespuestasWS.COD_RESPUESTA_ERROR);				
			String msg = "RUT no válido";				
			salidaListaVO.setMensaje("Error en servicio SI_INFO_AFILIADO_OUT: " + msg);
			logger.debug(salidaListaVO.getMensaje());
		}
