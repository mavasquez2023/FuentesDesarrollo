	...
	public void connectToFTP(FtpVO paramsFTP) throws FTPErrors {
		...
		try {
			...
			ErrorMessage errorMessage = new ErrorMessage(-1,
					"No fue posible conectarse al FTP a través del host=" + paramsFTP.getServer());
		...
		if (!FTPReply.isPositiveCompletion(reply)) {
			...
			try {
				...
				ErrorMessage errorMessage = new ErrorMessage(-2,
						"No fue posible conectarse al FTP, el host=" + paramsFTP.getServer() + " entregó la respuesta=" + reply);
				logger.error(errorMessage.toString());
				throw new FTPErrors(errorMessage);
			}
		}
		...
		try {
			...
			ErrorMessage errorMessage = new ErrorMessage(-3,
					"El usuario=" + paramsFTP.getUsuario() + ", y el pass=**** no fueron vÃ¡lidos para la autenticación.");
			logger.error(errorMessage.toString());
			throw new FTPErrors(errorMessage);
		}
