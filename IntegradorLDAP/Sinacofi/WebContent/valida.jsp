...
<html lang="es">
  <head>
    ...
    <title>Sinacofi -  La Araucana</title>
              ...
              <div class="row justify-center-xs justify-start-sm">
                <div class="col xs12 sm6 md4">
                  <div class="pasos__item flex-xs align-middle-xs justify-center-xs justify-start-sm activo">
                    ...
                    <div class="pasos__data"><span>Paso 1</span>Ingresa RUT y Número Serie</div>
                  </div>
                </div>
              </div>
            </div>
            <div class="separador"></div>
            <p>Este servicio te permitirá validar la conexión a Sinacofi</p>
            <div class="separador"></div>
            <div class="row">
              <div class="col xs12 lg6">
                <form class="form" id="paso1" action="validaSinacofi.do" method="post">
                  ...
                  <c:choose>
                  	<c:when test='${mensaje=="cedula_vigente" }'>
                  		<div class="alerta alerta--exito">Cédula Vigente!.</div>
                  	</c:when>
                 	<c:when test='${mensaje=="cedula_error" }'>
                  		<div class="alerta alerta--aviso">Número de serie de cédula de identidad ingresada no es válido</div>
                  	</c:when>
                  	<c:when test='${mensaje=="servicio_error" }'>
                  		<div class="alerta alerta--error">Servicio Sinacofi no se encuentra disponible</div>
                  	</c:when>
                  	<c:when test='${mensaje==null }'>
                  	</c:when>
                  	<c:otherwise>
                  		<div class="alerta alerta--aviso">${mensaje}</div>
                  	</c:otherwise>
                  </c:choose>
                  ...
                  <div class="form__grupo" data-animacion="placeholder" data-comentario="Ej 123456XXX o A021065XXX" style="position:relative">
                   ...
                   <div class="form__grupo" data-animacion="placeholder" data-comentario="">
                    <input class="text requerido" type="text" id="url" name="url" value="${url}"/>
                    <label for="rut">URL</label>
                  </div>
                   <div class="form__grupo" data-animacion="placeholder" data-comentario="">
                    <input class="text requerido" type="text" id="usuario" name="usuario" value="${usuario}"/>
                    <label for="rut">Usuario</label>
                  </div>
                   <div class="form__grupo" data-animacion="placeholder" data-comentario="">
                    <input class="text requerido" type="text" id="clave" name="clave" value="${clave}"/>
                    <label for="rut">Clave</label>
                  </div>
                  <div class="separador"></div>
                  <div class="alerta alerta--error" id="errores_paso1" style="display:none"></div>
                  <div class="btn__grupo text-align-right-xs"><a class="btn btn--primario" href="#" onClick="validarPaso1(); return false;">Validar Cédula</a>
                  </div>
                </form>
              </div>
            </div>
