...
var validarPaso2 = function validarPaso2() {
  ...
  if ($('#email').val() === $('#confirmar-email').val() && $('#email').val().length && $('#confirmar-email').val().length) {
    _emailValidos = true;
  }
  ...
  validarFormulario('paso2', {
    texto: 'Debe ingresar ',
    otrosCampos: {
      'Coincidir Emails': [_emailValidos, $('#paso2 .grupo-emails input')],
...
var validarRecuperarClavePaso1 = function validarRecuperarClavePaso1() {
  ...
  validarFormulario('recuperarClavePaso1', {
    texto: 'Debe ingresar ',
    otrosCampos: {
      /*'Captcha': [captchaValido, $('#recuperarClavePaso1').find('.captcha')]*/
    },
    exito: function exito() {
      /*
        Ingresar metodo para procesar el formulario aquí
