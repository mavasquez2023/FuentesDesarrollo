/*
LibrerÃ­a de automatizaciÃ³n para jQuery
Autor: FÃ¨line Interactive
...
patrones['tipo_texto'] = /[a-zA-Z \-'Ã¡Ã©Ã­Ã³ÃºÃ�Ã‰Ã�Ã“ÃšÃ±Ã‘Ã¤Ã«Ã¯Ã¶Ã¼Ã„Ã‹Ã�Ã–ÃœÃ¢ÃªÃ®Ã´Ã»Ã‚ÃŠÃŽÃ”Ã›]/;
...
antipatrones['tipo_texto'] = /[^a-zA-Z \-'Ã¡Ã©Ã­Ã³ÃºÃ�Ã‰Ã�Ã“ÃšÃ±Ã‘Ã¤Ã«Ã¯Ã¶Ã¼Ã„Ã‹Ã�Ã–ÃœÃ¢ÃªÃ®Ã´Ã»Ã‚ÃŠÃŽÃ”Ã›]/g;
...
function automatizar_init () {
  // readonly para los calendarios si estÃ¡ disponible el type="date"
  if (dateHabilitado) {
  ...
  $(':text, :password, textarea, select').each(function (i, el) {
    ...
    // forzar a que se validen los campos que no sean requeridos en caso de tener algÃºn caracter
  ...
  // automatizaciÃ³n para inputs type="password"
  $(':password').each(function (i, el) {
    $(el).data('tipo', 'tipo_password');
    aplicarEventos(el);
  });

  // automatizaciÃ³n para inputs de texto y textarea
  $(':text, textarea').each(function (i, el) {
    ...
    // bloqueo de teclas segÃºn patrones
    for (key in patrones) {
      if ($(el).hasClass(key)) {
        // patrones
        $(el).data('tipo', key);
  ...
  // crÃ©ditos
  console.log(
    'Se cargÃ³ la librerÃ­a de automatizaciÃ³n de FÃ¨line Interactive v' +
      version +
      '.'
  );
}
