...
function automatizar_init(){
	...
	$(':text, :password, textarea, select').each(function(i,el){
		...
		var valor="";
		if($(el).hasClass('requerido')){ $(el).data('requerido',true); }
		if($(el).prop("tagName")!="SELECT"){
			valor=$(el)[0].defaultValue;
		}
		else{
			valor=$(el).find('option[selected]').val();
			if(typeof(valor)=="undefined"){ valor=$(el).find("option:first").val(); }
		}
		$(el).data('value',valor);
		setTimeout(function(){
			$(el).val(valor);
		...
		$(el).blur(function(){
			...
			if($(this).prop("tagName")=="SELECT" && $(this).parent().hasClass('select')){ $(this).parent().removeClass('activo'); }
		});
	...
	$(':text, textarea').each(function(i,el){
		...
		for(key in patrones){
			if($(el).hasClass(key)){
				...
				$(el).bind('paste',function(event){
					...
					if(typeof(window.clipboardData)!="undefined"){ var data=window.clipboardData.getData("Text"); } //para IE
					else{ var data=event.originalEvent.clipboardData.getData('Text'); }
					var pasteValue=data.replace(antipatrones[$(el).data('tipo')],'');
					if($(el).attr('maxlength')){ pasteValue=pasteValue.substr(0,parseInt($(el).attr('maxlength'))); }
		...
		var tiposNoConsiderados=['tipo_email','tipo_rut','tipo_telefono','tipo_nombre'];
		var validar=1;
	...
	$('select').each(function(i,el){
		...
		$(this).change(function(){
			if($(this).parent().hasClass('select')){
				if(this.value){ aplicarClases($(this).parent(),validarSelect(this.value)); }
				else{ aplicarClases($(this).parent(),''); }
			}
			if(this.value){ aplicarClases(this,validarSelect(this.value)); }
			else{ aplicarClases(this,''); }
		});
		$(this).blur(function(){
			if($(this).parent().hasClass('select')){
				if(this.value){ aplicarClases($(this).parent(),validarSelect(this.value)); }
	...
	if(!soportado('textarea','maxLength')){
		$('textarea').each(function(i,el){
			...
			$(el).keypress(function(event){
				...
				if($.inArray(code,habilitados)>=0){ return true; }
	...
	//créditos
	console.log("Se cargó la librería de automatización de Fèline Interactive v"+version+".");
}
...
function aplicarEventos(el){
	...
	if(tipo=="tipo_password"){
		...
		$(el).blur(function(){
			var minLength=6;
			if(typeof($(el).data('minlength'))=="number"){ minLength=$(el).data('minlength'); }
	...
	if(tipo=="tipo_nombre"){
		...
		$(el).blur(function(){
			this.value=this.value.trim();
			if(($(this).data("requerido")) || (this.value && !$(this).data("placeholder")) || ($(this).data("placeholder") && this.value!=$(this).data("placeholder"))){
	...
	if(tipo=="tipo_telefono"){
		...
		$(el).blur(function(){
			if(!$(this).data("requerido") && this.value==$(this).data("valueRequerido")){ aplicarClases(this,''); }
			else{ aplicarClases(this,validarTelefono(this.value)); }
		});
		bloquearCampo(el,"+");
	}
	if(tipo=="tipo_email"){
...
function bloquearCampo(el,val){
	...
	if($(el).val()){}
	else if($(el).data('placeholder')){ $(el).val($(el).data('placeholder')); }
	else{ $(el).val(""); }
	...
	$(el).click(function(event){
		if(this.value==$(this).data("valueRequerido")){
			...
			this.setSelectionRange($(this).data("valueRequerido").length,$(this).data("valueRequerido").length);
		}
	});
	$(el).keypress(function(event){
		var code=event.which;
		if(this.selectionStart===0 && this.selectionEnd===this.value.length){ //todo seleccionado
			return verificarPatron(event,el);
		}
		else if(obtenerPosicionCursor($(el)[0])==0){
			if($.inArray(code,habilitados)<0){ return false; }
		}
	});
	$(el).keydown(function(event){
		var code=event.which;
		var continuar=true;
		if(obtenerPosicionCursor($(el)[0])==val.length){ //bloquear el backspace
			if(code===8){ continuar=false; }
		}
		else if(obtenerPosicionCursor($(el)[0])<val.length){ //bloquear el delete
			if(this.selectionStart===0 && this.selectionEnd===this.value.length){}
			else if(code===46){ continuar=false; }
		}
	...
	$(el).keyup(function(event){
		if(this.value==$(this).data("valueRequerido")){
			...
			if (code!=9 && code!=16) { //no considerar la tecla tab ni la tecla shift
				this.setSelectionRange($(this).data("valueRequerido").length,$(this).data("valueRequerido").length);
			}
			else{
				this.setSelectionRange(this.value.length, this.value.length);
		...
		if(this.value.indexOf($(this).data("valueRequerido"))!=0){
			if(!this.value || this.value==$(this).data("valueRequerido")){ this.value=$(this).data("valueRequerido"); }
			else if(nuevoValor.length==1){ this.value=$(this).data("valueRequerido")+nuevoValor; }
			else if($(this).data("textoAntes").indexOf($(this).data("valueRequerido"))==0){ this.value=$(this).data("textoAntes")+this.value; }
			else{ this.value=$(this).data("valueRequerido")+$(this).data("textoAntes"); }
		}
	});
	$(el).blur(function(){
		if(this.value==$(this).data("valueRequerido")){
			if($(el).data('placeholder')){ $(el).val($(el).data('placeholder')); }
			else{ $(el).val(""); }
		}
	});
}
...
function aplicarClases(el,clase){
	...
	for(i in clases){
		...
		if(campo_error.length>0){ campo_error.hide('fast'); }
	}
	if(clase){
		$(el).addClass(clase);
		var campo_error=$('#error_'+$(el).attr('id'));
		if(campo_error.length>0 && clase=="invalido"){ campo_error.show('fast'); }
		else{ campo_error.hide('fast'); }
	}
}
...
function verificarPatron(event,el){
	var tipo=$(el).data("tipo");
	var code = event.which;
	if(event.ctrlKey && code==99){ return true; } //permitir control+c
	if(event.ctrlKey && code==118){ return true; } //permitir control+v
	if($.inArray(code,habilitados)>=0){ return true; }
...
function validarRut(rut){
	...
	if(!rut || !dv){ estado='invalido'; }
	else if(digito!=dv){ estado='invalido'; }
	else { estado='valido'; }
...
function validarTelefono(telefono){
	var estado='invalido';
	if(telefono.length>=10){ estado='valido'; }
	return estado;
}
...
function validarNombre(nombre){
	...
	if(nombres.length>1 && nombres[0]!="" && nombres[1]!="" && nombre.length>4){ estado='valido'; }
	return estado;
}
...
function validarEmail(email){
	var estado='valido';
	var patron=/^(.+\@.+\..+)$/;
	if(!patron.test(email)){ estado='invalido'; }
	return estado;
}

function validarSelect(valor){
	valor=valor.trim();
	if(valor){ return 'valido'; }
	else{ return 'invalido'; }
}

function validarBasico(valor){
	valor=valor.trim();
	if(valor.length){ return 'valido'; }
	else{ return 'invalido'; }
}

function validarMinimo(valor,minimo){
	valor=valor.trim();
	if(valor.length>=minimo){ return 'valido'; }
	else{ return 'invalido'; }
}

function formatearErrores(errores,texto,separador){
	if(!texto){ texto="Por favor ingrese su "; }
	if(!separador){ separador="y"; }
	for(i=0;i<errores.length;i++){
		texto+=errores[i];
		if(i<errores.length-2){ texto+=", "; }
		else if(i==errores.length-2){ texto+=" "+separador+" "; }
	}
	return texto;
}
...
function validarFormulario(id_formulario,variables){
	...
	if(typeof(variables.action)=="undefined"){ variables.action=""; }
	if(typeof(variables.texto)=="undefined"){ variables.texto=""; }
	if(typeof(variables.separador)=="undefined"){ variables.separador=""; }
	if(typeof(variables.otrosCampos)=="undefined"){ variables.otrosCampos={}; }
	...
	if(typeof(variables.error)!="function"){
		variables.error=function(){
			if($("#errores_"+id_formulario).length>0){
				...
				if(toFormularios){ clearTimeout(toFormularios); }
	...
	$("#"+id_formulario+" .requerido:visible").each(function(i,el){
		$(el).blur();
		var valido=1;
		if($(el).attr("type")=="password" || $(el).attr("type")=="date" || $(el).attr("type")=="email" || $(el).attr("type")=="text" || el.tagName.toLowerCase()=="select" || el.tagName.toLowerCase()=="textarea"){
	...
	if(errores.length){
		var errores_formateados=formatearErrores(errores,variables.texto,variables.separador);
		variables.error();
	}
	else{
		variables.exito();
...
function limpiarFormulario(id_formulario){
	...
	$("#"+id_formulario+" :text, #"+id_formulario+" textarea, #"+id_formulario+" select").each(function(i,el){
		...
		if("#error_"+$(el).attr('id').length){ $("#error_"+$(el).attr('id')).hide('fast'); }
	});
	if($("#errores_"+id_formulario).length){ $("#errores_"+id_formulario).slideUp('fast'); }
}

function subir(tiempo){
	if(typeof(tiempo)!="number"){ tiempo=800; }
	$('html, body').animate({
		scrollTop: 0
	}, tiempo);
	return false;
...
function paralaxes(items,variables){
	if(typeof(variables)!="object"){ variables={}; }
	if(typeof(variables.factor)!="number"){ variables.factor=5; }
	if(typeof(variables.offset)!="number"){ variables.offset=0; }
	items.each(function(i,el){
		...
		if(!$(el).data('factor')){ $(el).data('factor',variables.factor); }
		img.css({
			'position':'absolute',
	...
	$(document).scroll(function(){
		items.each(function(i,el){
			...
			if($(el).parent().data('factor')){ factor=$(el).parent().data('factor'); }
			var distancia=$(document).scrollTop()-$(el).offset().top;
...
function popup(items,variables){
	if(typeof(variables)!="object"){ variables={}; }
	if(typeof(variables.width)!="number"){ variables.width=640; }
	if(typeof(variables.height)!="number"){ variables.height=390; }
	$(items).each(function(i,el){
		$(el).click(function(){
			var url=$(this).attr('href');
			var left = (screen.width/2)-(variables.width/2);
			var top = (screen.height/2)-(variables.height/2);
			window.open(url,'','width='+variables.width+',height='+variables.height+',location=no,menubar=no,scrollbars=no,directories=no,status=no,titlebar=no,toolbar=no,resizable=no,top='+top+',left='+left);
			return false;
		});
	});
}
...
function avisar(texto,tiempo){
	sinTiempo=0;
	if(tiempo==0){ sinTiempo=1; }
	else if(typeof(tiempo)=="undefined"){ tiempo=3000; }
	if(toAviso){
		clearTimeout(toAviso);
		if($('#aviso').length){ $('#aviso').fadeOut('fast'); }
	}
	if(!$('#aviso').length){
		$('body').append($('<div/>', {
			id: 'aviso',
	...
	$('#aviso').html(texto);
	$('#aviso').fadeIn('fast');
	if(!sinTiempo){ toAviso=setTimeout(function(){ $('#aviso').fadeOut('fast'); },tiempo); }
}
/**********************************/