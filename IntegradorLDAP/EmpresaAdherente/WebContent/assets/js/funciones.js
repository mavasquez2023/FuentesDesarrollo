...
var formsPlanillas = function formsPlanillas(id, tipo) {
  validarFormulario(id, {
    texto: 'Debe ingresar ',
    exito: function exito() {
      $.ajax({
        method: 'POST',
        url: '/empresas/ajax/form-planillas.php',
        data: {
          form: $(id).serialize(),
          tipo: tipo
        },
        success: function success(data) {
          ...
          if (_data.status === 'exito') {
            alert('exito'); // $('.exito-lightbox').slideDown();
            // setTimeout(function (){
            //   $('.exito-lightbox').slideUp();
            // }, 3000);
          } else {
            alert('error'); // $('.error-lightbox').slideDown();
            // setTimeout(function (){
            //   $('.error-lightbox').slideUp();
            // }, 3000);
          }
